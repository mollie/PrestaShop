...
it('DE / 100 EUR / 3 Klarna Payments should be visible', () => {
      ...
      cy.session (LoginFO, () => {
      cy.visit('http://172.17.0.1:8001/fr/accueil/20-testproduct1.html')
...
it('DE / <45 EUR / Klarna Slice It. should not be visible', () => {
      ...
      fasterLogin('LoginFO')
      cy.visit('http://172.17.0.1:8001/en/art/4-16-the-adventure-begins-framed-poster.html#')
      cy.get('.add > .btn').click()
      cy.get('.cart-content-btn > .btn-primary').click()
      cy.get('.text-sm-center > .btn').click()
      cy.contains('GERMANY').click()
...
it('FR / <35 EUR / Klarna Pay Later should not be visible', () => {
      ...
      cy.visit('http://172.17.0.1:8001/en/home-accessories/8-mug-today-is-a-good-day.html')
      cy.get('a > .material-icons').click()
      cy.get('[class="form-control"]').type(('demo@demo.com'), {log: false})
      cy.get('[class="form-control js-child-focus js-visible-password"]').type(('demodemo'), {log: false})
      cy.get('[class="btn btn-primary"]').click()
