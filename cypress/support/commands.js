...
// Open the dropdown menu (wait until it is visible and enabled)
    cy.get('.btn-group-action > .btn-group > .dropdown-toggle')
        .eq(0)
        .should('be.visible')
        .and('not.be.disabled')
        .click()
...
Cypress.Commands.add("OrderShippingRefundingOrdersAPI", () => {
    cy.visit('/admin1/index.php?controller=AdminOrders')
    cy.get(':nth-child(1) > .column-payment')
        .should('be.visible')
        .click()
    cy.scrollTo('bottom')
    // Shipping button in React
    cy.get('.btn-group > [title=""]')
        .eq(0)
        .should('be.visible')
        .and('not.be.disabled')
        .click()
...
Cypress.Commands.add("OrderRefundingPartialPaymentsAPI", () => {
    ...
    cy.get('#mollie_order > :nth-child(1)')
        .should('be.visible')
        .click()
    cy.get('.form-inline > :nth-child(1) > .btn')
        .should('exist')
        .and('be.visible')
...
Cypress.Commands.add("advancedSettingsValidation", () => {
    ...
    cy.get('#mollie-advanced-settings-root').should('contain.text', 'Order settings')
    cy.get('#mollie-advanced-settings-root').should('contain.text', 'Shipping')
    cy.get('#mollie-advanced-settings-root').should('contain.text', 'Visual')
    cy.get('#mollie-advanced-settings-root').contains('Save').click({force:true})
    cy.get('#mollie-advanced-settings-root', {timeout: 10000}).should('contain', 'saved successfully')
})
