...
use MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage;
use MolliePrefix\Symfony\Component\ExpressionLanguage\ParsedExpression;
use MolliePrefix\Symfony\Component\ExpressionLanguage\Tests\Fixtures\TestProvider;
class ExpressionLanguageTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    public function testCachedParse()
    {
        $cacheMock = $this->getMockBuilder('MolliePrefix\\Psr\\Cache\\CacheItemPoolInterface')->getMock();
        $cacheItemMock = $this->getMockBuilder('MolliePrefix\\Psr\\Cache\\CacheItemInterface')->getMock();
        $savedParsedExpression = null;
        $expressionLanguage = new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage($cacheMock);
        ...
        $cacheItemMock->expects($this->exactly(1))->method('set')->with($this->isInstanceOf(\MolliePrefix\Symfony\Component\ExpressionLanguage\ParsedExpression::class))->willReturnCallback(function ($parsedExpression) use(&$savedParsedExpression) {
    ...
    {
        $cacheMock = $this->getMockBuilder('MolliePrefix\\Symfony\\Component\\ExpressionLanguage\\ParserCache\\ParserCacheInterface')->getMock();
        $savedParsedExpression = null;
        $expressionLanguage = new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage($cacheMock);
        $cacheMock->expects($this->exactly(1))->method('fetch')->with('1%20%2B%201%2F%2F')->willReturn($savedParsedExpression);
        $cacheMock->expects($this->exactly(1))->method('save')->with('1%20%2B%201%2F%2F', $this->isInstanceOf(\MolliePrefix\Symfony\Component\ExpressionLanguage\ParsedExpression::class))->willReturnCallback(function ($key, $expression) use(&$savedParsedExpression) {
    ...
    public function testWrongCacheImplementation()
    {
        ...
        $cacheMock = $this->getMockBuilder('MolliePrefix\\Psr\\Cache\\CacheItemSpoolInterface')->getMock();
        new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage($cacheMock);
    }
    public function testConstantFunction()
    {
        $expressionLanguage = new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage();
    ...
    {
        $expressionLanguage = new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage(null, [new \MolliePrefix\Symfony\Component\ExpressionLanguage\Tests\Fixtures\TestProvider()]);
    ...
    public function testRegisterAfterParse($registerCallback)
    {
        ...
        $el = new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage();
        $el->parse('1 + 1', []);
        $registerCallback($el);
    }
    /**
    ...
    {
        return [[function (\MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage $el) {
            $el->register('fn', function () {
            }, function () {
            });
        }], [function (\MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage $el) {
            $el->addFunction(new \MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionFunction('fn', function () {
            }, function () {
            }));
        }], [function (\MolliePrefix\Symfony\Component\ExpressionLanguage\ExpressionLanguage $el) {
            $el->registerProvider(new \MolliePrefix\Symfony\Component\ExpressionLanguage\Tests\Fixtures\TestProvider());
        }]];
    }
}
