...
class FinderTest extends \MolliePrefix\Symfony\Component\Finder\Tests\Iterator\RealIteratorTestCase
{
    public function testCreate()
    {
        $this->assertInstanceOf('MolliePrefix\\Symfony\\Component\\Finder\\Finder', \MolliePrefix\Symfony\Component\Finder\Finder::create());
    }
    ...
    public function testIgnoreVCS()
    {
        ...
        $this->assertIterator($this->toAbsolute(['.git', 'foo', 'foo/bar.tmp', 'test.php', 'test.py', 'toto', 'toto/.git', '.bar', '.foo', '.foo/.bar', '.foo/bar', 'foo bar']), $finder->in(self::$tmpDir)->getIterator());
        $finder = $this->buildFinder();
        $this->assertSame($finder, $finder->ignoreVCS(\true)->ignoreDotFiles(\false));
    ...
    public function testIgnoreDotFiles()
    {
        ...
        $this->assertIterator($this->toAbsolute(['.git', '.bar', '.foo', '.foo/.bar', '.foo/bar', 'foo', 'foo/bar.tmp', 'test.php', 'test.py', 'toto', 'toto/.git', 'foo bar']), $finder->in(self::$tmpDir)->getIterator());
        $finder = $this->buildFinder();
        $this->assertSame($finder, $finder->ignoreDotFiles(\true)->ignoreVCS(\false));
    ...
    public function testIn()
    {
        ...
        $expected = [self::$tmpDir . \DIRECTORY_SEPARATOR . 'test.php', __DIR__ . \DIRECTORY_SEPARATOR . 'FinderTest.php', __DIR__ . \DIRECTORY_SEPARATOR . 'GlobTest.php'];
    ...
    public function testGetIteratorWithoutIn()
    {
        ...
        $finder = \MolliePrefix\Symfony\Component\Finder\Finder::create();
        $finder->getIterator();
    }
    ...
    {
        $this->assertInstanceOf('MolliePrefix\\Symfony\\Component\\Finder\\Finder', \MolliePrefix\Symfony\Component\Finder\Finder::create()->append([]));
    }
    public function testAppendDoesNotRequireIn()
    {
        $finder = $this->buildFinder();
        $finder->in(self::$tmpDir . \DIRECTORY_SEPARATOR . 'foo');
        $finder1 = \MolliePrefix\Symfony\Component\Finder\Finder::create()->append($finder);
        $this->assertIterator(\iterator_to_array($finder->getIterator()), $finder1->getIterator());
    }
    public function testCountDirectories()
    {
        $directory = \MolliePrefix\Symfony\Component\Finder\Finder::create()->directories()->in(self::$tmpDir);
        $i = 0;
    ...
    {
        $files = \MolliePrefix\Symfony\Component\Finder\Finder::create()->files()->in(__DIR__ . \DIRECTORY_SEPARATOR . 'Fixtures');
        $i = 0;
    ...
    {
        $this->expectException('LogicException');
        $finder = \MolliePrefix\Symfony\Component\Finder\Finder::create()->files();
        \count($finder);
    }
    ...
    public function testMultipleLocations()
    {
        ...
        $finder = new \MolliePrefix\Symfony\Component\Finder\Finder();
        $finder->in($locations)->ignoreDotFiles(\false)->depth('< 1')->name('test.php');
    ...
    {
        $locations = [__DIR__ . '/Fixtures/one', self::$tmpDir . \DIRECTORY_SEPARATOR . 'toto'];
        $finder = $this->buildFinder();
        $finder->in($locations)->depth('< 10')->name('*.neon');
        $expected = [__DIR__ . '/Fixtures/one' . \DIRECTORY_SEPARATOR . 'b' . \DIRECTORY_SEPARATOR . 'c.neon', __DIR__ . '/Fixtures/one' . \DIRECTORY_SEPARATOR . 'b' . \DIRECTORY_SEPARATOR . 'd.neon'];
    ...
    public function getTestPathData()
    {
        ...
        ]], ['/^with space\\//', 'foobar', ['with space' . \DIRECTORY_SEPARATOR . 'foo.txt']]];
    }
    public function testAccessDeniedException()
    {
        ...
        if (\false === ($couldRead = \is_readable($testDir))) {
            try {
                ...
                $expectedExceptionClass = 'MolliePrefix\\Symfony\\Component\\Finder\\Exception\\AccessDeniedException';
                if ($e instanceof \MolliePrefix\PHPUnit\Framework\ExpectationFailedException) {
                    $this->fail(\sprintf("Expected exception:\n%s\nGot:\n%s\nWith comparison failure:\n%s", $expectedExceptionClass, 'MolliePrefix\\PHPUnit\\Framework\\ExpectationFailedException', $e->getComparisonFailure()->getExpectedAsString()));
                }
    ...
    {
        return \MolliePrefix\Symfony\Component\Finder\Finder::create();
    }
}
