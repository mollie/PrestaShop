 ...
 */
abstract class AbstractCache implements \MolliePrefix\Psr\SimpleCache\CacheInterface, \MolliePrefix\Psr\Log\LoggerAwareInterface, \MolliePrefix\Symfony\Component\Cache\ResettableInterface
{
    /**
     * @internal
     */
    ...
    public function getMultiple($keys, $default = null)
    {
        ...
        try {
            ...
            \MolliePrefix\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch requested values', ['keys' => $keys, 'exception' => $e]);
            $values = [];
        }
    ...
    public function setMultiple($values, $ttl = null)
    {
        ...
        \MolliePrefix\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to save values', ['keys' => $keys, 'exception' => $e instanceof \Exception ? $e : null]);
        return \false;
    }
    /**
     * {@inheritdoc}
    ...
    private function generateValues($values, &$keys, $default)
    {
        try {
            ...
            \MolliePrefix\Symfony\Component\Cache\CacheItem::log($this->logger, 'Failed to fetch requested values', ['keys' => \array_values($keys), 'exception' => $e]);
        }
        foreach ($keys as $key) {
            (yield $key => $default);
        }
