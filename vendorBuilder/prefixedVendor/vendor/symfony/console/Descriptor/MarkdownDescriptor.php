 ...
 * @internal
 */
class MarkdownDescriptor extends \MolliePrefix\Symfony\Component\Console\Descriptor\Descriptor
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function describeCommand(\MolliePrefix\Symfony\Component\Console\Command\Command $command, array $options = [])
    {
        ...
        $this->write('`' . $command->getName() . "`\n" . \str_repeat('-', \MolliePrefix\Symfony\Component\Console\Helper\Helper::strlen($command->getName()) + 2) . "\n\n" . ($command->getDescription() ? $command->getDescription() . "\n\n" : '') . '### Usage' . "\n\n" . \array_reduce(\array_merge([$command->getSynopsis()], $command->getAliases(), $command->getUsages()), function ($carry, $usage) {
            return $carry . '* `' . $usage . '`' . "\n";
        }));
    ...
    protected function describeApplication(\MolliePrefix\Symfony\Component\Console\Application $application, array $options = [])
    {
        ...
        $this->write($title . "\n" . \str_repeat('=', \MolliePrefix\Symfony\Component\Console\Helper\Helper::strlen($title)));
        foreach ($description->getNamespaces() as $namespace) {
            if (\MolliePrefix\Symfony\Component\Console\Descriptor\ApplicationDescription::GLOBAL_NAMESPACE !== $namespace['id']) {
                $this->write("\n\n");
    ...
    private function getApplicationTitle(\MolliePrefix\Symfony\Component\Console\Application $application)
    {
