...
class InlineControlStructureSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_WHILE || $tokens[$stackPtr]['code'] === \T_FOR) {
            ...
            if (isset($tokens[$stackPtr]['parenthesis_closer']) === \true) {
                $afterParensCloser = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $tokens[$stackPtr]['parenthesis_closer'] + 1, null, \true);
        ...
        $nextNonEmpty = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $start + 1, null, \true);
        ...
        $nextContent = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, null, \true);
        ...
        if ($next !== $end) {
            if ($nextContent === \false || $tokens[$nextContent]['line'] !== $tokens[$end]['line']) {
                ...
                if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$endLine]['code']]) === \false && ($tokens[$endLine]['code'] !== \T_WHITESPACE || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$endLine - 1]['code']]) === \false)) {
                    $endLine = $end;
                }
            }
            if ($endLine !== $end) {
