...
class DisallowComparisonAssignmentSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        $array = $phpcsFile->findNext(\T_ARRAY, $stackPtr + 1, null, \false, null, \true);
        if ($array !== \false) {
            return;
        }
        // Ignore function calls.
        $ignore = [\T_STRING, \T_WHITESPACE, \T_OBJECT_OPERATOR];
        ...
        for ($i = $stackPtr + 1; $i < $endStatement; $i++) {
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$comparisonTokens[$tokens[$i]['code']]) === \true && $tokens[$i]['code'] !== \T_COALESCE || $tokens[$i]['code'] === T_INLINE_THEN) {
                $error = 'The value of a comparison must not be assigned to a variable';
                $phpcsFile->addError($error, $stackPtr, 'AssignedComparison');
                break;
            }
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$booleanOperators[$tokens[$i]['code']]) === \true || $tokens[$i]['code'] === T_BOOLEAN_NOT) {
