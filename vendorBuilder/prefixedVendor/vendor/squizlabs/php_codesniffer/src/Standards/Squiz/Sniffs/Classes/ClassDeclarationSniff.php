...
use MolliePrefix\PHP_CodeSniffer\Standards\PSR2\Sniffs\Classes\ClassDeclarationSniff as PSR2ClassDeclarationSniff;
use MolliePrefix\PHP_CodeSniffer\Util\Tokens;
class ClassDeclarationSniff extends \MolliePrefix\PHP_CodeSniffer\Standards\PSR2\Sniffs\Classes\ClassDeclarationSniff
{
    /**
     ...
     * @return void
     */
    public function processOpen(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
     ...
     * @return void
     */
    public function processClose(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        for ($nextContent = $closeBrace + 1; $nextContent < $phpcsFile->numTokens; $nextContent++) {
            ...
            if ($tokens[$nextContent]['line'] === $tokens[$closeBrace]['line'] && ($tokens[$nextContent]['code'] === \T_WHITESPACE || $tokens[$nextContent]['code'] === \T_COMMENT || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$nextContent]['code']]) === \true)) {
                continue;
            }
