 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Squiz\Sniffs\ControlStructures;
...
class ControlSignatureSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        $nextNonEmpty = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        if (isset($tokens[$stackPtr]['scope_opener']) === \true) {
            ...
            for ($next = $opener + 1; $next < $phpcsFile->numTokens; $next++) {
                ...
                if ($tokens[$next]['line'] === $tokens[$opener]['line'] && (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$code]) === \true || $code === \T_CLOSE_TAG)) {
                    continue;
                }
            ...
            if ($tokens[$stackPtr]['code'] === \T_WHILE) {
                ...
                $closer = $tokens[$stackPtr]['parenthesis_closer'];
                $nextNonEmpty = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $closer + 1, null, \true);
        ...
        if ($tokens[$stackPtr]['code'] === \T_WHILE) {
            ...
            $closer = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        ...
        if ($found !== 1) {
            ...
            $data = [$found];
            if ($phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens, $closer + 1, $stackPtr) !== \false) {
