...
use MolliePrefix\PHP_CodeSniffer\Reports\Report;
...
class Reporter
{
    ...
    public function __construct(\MolliePrefix\PHP_CodeSniffer\Config $config)
    {
        ...
        foreach ($config->reports as $type => $output) {
            ...
            if (\strpos($type, '.') !== \false) {
                ...
                $reportClassName = \MolliePrefix\PHP_CodeSniffer\Autoload::loadFile($filename);
            } else {
                if (\class_exists('PHP_CodeSniffer\\Reports\\' . \ucfirst($type)) === \true) {
                    ...
                    if (\class_exists($type) === \true) {
                        ...
                        $registeredNamespaces = \MolliePrefix\PHP_CodeSniffer\Autoload::getSearchPaths();
            ...
            if ($reportClass instanceof \MolliePrefix\PHP_CodeSniffer\Reports\Report === \false) {
                throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException('Class "' . $reportClassName . '" must implement the "PHP_CodeSniffer\\Report" interface.');
            }
     ...
     * @return void
     */
    public function cacheFileReport(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile)
    {
     ...
     * @return array
     */
    public function prepareFileReport(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile)
    {
        $report = ['filename' => \MolliePrefix\PHP_CodeSniffer\Util\Common::stripBasepath($phpcsFile->getFilename(), $this->config->basepath), 'errors' => $phpcsFile->getErrorCount(), 'warnings' => $phpcsFile->getWarningCount(), 'fixable' => $phpcsFile->getFixableCount(), 'messages' => []];
