...
class DummyNode extends \MolliePrefix\PhpParser\NodeAbstract
{
    public $subNode1;
    public $subNode2;
...
class NodeAbstractTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    public function provideNodes()
    {
        $attributes = array('startLine' => 10, 'comments' => array(new \MolliePrefix\PhpParser\Comment('// Comment' . "\n"), new \MolliePrefix\PhpParser\Comment\Doc('/** doc comment */')));
        $node = new \MolliePrefix\PhpParser\DummyNode('value1', 'value2', $attributes);
     ...
     */
    public function testConstruct(array $attributes, \MolliePrefix\PhpParser\Node $node)
    {
     ...
     * @dataProvider provideNodes
     */
    public function testGetDocComment(array $attributes, \MolliePrefix\PhpParser\Node $node)
    {
        $this->assertSame('/** doc comment */', $node->getDocComment()->getText());
    ...
    {
        $node = new \MolliePrefix\PhpParser\DummyNode(null, null, []);
        // Add doc comment to node without comments
        $docComment = new \MolliePrefix\PhpParser\Comment\Doc('/** doc */');
        $node->setDocComment($docComment);
        $this->assertSame($docComment, $node->getDocComment());
        // Replace it
        $docComment = new \MolliePrefix\PhpParser\Comment\Doc('/** doc 2 */');
        $node->setDocComment($docComment);
        $this->assertSame($docComment, $node->getDocComment());
        // Add docmment to node with other comments
        $c1 = new \MolliePrefix\PhpParser\Comment('/* foo */');
        $c2 = new \MolliePrefix\PhpParser\Comment('/* bar */');
        $docComment = new \MolliePrefix\PhpParser\Comment\Doc('/** baz */');
     ...
     * @dataProvider provideNodes
     */
    public function testChange(array $attributes, \MolliePrefix\PhpParser\Node $node)
    {
        // change of line
        $node->setLine(15);
     ...
     * @dataProvider provideNodes
     */
    public function testIteration(array $attributes, \MolliePrefix\PhpParser\Node $node)
    {
    ...
    {
        /** @var $node Node */
        $node = $this->getMockForAbstractClass('MolliePrefix\\PhpParser\\NodeAbstract');
