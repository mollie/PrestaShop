 ...
 */
namespace MolliePrefix\PhpCsFixer\Indicator;
...
{
    public function isPhpUnitClass(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        if (!$tokens[$index]->isGivenKind(\T_CLASS)) {
            throw new \LogicException(\sprintf('No T_CLASS at given index %d, got %s.', $index, $tokens[$index]->getName()));
        }
        $index = $tokens->getNextMeaningfulToken($index);
        if (0 !== \MolliePrefix\PhpCsFixer\Preg::match('/(?:Test|TestCase)$/', $tokens[$index]->getContent())) {
            return \true;
        }
        while (null !== ($index = $tokens->getNextMeaningfulToken($index))) {
            ...
            if (0 !== \MolliePrefix\PhpCsFixer\Preg::match('/(?:Test|TestCase)(?:Interface)?$/', $tokens[$index]->getContent())) {
                return \true;
            }
        }
        return \false;
     ...
     */
    public function findPhpUnitClasses(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $beginAtBottom = \true)
    {
