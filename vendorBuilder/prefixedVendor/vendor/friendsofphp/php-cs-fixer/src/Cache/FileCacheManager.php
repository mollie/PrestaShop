 ...
 * @internal
 */
final class FileCacheManager implements \MolliePrefix\PhpCsFixer\Cache\CacheManagerInterface
{
    /**
     ...
     * @param bool $isDryRun
     */
    public function __construct(\MolliePrefix\PhpCsFixer\Cache\FileHandlerInterface $handler, \MolliePrefix\PhpCsFixer\Cache\SignatureInterface $signature, $isDryRun = \false, \MolliePrefix\PhpCsFixer\Cache\DirectoryInterface $cacheDirectory = null)
    {
        $this->handler = $handler;
        $this->signature = $signature;
        $this->isDryRun = $isDryRun;
        $this->cacheDirectory = $cacheDirectory ?: new \MolliePrefix\PhpCsFixer\Cache\Directory('');
        $this->readCache();
    }
    ...
    private function readCache()
    {
        ...
        if (!$cache || !$this->signature->equals($cache->getSignature())) {
            $cache = new \MolliePrefix\PhpCsFixer\Cache\Cache($this->signature);
        }
        $this->cache = $cache;
    }
