...
run-ps-for-tests:
	composer install
	docker network create prestashop-net-1.7.8-7.4-apache
	docker run -ti --name some-mysql-1.7.8-7.4-apache --network prestashop-net-1.7.8-7.4-apache -e MYSQL_ROOT_PASSWORD=admin -e MYSQL_DATABASE=prestashop -p 3307:3306 -d mysql:5.7
	docker run -ti -v $(ROOT_DIR):/var/www/html/modules/mollie -v $(ROOT_DIR)/.docker/php/php.ini:/usr/local/etc/php/conf.d/custom-php.ini --name some-prestashop-1.7.8-7.4-apache --network prestashop-net-1.7.8-7.4-apache -e DB_SERVER=some-mysql-1.7.8-7.4-apache -e PS_INSTALL_AUTO=1 -e DB_NAME=prestashop -e PS_DOMAIN=localhost:8080 -e PS_FOLDER_ADMIN=admin1 -p 8080:80 -d prestashop/prestashop:1.7.8-7.4-apache
	docker exec -i some-prestashop-1.7.8-7.4 sh -c "cd /var/www/html/modules/mollie && php vendor/bin/phpunit -c tests/phpunit.xml --testsuite Unit"
