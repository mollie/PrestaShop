...
run-ps-for-tests:
	...
	docker run -ti --name some-mysql-1.7.8-7.4 --network prestashop-net-1.7.8-7.4 -e MYSQL_ROOT_PASSWORD=admin -e MYSQL_DATABASE=prestashop -p 3307:3306 -d mysql:5.7
	docker exec -i some-prestashop-1.7.8-7.4 bash -c "cd /var/www/html/ && php bin/console prestashop:module install mollie"
	docker exec -i some-prestashop-1.7.8-7.4 bash -c "cd /var/www/html/modules/mollie && php vendor/bin/phpunit -c tests/phpunit.xml --filter ArrayUtilityTest"
