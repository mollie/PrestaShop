...
class Emulative extends \MolliePrefix\PhpParser\Lexer
{
    ...
    public function __construct(array $options = array())
    {
        ...
        $this->tokenMap[self::T_COALESCE] = \MolliePrefix\PhpParser\Parser\Tokens::T_COALESCE;
        $this->tokenMap[self::T_SPACESHIP] = \MolliePrefix\PhpParser\Parser\Tokens::T_SPACESHIP;
        $this->tokenMap[self::T_YIELD_FROM] = \MolliePrefix\PhpParser\Parser\Tokens::T_YIELD_FROM;
        if (\version_compare(\PHP_VERSION, self::PHP_5_6, '>=')) {
            return;
        }
        $this->tokenMap[self::T_ELLIPSIS] = \MolliePrefix\PhpParser\Parser\Tokens::T_ELLIPSIS;
        $this->tokenMap[self::T_POW] = \MolliePrefix\PhpParser\Parser\Tokens::T_POW;
        $this->tokenMap[self::T_POW_EQUAL] = \MolliePrefix\PhpParser\Parser\Tokens::T_POW_EQUAL;
    }
    ...
    public function getNextToken(&$value = null, &$startAttributes = null, &$endAttributes = null)
    {
        ...
        if (\MolliePrefix\PhpParser\Parser\Tokens::T_STRING === $token && !$this->inObjectAccess) {
            if (isset($this->newKeywords[\strtolower($value)])) {
                return $this->newKeywords[\strtolower($value)];
            }
        } else {
            // keep track of whether we currently are in an object access (after ->)
            $this->inObjectAccess = \MolliePrefix\PhpParser\Parser\Tokens::T_OBJECT_OPERATOR === $token;
        }
        return $token;
    }
}
