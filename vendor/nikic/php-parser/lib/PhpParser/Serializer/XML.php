...
use MolliePrefix\PhpParser\Serializer;
use XMLWriter;
/**
 * @deprecated
 */
class XML implements \MolliePrefix\PhpParser\Serializer
{
    protected $writer;
    /**
    ...
    protected function _serialize($node)
    {
        if ($node instanceof \MolliePrefix\PhpParser\Node) {
            ...
            $this->writer->writeAttribute('isDocComment', $node instanceof \MolliePrefix\PhpParser\Comment\Doc ? 'true' : 'false');
