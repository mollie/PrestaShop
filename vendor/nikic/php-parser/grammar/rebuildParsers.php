...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \MolliePrefix\resolveNodes($grammarCode);
    $grammarCode = \MolliePrefix\resolveMacros($grammarCode);
    $grammarCode = \MolliePrefix\resolveStackAccess($grammarCode);
    ...
    $resultCode = \MolliePrefix\removeTrailingWhitespace($resultCode);
    \MolliePrefix\ensureDirExists($resultDir);
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \MolliePrefix\resolveNodes($matches['params']);
        $params = \MolliePrefix\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \MolliePrefix\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \MolliePrefix\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' == $name) {
            \MolliePrefix\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' == $name) {
            \MolliePrefix\assertArgs(1, $args, $name);
        ...
        if ('push' == $name) {
            \MolliePrefix\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' == $name) {
            \MolliePrefix\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\MolliePrefix\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
