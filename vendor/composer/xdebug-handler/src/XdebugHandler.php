 ...
 */
namespace MolliePrefix\Composer\XdebugHandler;
...
class XdebugHandler
{
    ...
    public function __construct($envPrefix, $colorOption = '')
    {
        ...
        $this->statusWriter = new \MolliePrefix\Composer\XdebugHandler\Status($this->envAllowXdebug, (bool) $this->debug);
    }
    /**
    ...
    {
        $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::CHECK, $this->loaded);
        $envArgs = \explode('|', (string) \getenv($this->envAllowXdebug));
        if (empty($envArgs[0]) && $this->requiresRestart((bool) $this->loaded)) {
            // Restart required
            $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::RESTART);
        ...
        if (self::RESTART_ID === $envArgs[0] && \count($envArgs) === 5) {
            ...
            $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::RESTARTED);
            \MolliePrefix\Composer\XdebugHandler\Process::setEnv($this->envAllowXdebug);
            self::$inRestart = \true;
            if (!$this->loaded) {
        ...
        $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::NORESTART);
    ...
    private function prepareRestart()
    {
        ...
        if ($error) {
            $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::ERROR, $error);
        }
        return empty($error);
    }
    /**
    ...
    private function getCommand()
    {
        ...
        if (\defined('STDOUT') && \MolliePrefix\Composer\XdebugHandler\Process::supportsColor(\STDOUT)) {
            $args = \MolliePrefix\Composer\XdebugHandler\Process::addColorOption($args, $this->colorOption);
        }
        $args = \array_merge($php, array($this->script), $args);
        $cmd = \MolliePrefix\Composer\XdebugHandler\Process::escape(\array_shift($args), \true, \true);
        foreach ($args as $arg) {
            $cmd .= ' ' . \MolliePrefix\Composer\XdebugHandler\Process::escape($arg);
        }
        return $cmd;
    }
    /**
    ...
    private function setEnvRestartSettings($envArgs)
    {
        ...
        \MolliePrefix\Composer\XdebugHandler\Process::setEnv(self::RESTART_SETTINGS, \implode('|', $settings));
    }
    /**
    ...
    private function syncSettings(array $settings)
    {
        if (\false === \getenv($this->envOriginalInis)) {
            ...
            \MolliePrefix\Composer\XdebugHandler\Process::setEnv($this->envOriginalInis, \implode(\PATH_SEPARATOR, $settings['inis']));
        }
        self::$skipped = $settings['skipped'];
        $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::INFO, 'Process called with existing restart settings');
    }
    /**
    ...
    {
        if (!\function_exists('\MolliePrefix\pcntl_async_signals')) {
            return;
        }
        ...
        $this->notify(\MolliePrefix\Composer\XdebugHandler\Status::INFO, $message);
    }
}
