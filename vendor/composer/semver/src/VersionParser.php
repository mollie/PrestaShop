...
use MolliePrefix\Composer\Semver\Constraint\EmptyConstraint;
...
class VersionParser
{
    ...
    public function parseConstraints($constraints)
    {
        ...
        foreach ($orConstraints as $constraints) {
            ...
            if (1 === \count($constraintObjects)) {
                ...
                $constraint = new \MolliePrefix\Composer\Semver\Constraint\MultiConstraint($constraintObjects);
            }
            $orGroups[] = $constraint;
        }
        if (1 === \count($orGroups)) {
            $constraint = $orGroups[0];
        } elseif (2 === \count($orGroups) && $orGroups[0] instanceof \MolliePrefix\Composer\Semver\Constraint\MultiConstraint && $orGroups[1] instanceof \MolliePrefix\Composer\Semver\Constraint\MultiConstraint && 2 === \count($orGroups[0]->getConstraints()) && 2 === \count($orGroups[1]->getConstraints()) && ($a = (string) $orGroups[0]) && \strpos($a, '[>=') === 0 && \false !== ($posA = \strpos($a, '<', 4)) && ($b = (string) $orGroups[1]) && \strpos($b, '[>=') === 0 && \false !== ($posB = \strpos($b, '<', 4)) && \substr($a, $posA + 2, -1) === \substr($b, 4, $posB - 5)) {
            $constraint = new \MolliePrefix\Composer\Semver\Constraint\MultiConstraint(array(new \MolliePrefix\Composer\Semver\Constraint\Constraint('>=', \substr($a, 4, $posA - 5)), new \MolliePrefix\Composer\Semver\Constraint\Constraint('<', \substr($b, $posB + 2, -1))));
        } else {
            $constraint = new \MolliePrefix\Composer\Semver\Constraint\MultiConstraint($orGroups, \false);
        }
    ...
    private function parseConstraint($constraint)
    {
        ...
        if (\preg_match('{^v?[xX*](\\.[xX*])*$}i', $constraint)) {
            return array(new \MolliePrefix\Composer\Semver\Constraint\EmptyConstraint());
        }
        ...
        if (\preg_match('{^~>?' . $versionRegex . '$}i', $constraint, $matches)) {
            ...
            $lowerBound = new \MolliePrefix\Composer\Semver\Constraint\Constraint('>=', $lowVersion);
            // For upper bound, we increment the position of one more significance,
            // but highPosition = 0 would be illegal
            $highPosition = \max(1, $position - 1);
            $highVersion = $this->manipulateVersionString($matches, $highPosition, 1) . '-dev';
            $upperBound = new \MolliePrefix\Composer\Semver\Constraint\Constraint('<', $highVersion);
        ...
        if (\preg_match('{^v?(\\d++)(?:\\.(\\d++))?(?:\\.(\\d++))?(?:\\.[xX*])++$}', $constraint, $matches)) {
            ...
            if ($lowVersion === '0.0.0.0-dev') {
                return array(new \MolliePrefix\Composer\Semver\Constraint\Constraint('<', $highVersion));
            }
            return array(new \MolliePrefix\Composer\Semver\Constraint\Constraint('>=', $lowVersion), new \MolliePrefix\Composer\Semver\Constraint\Constraint('<', $highVersion));
        }
        // Hyphen Range
        //
        ...
        if (\preg_match('{^(?P<from>' . $versionRegex . ') +- +(?P<to>' . $versionRegex . ')($)}i', $constraint, $matches)) {
            ...
            $lowerBound = new \MolliePrefix\Composer\Semver\Constraint\Constraint('>=', $lowVersion . $lowStabilitySuffix);
            $empty = function ($x) {
        ...
        if (\preg_match('{^(<>|!=|>=?|<=?|==?)?\\s*(.*)}', $constraint, $matches)) {
            try {
                ...
                return array(new \MolliePrefix\Composer\Semver\Constraint\Constraint($matches[1] ?: '=', $version));
            } catch (\Exception $e) {
            }
        }
