 ...
 */
namespace MolliePrefix\PhpCsFixer\Diff\v3_0\Output;

use MolliePrefix\PhpCsFixer\Diff\v3_0\ConfigurationException;
 ...
 */
final class StrictUnifiedDiffOutputBuilder implements \MolliePrefix\PhpCsFixer\Diff\v3_0\Output\DiffOutputBuilderInterface
{
    /**
     * @var bool
     */
    ...
    public function __construct(array $options = [])
    {
        ...
        if (!\is_bool($options['collapseRanges'])) {
            throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\ConfigurationException('collapseRanges', 'a bool', $options['collapseRanges']);
        }
        if (!\is_int($options['contextLines']) || $options['contextLines'] < 0) {
            throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\ConfigurationException('contextLines', 'an int >= 0', $options['contextLines']);
        }
        if (!\is_int($options['commonLineThreshold']) || $options['commonLineThreshold'] <= 0) {
            throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\ConfigurationException('commonLineThreshold', 'an int > 0', $options['commonLineThreshold']);
        }
        foreach (['fromFile', 'toFile'] as $option) {
            if (!\is_string($options[$option])) {
                throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\ConfigurationException($option, 'a string', $options[$option]);
            }
        }
        foreach (['fromFileDate', 'toFileDate'] as $option) {
            if (null !== $options[$option] && !\is_string($options[$option])) {
                throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\ConfigurationException($option, 'a string or <null>', $options[$option]);
            }
        }
    ...
    private function writeDiffHunks($output, array $diff)
    {
        ...
        if (0 === $diff[$upperLimit - 1][1]) {
            ...
            if ("\n" !== $lc) {
                \array_splice($diff, $upperLimit, 0, [["\n\\ No newline at end of file\n", \MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::NO_LINE_END_EOF_WARNING]]);
            }
        } else {
        ...
        foreach ($diff as $i => $entry) {
            ...
            $sameCount = 0;
            if ($entry[1] === \MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::NO_LINE_END_EOF_WARNING) {
                continue;
            }
            $this->changed = \true;
            if (\false === $hunkCapture) {
            ...
            if (\MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::ADDED === $entry[1]) {
                // added
                ++$toRange;
            }
            if (\MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::REMOVED === $entry[1]) {
                // removed
                ++$fromRange;
            }
        }
    ...
    private function writeHunk(array $diff, $diffStartIndex, $diffEndIndex, $fromStart, $fromRange, $toStart, $toRange, $output)
    {
        ...
        for ($i = $diffStartIndex; $i < $diffEndIndex; ++$i) {
            if ($diff[$i][1] === \MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::ADDED) {
                $this->changed = \true;
                \fwrite($output, '+' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::REMOVED) {
                $this->changed = \true;
                \fwrite($output, '-' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::OLD) {
                \fwrite($output, ' ' . $diff[$i][0]);
            } elseif ($diff[$i][1] === \MolliePrefix\PhpCsFixer\Diff\v3_0\Differ::NO_LINE_END_EOF_WARNING) {
                $this->changed = \true;
                \fwrite($output, $diff[$i][0]);
            }
