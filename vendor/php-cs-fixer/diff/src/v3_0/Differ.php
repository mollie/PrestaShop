...
use MolliePrefix\PhpCsFixer\Diff\v3_0\Output\DiffOutputBuilderInterface;
use MolliePrefix\PhpCsFixer\Diff\v3_0\Output\UnifiedDiffOutputBuilder;
/**
 * Diff implementation.
 */
final class Differ
{
    ...
    {
        if ($outputBuilder instanceof \MolliePrefix\PhpCsFixer\Diff\v3_0\Output\DiffOutputBuilderInterface) {
            $this->outputBuilder = $outputBuilder;
        } elseif (null === $outputBuilder) {
            $this->outputBuilder = new \MolliePrefix\PhpCsFixer\Diff\v3_0\Output\UnifiedDiffOutputBuilder();
            ...
            // @deprecated
            $this->outputBuilder = new \MolliePrefix\PhpCsFixer\Diff\v3_0\Output\UnifiedDiffOutputBuilder($outputBuilder);
        } else {
            throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\InvalidArgumentException(\sprintf('Expected builder to be an instance of DiffOutputBuilderInterface, <null> or a string, got %s.', \is_object($outputBuilder) ? 'instance of "' . \get_class($outputBuilder) . '"' : \gettype($outputBuilder) . ' "' . $outputBuilder . '"'));
        }
    }
    /**
     ...
     * @return string
     */
    public function diff($from, $to, \MolliePrefix\PhpCsFixer\Diff\v3_0\LongestCommonSubsequenceCalculator $lcs = null)
    {
     ...
     * @return array
     */
    public function diffToArray($from, $to, \MolliePrefix\PhpCsFixer\Diff\v3_0\LongestCommonSubsequenceCalculator $lcs = null)
    {
        if (\is_string($from)) {
            $from = $this->splitStringByLines($from);
        } elseif (!\is_array($from)) {
            throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\InvalidArgumentException('"from" must be an array or string.');
        }
        if (\is_string($to)) {
            $to = $this->splitStringByLines($to);
        } elseif (!\is_array($to)) {
            throw new \MolliePrefix\PhpCsFixer\Diff\v3_0\InvalidArgumentException('"to" must be an array or string.');
        }
    ...
    private function selectLcsImplementation(array $from, array $to)
    {
        ...
        if ($this->calculateEstimatedFootprint($from, $to) > $memoryLimit) {
            return new \MolliePrefix\PhpCsFixer\Diff\v3_0\MemoryEfficientLongestCommonSubsequenceCalculator();
        }
        return new \MolliePrefix\PhpCsFixer\Diff\v3_0\TimeEfficientLongestCommonSubsequenceCalculator();
    }
    /**
