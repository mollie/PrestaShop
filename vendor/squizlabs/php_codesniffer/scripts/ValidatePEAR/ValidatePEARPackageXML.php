...
class ValidatePEARPackageXML
{
     ...
     */
    private $validRoles = ['data' => \true, 'doc' => \true, 'ext' => \true, 'extsrc' => \true, 'php' => \true, 'script' => \true, 'src' => \true, 'test' => \true];
    /**
    ...
    public function __construct()
    {
        ...
        $allFiles = (new \MolliePrefix\PHP_CodeSniffer\Tests\FileList($this->projectRoot, $this->projectRoot))->getList();
    ...
    {
        $exitCode = 0;
        if ($this->checkContents() !== \true) {
            $exitCode = 1;
        }
        if ($this->checkPHPRelease() !== \true) {
            $exitCode = 1;
        }
        exit($exitCode);
    }
    ...
    protected function checkContents()
    {
         ...
         */
        $srcFiles = (new \MolliePrefix\PHP_CodeSniffer\Tests\FileList($this->projectRoot . 'src/', $this->projectRoot, '`\\.(css|fixed|inc|js|php|xml)$`Di'))->getList();
        $testsFiles = (new \MolliePrefix\PHP_CodeSniffer\Tests\FileList($this->projectRoot . 'tests/', $this->projectRoot, '`\\.(css|inc|js|php|xml)$`Di'))->getList();
        $files = \array_merge($srcFiles, $testsFiles);
        foreach ($files as $file) {
            if (isset($this->listedContents[$file]) === \true) {
                continue;
            }
    ...
    protected function checkPHPRelease()
    {
        ...
        foreach ($this->packageXML->phprelease as $release) {
            foreach ($release->filelist->install as $install) {
                ...
                if (empty($as) === \true || empty($name) === \true) {
                    continue;
                }
                if (isset($this->allFiles[$name]) === \false) {
                    echo "- File '{$name}' does not exist." . \PHP_EOL;
                ...
                if (\preg_match('`^tests/([^/]+\\.php)$`', $name, $matches) === 1 && ($as === $name || $as === $matches[1])) {
                    continue;
                }
                // Check validity of the tags for files in the tests root subdirectories.
                if (\preg_match('`^tests/.+\\.(php|inc|js|css|xml)$`', $name) === 1 && $as === \str_replace('tests/', 'CodeSniffer/', $name)) {
                    continue;
                }
         ...
         */
        $testFiles = (new \MolliePrefix\PHP_CodeSniffer\Tests\FileList($this->projectRoot . 'tests/', $this->projectRoot, '`\\.(inc|php|js|css|xml)$`Di'))->getList();
        foreach ($testFiles as $file) {
            foreach ($listedFiles as $key => $listed) {
                if (isset($listed[$file]) === \true) {
                    continue;
                }
