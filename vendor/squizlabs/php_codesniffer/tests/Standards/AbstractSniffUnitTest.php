 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Tests\Standards;
...
use MolliePrefix\PHP_CodeSniffer\Files\LocalFile;
use MolliePrefix\PHP_CodeSniffer\Util\Common;
use MolliePrefix\PHPUnit\Framework\TestCase;
abstract class AbstractSniffUnitTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    /**
    ...
    public final function testSniff()
    {
        ...
        $sniffCode = \MolliePrefix\PHP_CodeSniffer\Util\Common::getSniffCode(\get_class($this));
        ...
        if (isset($GLOBALS['PHP_CODESNIFFER_CONFIG']) === \true) {
            ...
            $config = new \MolliePrefix\PHP_CodeSniffer\Config();
            $config->cache = \false;
        ...
        if (isset($GLOBALS['PHP_CODESNIFFER_RULESETS'][$standardName]) === \false) {
            $ruleset = new \MolliePrefix\PHP_CodeSniffer\Ruleset($config);
        ...
        $sniffClassName = \MolliePrefix\PHP_CodeSniffer\Util\Common::cleanSniffClass($sniffClassName);
        ...
        foreach ($testFiles as $testFile) {
            ...
            try {
                ...
                $phpcsFile = new \MolliePrefix\PHP_CodeSniffer\Files\LocalFile($testFile, $ruleset, $config);
                $phpcsFile->process();
     ...
     */
    public function generateFailureMessages(\MolliePrefix\PHP_CodeSniffer\Files\LocalFile $file)
    {
        ...
        if (\is_array($expectedErrors) === \false) {
            throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException('getErrorList() must return an array');
        }
        if (\is_array($expectedWarnings) === \false) {
            throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException('getWarningList() must return an array');
        }
        /*
