 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PSR2\Sniffs\ControlStructures;
...
class SwitchDeclarationSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        while (($nextCase = $this->findNextCase($phpcsFile, $nextCase + 1, $switch['scope_closer'])) !== \false) {
            ...
            if ($tokens[$opener]['code'] === T_COLON) {
                ...
                if ($tokens[$next]['line'] === $tokens[$opener]['line'] && ($tokens[$next]['code'] === \T_COMMENT || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$next]['code']]) === \true)) {
            ...
            if ($tokens[$nextCode]['code'] !== \T_CASE && $tokens[$nextCode]['code'] !== \T_DEFAULT) {
                ...
                if ($nextCode !== \false) {
                    ...
                    if ($tokens[$prevCode]['code'] !== \T_COMMENT && $this->findNestedTerminator($phpcsFile, $opener + 1, $nextCode) === \false) {
    ...
    {
        $tokens = $phpcsFile->getTokens();
        $terminators = [\T_RETURN, \T_BREAK, \T_CONTINUE, \T_THROW, \T_EXIT];
        ...
        if ($lastToken !== \false) {
            if ($tokens[$lastToken]['code'] === T_CLOSE_CURLY_BRACKET) {
                ...
                do {
                    ...
                    $prevToken = $phpcsFile->findPrevious(\T_WHITESPACE, $scopeOpener - 1, $stackPtr, \true);
                    if ($prevToken === \false) {
                        return \false;
                    }
                    ...
                    if ($tokens[$prevToken]['code'] === \T_IF) {
                        ...
                        if ($tokens[$prevToken]['code'] === \T_ELSEIF || $tokens[$prevToken]['code'] === \T_ELSE) {
                            ...
                            $currentCloser = $phpcsFile->findPrevious(\T_WHITESPACE, $prevToken - 1, $stackPtr, \true);
                ...
                if ($tokens[$lastToken]['code'] === T_SEMICOLON) {
                    ...
                    if (\in_array($tokens[$terminator]['code'], $terminators, \true) === \true) {
                        return $terminator;
                    }
                }
            }
