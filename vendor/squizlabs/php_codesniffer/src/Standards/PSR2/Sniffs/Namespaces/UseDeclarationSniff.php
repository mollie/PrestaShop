...
class UseDeclarationSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
            ...
            if ($tokens[$next]['code'] === T_COMMA) {
                ...
                if ($closingCurly === \false) {
                    ...
                    if ($fix === \true) {
                        ...
                        do {
                            $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next + 1, $closingCurly, \true);
                            if ($next === \false) {
                            ...
                            if ($next !== \false) {
                                $nextNonEmpty = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next + 1, $closingCurly, \true);
                        ...
                        $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $closingCurly + 1, null, \true);
        ...
        if ($tokens[$end]['code'] === T_CLOSE_USE_GROUP) {
            $nextNonEmpty = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, null, \true);
        ...
        for ($end = ++$end; $end < $phpcsFile->numTokens; $end++) {
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$end]['code']]) === \false) {
                break;
            }
        ...
        --$end;
        if (($tokens[$end]['code'] === \T_COMMENT || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$end]['code']]) === \true) && \substr($tokens[$end]['content'], 0, 2) === '/*' && \substr($tokens[$end]['content'], -2) !== '*/') {
    ...
    private function shouldIgnoreUse($phpcsFile, $stackPtr)
    {
        ...
        if ($phpcsFile->hasCondition($stackPtr, [\T_CLASS, \T_TRAIT]) === \true) {
            return \true;
        }
        return \false;
    }
