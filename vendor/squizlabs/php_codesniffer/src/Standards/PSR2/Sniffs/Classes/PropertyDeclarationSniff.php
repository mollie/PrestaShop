 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PSR2\Sniffs\Classes;
...
class PropertyDeclarationSniff extends \MolliePrefix\PHP_CodeSniffer\Sniffs\AbstractVariableSniff
{
    /**
    ...
    protected function processMemberVar(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        // repeat errors.
        $find = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeModifiers;
        $find[] = \T_VARIABLE;
        $find[] = \T_VAR;
        $find[] = T_SEMICOLON;
        ...
        if ($propertyInfo['type'] !== '') {
            ...
            if ($tokens[$typeToken + 1]['code'] !== \T_WHITESPACE) {
                ...
                if ($tokens[$typeToken + 1]['content'] !== ' ') {
                    ...
                    $data = [$found];
                    $nextNonWs = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $typeToken + 1, null, \true);
                    if ($nextNonWs !== $next) {
        ...
        if ($propertyInfo['scope_specified'] === \true && $propertyInfo['is_static'] === \true) {
            $scopePtr = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeModifiers, $stackPtr - 1);
