 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PSR12\Sniffs\ControlStructures;

use MolliePrefix\PHP_CodeSniffer\Files\File;
use MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff;
use MolliePrefix\PHP_CodeSniffer\Standards\PSR2\Sniffs\ControlStructures\ControlStructureSpacingSniff as PSR2Spacing;
...
class ControlStructureSpacingSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$parenOpener]['line'] === $tokens[$parenCloser]['line']) {
            ...
            $sniff = new \MolliePrefix\PHP_CodeSniffer\Standards\PSR2\Sniffs\ControlStructures\ControlStructureSpacingSniff();
        ...
        for ($i = $parenOpener; $i < $parenCloser; $i++) {
            if ($tokens[$i]['column'] !== 1 || $tokens[$i + 1]['line'] > $tokens[$i]['line'] || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$i]['code']]) === \true) {
                continue;
            }
            if ($i + 1 === $parenCloser) {
                break;
            ...
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$textStringTokens[$tokens[$i]['code']]) === \true || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$heredocTokens[$tokens[$i]['code']]) === \true) {
                continue;
            }
