 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PSR12\Sniffs\Files;
...
class FileHeaderSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        $possibleHeaders = [];
        $searchFor = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens;
        ...
        do {
            ...
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[$tokens[$next]['code']]) === \true) {
     ...
     * @return array
     */
    public function getHeaderLines(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        $foundDocblock = \false;
        $commentOpeners = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeOpeners;
        ...
        do {
            switch ($tokens[$next]['code']) {
                case T_DOC_COMMENT_OPEN_TAG:
                    ...
                    $docToken = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, null, \true);
                    if (isset($commentOpeners[$tokens[$docToken]['code']]) === \false && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$methodPrefixes[$tokens[$docToken]['code']]) === \false) {
                ...
                case \T_USE:
                    $type = 'use';
                    $useType = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next + 1, null, \true);
                ...
                default:
                    ...
                    if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$next]['code']]) === \true) {
                        $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens, $next + 1, null, \true);
                        if ($next === \false) {
     ...
     * @return int|null
     */
    public function processHeaderLines(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $headerLines)
    {
        $tokens = $phpcsFile->getTokens();
        $found = [];
