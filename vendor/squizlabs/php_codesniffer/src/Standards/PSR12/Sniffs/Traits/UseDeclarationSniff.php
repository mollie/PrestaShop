 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PSR12\Sniffs\Traits;
...
class UseDeclarationSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        \end($conditions);
        if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens[\current($conditions)]) === \false) {
            return;
        }
        ...
        foreach ($useTokens as $usePos => $useToken) {
            if ($usePos === 0) {
                ...
                */
                ...
                for ($i = $useToken - 1; $i > $opener; $i--) {
                    ...
                    if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$i]['code']]) === \true) {
                ...
                if ($tokens[$lastValidContent]['line'] !== $tokens[$opener]['line'] + 1) {
                    ...
                    $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $useToken - 1, $opener - 1, \true);
                ...
                $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $useToken - 1, null, \true);
            ...
            if ($usePos === $numUseTokens - 1) {
                ...
                */
                ...
                if ($next === $tokens[$ooToken]['scope_closer']) {
                    ...
                    if ($tokens[$next]['code'] !== \T_USE) {
                        ...
                        for ($next = $end + 1; $next < $tokens[$ooToken]['scope_closer']; $next++) {
                            ...
                            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$next]['code']]) === \true && $tokens[$next]['line'] === $tokens[$end]['line']) {
                                continue;
                            }
                            break;
                        }
                ...
                $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, null, \true);
     ...
     * @return void
     */
    protected function processUseGroup(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        //end if
        $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $opener + 1, $closer - 1, \true);
        ...
        for ($i = $opener + 1; $i < $closer; $i++) {
            if ($tokens[$i]['code'] === \T_INSTEADOF) {
                ...
                if ($tokens[$i - 1]['code'] !== \T_WHITESPACE) {
                    ...
                    if ($tokens[$i - 1]['content'] !== ' ') {
                        ...
                        $data = [$found];
                        $prevNonWs = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i - 1, $opener, \true);
                        if ($prevNonWs !== $prev) {
                ...
                if ($tokens[$i + 1]['code'] !== \T_WHITESPACE) {
                    ...
                    if ($tokens[$i + 1]['content'] !== ' ') {
                        ...
                        $data = [$found];
                        $nextNonWs = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, $closer, \true);
                        if ($nextNonWs !== $next) {
            ...
            if ($tokens[$i]['code'] === T_SEMICOLON) {
                ...
                $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, $closer - 1, \true);
        ...
        //end for
        $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $closer - 1, $opener + 1, \true);
     ...
     * @return void
     */
    protected function processUseStatement(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
