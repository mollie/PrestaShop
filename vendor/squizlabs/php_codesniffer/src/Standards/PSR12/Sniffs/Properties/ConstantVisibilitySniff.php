 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PSR12\Sniffs\Properties;
...
class ConstantVisibilitySniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($phpcsFile->hasCondition($stackPtr, \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens) === \false) {
            return;
        }
        $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeModifiers[$tokens[$prev]['code']]) === \true) {
            return;
        }
