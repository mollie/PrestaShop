 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Zend\Sniffs\Debug;
...
class CodeAnalyzerSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if (\is_numeric($exitCode) === \true && $exitCode > 0) {
            ...
            throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException("Failed invoking ZendCodeAnalyzer, exitcode was [{$exitCode}], retval was [{$retval}], output was [{$msg}]");
        }
        if (\is_array($output) === \true) {
