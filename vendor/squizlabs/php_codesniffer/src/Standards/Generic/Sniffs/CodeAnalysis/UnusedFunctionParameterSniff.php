 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Generic\Sniffs\CodeAnalysis;
...
class UnusedFunctionParameterSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        $validTokens += \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
        for (; $next <= $end; ++$next) {
            $token = $tokens[$next];
            $code = $token['code'];
            // Ignorable tokens.
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$code]) === \true) {
                continue;
            }
            if ($foundContent === \false) {
                ...
                if ($code === \T_RETURN) {
                    $tmp = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $next + 1, null, \true);
                    ...
                    $tmp = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $tmp + 1, null, \true);
            ...
            for ($i = $lastIndex; $i >= 0; --$i) {
                if ($foundLastUsed !== \false) {
                    if (isset($params[$methodParams[$i]['name']]) === \true) {
                        $errorInfo[$methodParams[$i]['name']] = ['position' => $params[$methodParams[$i]['name']], 'errorcode' => $errorCode . 'BeforeLastUsed'];
                    }
                } else {
                    if (isset($params[$methodParams[$i]['name']]) === \false) {
                        $foundLastUsed = \true;
                    } else {
                        $errorInfo[$methodParams[$i]['name']] = ['position' => $params[$methodParams[$i]['name']], 'errorcode' => $errorCode . 'AfterLastUsed'];
                    }
                }
            }
