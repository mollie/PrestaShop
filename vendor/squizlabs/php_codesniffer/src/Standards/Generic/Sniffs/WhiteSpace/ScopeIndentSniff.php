 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Generic\Sniffs\WhiteSpace;
...
class ScopeIndentSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        $debug = \MolliePrefix\PHP_CodeSniffer\Config::getConfigData('scope_indent_debug');
        if ($debug !== null) {
        ...
        for ($i = $stackPtr + 1; $i < $phpcsFile->numTokens; $i++) {
            ...
            */
            ...
            //end if
            if ($checkToken !== null && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$tokens[$checkToken]['code']]) === \true && \in_array($tokens[$checkToken]['code'], $this->nonIndentingScopes, \true) === \false && isset($tokens[$checkToken]['scope_opener']) === \true) {
                $exact = \true;
                $lastOpener = null;
            ...
            if ($checkToken !== null && $tokens[$checkToken]['code'] === \T_OBJECT_OPERATOR && $exact === \true) {
                $exact = \false;
            }
            if ($checkIndent === null) {
                $checkIndent = $currentIndent;
            ...
            */
            if ($checkToken !== null && isset($this->ignoreIndentation[$tokens[$checkToken]['code']]) === \false && ($tokenIndent !== $checkIndent && $exact === \true || $tokenIndent < $checkIndent && $exact === \false)) {
                ...
                if ($this->tabIndent === \true) {
                    ...
                    $data = [\floor($checkIndent / $this->tabWidth), \floor($tokenIndent / $this->tabWidth)];
                } else {
            ...
            if ($tokens[$i]['code'] === \T_START_HEREDOC || $tokens[$i]['code'] === T_START_NOWDOC) {
                ...
                $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
            ...
            if (isset($tokens[$i]['scope_condition']) === \true && isset($tokens[$i]['scope_opener']) === \true && $tokens[$i]['scope_opener'] === $i) {
                ...
                if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeOpeners[$condition]) === \true && \in_array($condition, $this->nonIndentingScopes, \true) === \false) {
                    if ($this->debug === \true) {
                        $line = $tokens[$i]['line'];
                        $type = $tokens[$tokens[$i]['scope_condition']]['type'];
            ...
            if (isset($tokens[$i]['scope_condition']) === \true && $tokens[$i]['scope_closer'] === $i && ($tokens[$tokens[$i]['scope_condition']]['code'] === T_CLOSURE || $tokens[$tokens[$i]['scope_condition']]['code'] === T_ANON_CLASS)) {
                if ($this->debug === \true) {
                ...
                if ($parens > $object && $parens > $condition) {
                    ...
                    $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $parens - 1, null, \true);
                    $object = 0;
                    $condition = 0;
                } else {
     ...
     * @return bool
     */
    protected function adjustIndent(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, $length, $change)
    {
        $tokens = $phpcsFile->getTokens();
