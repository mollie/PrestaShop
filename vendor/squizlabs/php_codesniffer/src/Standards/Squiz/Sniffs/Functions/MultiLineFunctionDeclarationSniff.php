 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Squiz\Sniffs\Functions;

use MolliePrefix\PHP_CodeSniffer\Standards\PEAR\Sniffs\Functions\FunctionDeclarationSniff as PEARFunctionDeclarationSniff;
use MolliePrefix\PHP_CodeSniffer\Util\Tokens;
class MultiLineFunctionDeclarationSniff extends \MolliePrefix\PHP_CodeSniffer\Standards\PEAR\Sniffs\Functions\FunctionDeclarationSniff
{
    /**
    ...
    public function isMultiLineDeclaration($phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        foreach ($bracketsToCheck as $stackPtr => $openBracket) {
            ...
            while ($tokens[$end]['code'] === T_COMMA) {
                ...
                $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $end + 1, $closeBracket, \true);
                if ($next === \false) {
                    continue 2;
                }
    ...
    public function processSingleLineDeclaration($phpcsFile, $stackPtr, $tokens)
    {
        ...
        if ($tokens[$prevNonWhiteSpace]['line'] !== $tokens[$closingBracket]['line']) {
            ...
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$prevNonWhiteSpace]['code']]) === \true) {
