...
class FunctionDeclarationArgumentSpacingSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function processBracket($phpcsFile, $openBracket)
    {
        ...
        foreach ($params as $paramNumber => $param) {
            ...
            if ($commaToken !== \false) {
                ...
                $checkComma = \true;
                if ($multiLine === \true) {
                    $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $commaToken + 1, $closeBracket, \true);
