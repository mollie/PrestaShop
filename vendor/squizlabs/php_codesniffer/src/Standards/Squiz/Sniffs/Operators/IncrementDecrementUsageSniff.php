 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Squiz\Sniffs\Operators;
...
class IncrementDecrementUsageSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    protected function processIncDec($phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr - 1]['code'] === \T_VARIABLE || $tokens[$stackPtr - 1]['code'] === \T_STRING && $tokens[$stackPtr - 2]['code'] === \T_OBJECT_OPERATOR) {
            $start = $stackPtr + 1;
        } else {
            $start = $stackPtr + 2;
        }
        $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $start, null, \true);
        if ($next === \false) {
            return;
        }
        if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$arithmeticTokens[$tokens[$next]['code']]) === \true) {
            $error = 'Increment and decrement operators cannot be used in an arithmetic operation';
            $phpcsFile->addError($error, $stackPtr, 'NotAllowed');
            return;
        }
        $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $start - 3, null, \true);
        if ($prev === \false) {
            return;
        }
    ...
    protected function processAssignment($phpcsFile, $stackPtr)
    {
        ...
        $noiseTokens = $phpcsFile->findNext([\T_LNUMBER, \T_VARIABLE, \T_WHITESPACE, T_PLUS, T_MINUS, T_OPEN_PARENTHESIS], $stackPtr + 1, $statementEnd, \true);
