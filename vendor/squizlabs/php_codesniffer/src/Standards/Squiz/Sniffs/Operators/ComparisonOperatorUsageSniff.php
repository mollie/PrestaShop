...
class ComparisonOperatorUsageSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_INLINE_THEN) {
            $end = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
            if ($tokens[$end]['code'] !== T_CLOSE_PARENTHESIS) {
                ...
                //end for
                $start = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, null, \true);
            } else {
        ...
        for ($i = $start; $i <= $end; $i++) {
            ...
            if ($type === T_OPEN_PARENTHESIS && isset($tokens[$i]['parenthesis_closer']) === \true && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$functionNameTokens[$tokens[$lastNonEmpty]['code']]) === \true) {
            ...
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$type]) === \false) {
                $lastNonEmpty = $i;
            }
        }
        //end for
