...
class JavaScriptLintSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        $jslPath = \MolliePrefix\PHP_CodeSniffer\Config::getExecutablePath('jsl');
        if ($jslPath === null) {
            return;
        }
        ...
        if ($retval === 2 || $retval === 4) {
            ...
            throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException("Failed invoking JavaScript Lint, retval was [{$retval}], output was [{$msg}]");
        }
        if (\is_array($output) === \true) {
