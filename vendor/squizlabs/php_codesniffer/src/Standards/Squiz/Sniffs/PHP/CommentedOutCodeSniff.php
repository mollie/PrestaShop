...
class CommentedOutCodeSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $stackPtr; $i < $phpcsFile->numTokens; $i++) {
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$i]['code']]) === \false) {
                break;
            }
            if ($tokens[$i]['code'] === \T_WHITESPACE) {
                continue;
            }
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$i]['code']]) === \true) {
        ...
        */
        ...
        if ($phpcsFile->tokenizerType === 'PHP') {
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$stringTokens[$numTokens - 1]['code']]) === \false) {
                return $lastCommentBlockToken + 1;
            }
        ...
        $emptyTokens += \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens;
        $numComment = 0;
        $numPossible = 0;
        $numCode = 0;
        $numNonWhitespace = 0;
        for ($i = 0; $i < $numTokens; $i++) {
            if (isset($emptyTokens[$stringTokens[$i]['code']]) === \true) {
                ...
                if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$comparisonTokens[$stringTokens[$i]['code']]) === \true || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$arithmeticTokens[$stringTokens[$i]['code']]) === \true || $stringTokens[$i]['code'] === T_GOTO_LABEL) {
