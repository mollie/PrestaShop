...
class EmbeddedPhpSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    private function validateInlineEmbeddedPhp($phpcsFile, $stackPtr)
    {
        ...
        $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $closeTag - 1, $stackPtr, \true);
        if ($prev !== $stackPtr) {
        ...
        if ($tokens[$closeTag - 1]['code'] === \T_WHITESPACE) {
            ...
            if (($tokens[$closeTag - 1]['code'] === \T_COMMENT || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$closeTag - 1]['code']]) === \true) && \substr($tokens[$closeTag - 1]['content'], -1) === ' ') {
        ...
        if ($trailingSpace !== 1) {
            ...
            if ($fix === \true) {
                if ($trailingSpace === 0) {
                    ...
                    if ($tokens[$closeTag - 1]['code'] === \T_COMMENT || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$phpcsCommentTokens[$tokens[$closeTag - 1]['code']]) === \true) {
