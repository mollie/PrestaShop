 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Squiz\Sniffs\NamingConventions;
...
class ValidVariableNameSniff extends \MolliePrefix\PHP_CodeSniffer\Sniffs\AbstractVariableSniff
{
    /**
    ...
    protected function processVariable(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$objOperator]['code'] === \T_OBJECT_OPERATOR || $tokens[$objOperator]['code'] === T_NULLSAFE_OBJECT_OPERATOR) {
            ...
            if ($tokens[$var]['code'] === \T_STRING) {
                ...
                if ($tokens[$bracket]['code'] !== T_OPEN_PARENTHESIS) {
                    ...
                    if (\MolliePrefix\PHP_CodeSniffer\Util\Common::isCamelCaps($objVarName, \false, \true, \false) === \false) {
        ...
        if (\substr($varName, 0, 1) === '_') {
            ...
            if ($tokens[$objOperator]['code'] === \T_DOUBLE_COLON) {
                ...
                $inClass = $phpcsFile->hasCondition($stackPtr, \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$ooScopeTokens);
            }
            if ($inClass === \true) {
                $varName = \substr($varName, 1);
            }
        }
        if (\MolliePrefix\PHP_CodeSniffer\Util\Common::isCamelCaps($varName, \false, \true, \false) === \false) {
            $error = 'Variable "%s" is not in valid camel caps format';
            $data = [$originalVarName];
