 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\Squiz\Sniffs\Formatting;
...
class OperatorBracketSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        return \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$operators;
    }
    //end register()
    /**
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_MINUS) {
            ...
            $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr - 1, null, \true);
            ...
            if ($tokens[$number]['code'] === \T_LNUMBER || $tokens[$number]['code'] === \T_DNUMBER) {
                ...
                if ($previous !== \false) {
                    $isAssignment = isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$assignmentTokens[$tokens[$previous]['code']]);
                    $isEquality = isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$equalityTokens[$tokens[$previous]['code']]);
                    $isComparison = isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$comparisonTokens[$tokens[$previous]['code']]);
        ...
        $allowed += \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$operators;
        $lastBracket = \false;
        if (isset($tokens[$stackPtr]['nested_parenthesis']) === \true) {
            ...
            foreach ($parenthesis as $bracket => $endBracket) {
                ...
                //end if
                if (\in_array($prevCode, \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$scopeOpeners, \true) === \true) {
        ...
        //end if
        $lastAssignment = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$assignmentTokens, $stackPtr, null, \false, null, \true);
    ...
    public function addMissingBracketsError($phpcsFile, $stackPtr)
    {
        ...
        for ($before = $stackPtr - 1; $before > 0; $before--) {
            ...
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$before]['code']]) === \true || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$operators[$tokens[$before]['code']]) === \true || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$castTokens[$tokens[$before]['code']]) === \true || isset($allowed[$tokens[$before]['code']]) === \true) {
                continue;
            }
        ...
        //end for
        $before = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $before + 1, null, \true);
        ...
        for ($after = $stackPtr + 1; $after < $phpcsFile->numTokens; $after++) {
            ...
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$after]['code']]) === \true || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$operators[$tokens[$after]['code']]) === \true || isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$castTokens[$tokens[$after]['code']]) === \true || isset($allowed[$tokens[$after]['code']]) === \true) {
                continue;
            }
        ...
        //end for
        $after = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $after - 1, null, \true);
