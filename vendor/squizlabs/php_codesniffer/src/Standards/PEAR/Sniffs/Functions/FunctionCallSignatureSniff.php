 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Standards\PEAR\Sniffs\Functions;
...
class FunctionCallSignatureSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        $tokens = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$functionNameTokens;
        $tokens[] = \T_VARIABLE;
    ...
    public function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        ...
        $openBracket = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        ...
        $search = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
        ...
        if ($tokens[$next]['code'] === T_SEMICOLON) {
            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$closeBracket + 1]['code']]) === \true) {
     ...
     * @return bool
     */
    public function isMultiLineCall(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        $closeBracket = $tokens[$openBracket]['parenthesis_closer'];
     ...
     * @return void
     */
    public function processSingleLineCall(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
            ...
            if ($fix === \true) {
                ...
                if ($spaceBeforeClose === 0) {
                    ...
                    if ($spaceBeforeClose === 'newline') {
                        ...
                        $prev = $closer - 1;
                        while (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$prev]['code']]) === \true) {
     ...
     * @return void
     */
    public function processMultiLineCall(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr, $openBracket, $tokens)
    {
        ...
        if ($tokens[$first]['code'] === \T_CONSTANT_ENCAPSED_STRING && $tokens[$first - 1]['code'] === \T_CONSTANT_ENCAPSED_STRING) {
            ...
            $first = $phpcsFile->findFirstOnLine(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $prev, \true);
            if ($first === \false) {
                $first = $prev + 1;
            }
        }
        ...
        $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $openBracket + 1, null, \true);
        ...
        //end if
        $i = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $openBracket + 1, null, \true);
        ...
        for ($i; $i < $closeBracket; $i++) {
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
                ...
                if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$heredocTokens[$tokens[$i]['code']]) === \true) {
                    continue;
                }
                // Ignore multi-line string indentation.
                if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$stringTokens[$tokens[$i]['code']]) === \true && $tokens[$i]['code'] === $tokens[$i - 1]['code']) {
                    continue;
                }
                // Ignore inline HTML.
            ...
            if ($inArg === \false && $tokens[$i]['code'] === T_COMMA) {
                $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $i + 1, $closeBracket, \true);
                if ($next === \false) {
                    continue;
                }
