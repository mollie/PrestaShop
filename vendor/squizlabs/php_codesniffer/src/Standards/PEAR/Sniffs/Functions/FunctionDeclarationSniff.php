...
use MolliePrefix\PHP_CodeSniffer\Standards\Generic\Sniffs\Functions\OpeningFunctionBraceBsdAllmanSniff;
use MolliePrefix\PHP_CodeSniffer\Standards\Generic\Sniffs\Functions\OpeningFunctionBraceKernighanRitchieSniff;
use MolliePrefix\PHP_CodeSniffer\Util\Tokens;
class FunctionDeclarationSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    {
        if ($tokens[$stackPtr]['code'] === T_CLOSURE) {
            $sniff = new \MolliePrefix\PHP_CodeSniffer\Standards\Generic\Sniffs\Functions\OpeningFunctionBraceKernighanRitchieSniff();
        } else {
            $sniff = new \MolliePrefix\PHP_CodeSniffer\Standards\Generic\Sniffs\Functions\OpeningFunctionBraceBsdAllmanSniff();
        }
    ...
    public function processMultiLineDeclaration($phpcsFile, $stackPtr, $tokens)
    {
        ...
        if ($tokens[$opener]['line'] !== $tokens[$closeBracket]['line']) {
            ...
            if ($fix === \true) {
                $prev = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $opener - 1, $closeBracket, \true);
