 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Util;

use MolliePrefix\PHP_CodeSniffer\Autoload;
...
class Cache
{
     ...
     * @return void
     */
    public static function load(\MolliePrefix\PHP_CodeSniffer\Ruleset $ruleset, \MolliePrefix\PHP_CodeSniffer\Config $config)
    {
        ...
        $codeHashFiles = [];
        $classes = \array_keys(\MolliePrefix\PHP_CodeSniffer\Autoload::getLoadedClasses());
        \sort($classes);
        ...
        $filter = new \RecursiveCallbackFilterIterator($di, function ($file, $key, $iterator) {
            ...
            $filePath = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath($file->getPathname());
            if ($filePath === \false) {
                return \false;
            }
        ...
        $configData = ['phpVersion' => \PHP_VERSION_ID, 'phpExtensions' => $phpExtensionsHash, 'tabWidth' => $config->tabWidth, 'encoding' => $config->encoding, 'recordErrors' => $config->recordErrors, 'annotations' => $config->annotations, 'configData' => \MolliePrefix\PHP_CodeSniffer\Config::getAllConfigData(), 'codeHash' => $codeHash, 'rulesetHash' => $rulesetHash];
        ...
        if ($config->cacheFile !== null) {
            ...
            foreach ($config->files as $file) {
                $file = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath($file);
