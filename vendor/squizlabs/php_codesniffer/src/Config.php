...
class Config
{
    ...
    public function __get($name)
    {
        if (\array_key_exists($name, $this->settings) === \false) {
            throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException("ERROR: unable to get value of property \"{$name}\"");
        }
    ...
    {
        if (\array_key_exists($name, $this->settings) === \false) {
            throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException("Can't __set() {$name}; setting doesn't exist");
        }
        switch ($name) {
            case 'reportWidth':
                // Support auto terminal width.
                if ($value === 'auto' && \function_exists('\MolliePrefix\shell_exec') === \true && \preg_match('|\\d+ (\\d+)|', \shell_exec('stty size 2>&1'), $matches) === 1) {
                    $value = (int) $matches[1];
                } else {
                    $value = (int) $value;
                }
            ...
            case 'standards':
                ...
                $installedStandards = \MolliePrefix\PHP_CodeSniffer\Util\Standards::getInstalledStandards();
    ...
    public function __construct(array $cliArgs = [], $dieOnUnknownArg = \true)
    {
        ...
        if ($this->stdin === \true || \MolliePrefix\PHP_CodeSniffer\Util\Common::isStdinATTY() === \false && \feof($handle) === \false) {
            $readStreams = [$handle];
            $writeSteams = null;
            $fileContents = '';
    ...
    public function processShortArgument($arg, $pos)
    {
        switch ($arg) {
            ...
            case '?':
                ...
                \ob_end_clean();
                throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\DeepExitException($output, 0);
            case 'i':
                \ob_start();
        ...
        switch ($arg) {
            ...
            case 'config-set':
                ...
                try {
                    ...
                    throw new \MolliePrefix\PHP_CodeSniffer\Exceptions\DeepExitException($e->getMessage() . \PHP_EOL, 3);
                }
                ...
                if (\substr($arg, 0, 7) === 'sniffs=') {
                    ...
                    if (\substr($arg, 0, 8) === 'exclude=') {
                        ...
                        if (\defined('PHP_CODESNIFFER_IN_TESTS') === \false && \substr($arg, 0, 6) === 'cache=') {
                            ...
                            $this->cache = \true;
                            self::$overriddenDefaults['cache'] = \true;
                            $this->cacheFile = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\substr($arg, 6));
                            ...
                            if ($this->cacheFile === \false) {
                                ...
                                if ($dir === '.') {
                                    ...
                                    if ($dir[0] === '/') {
                                        // An absolute path.
                                        $dir = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath($dir);
                                    } else {
                                        $dir = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\getcwd() . '/' . $dir);
                                    }
                                    if ($dir !== \false) {
                            ...
                            if (\substr($arg, 0, 10) === 'bootstrap=') {
                                ...
                                foreach ($files as $file) {
                                    $path = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath($file);
                                    if ($path === \false) {
                                ...
                                if (\substr($arg, 0, 10) === 'file-list=') {
                                    ...
                                    $path = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath($fileList);
                                    if ($path === \false) {
                                    ...
                                    if (\substr($arg, 0, 11) === 'stdin-path=') {
                                        ...
                                        $this->stdinPath = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\substr($arg, 11));
                                        ...
                                        if (PHP_CODESNIFFER_CBF === \false && \substr($arg, 0, 12) === 'report-file=') {
                                            ...
                                            $this->reportFile = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\substr($arg, 12));
                                            // It may not exist and return false instead.
                                            if ($this->reportFile === \false) {
                                                $this->reportFile = \substr($arg, 12);
                                                $dir = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\dirname($this->reportFile));
                                                if (\is_dir($dir) === \false) {
                                            ...
                                            if (\substr($arg, 0, 13) === 'report-width=') {
                                                ...
                                                if (\substr($arg, 0, 9) === 'basepath=') {
                                                    ...
                                                    $this->basepath = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\substr($arg, 9));
                                                    // It may not exist and return false instead.
                                                        ...
                                                        if ($arg[6] === '-') {
                                                            ...
                                                            if ($split === \false) {
                                                                ...
                                                                if ($output === \false) {
                                                                    ...
                                                                    $dir = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath(\dirname($output));
                                                                    if (\is_dir($dir) === \false) {
    ...
    public function processFilePath($path)
    {
        ...
        $file = \MolliePrefix\PHP_CodeSniffer\Util\Common::realpath($path);
