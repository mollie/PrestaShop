...
class Code implements \MolliePrefix\PHP_CodeSniffer\Reports\Report
{
    /**
        ...
        foreach ($report['messages'] as $line => $lineErrors) {
            ...
            if (isset($lineTokens[$startLine]) === \true) {
                for ($i = $lineTokens[$startLine]['start']; $i <= $lineTokens[$endLine]['end']; $i++) {
                    ...
                    $tokenContent = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($tokenContent, ["\r", "\n", "\t"]);
