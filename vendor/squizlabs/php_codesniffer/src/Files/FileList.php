...
class FileList implements \Iterator, \Countable
{
     ...
     * @return void
     */
    public function __construct(\MolliePrefix\PHP_CodeSniffer\Config $config, \MolliePrefix\PHP_CodeSniffer\Ruleset $ruleset)
    {
        $this->ruleset = $ruleset;
        $this->config = $config;
        $paths = $config->files;
        foreach ($paths as $path) {
            $isPharFile = \MolliePrefix\PHP_CodeSniffer\Util\Common::isPharFile($path);
    ...
    private function getFilterClass()
    {
        ...
        if ($filterType === null) {
            $filterClass = 'MolliePrefix\\PHP_CodeSniffer\\Filters\\Filter';
        } else {
            if (\strpos($filterType, '.') !== \false) {
                ...
                $filterClass = \MolliePrefix\PHP_CodeSniffer\Autoload::loadFile($filename);
            } else {
    ...
    public function current()
    {
        ...
        if ($this->files[$path] === null) {
            $this->files[$path] = new \MolliePrefix\PHP_CodeSniffer\Files\LocalFile($path, $this->ruleset, $this->config);
        }
