...
class CSS extends \MolliePrefix\PHP_CodeSniffer\Tokenizers\PHP
{
    /**
        ...
        for ($stackPtr = 1; $stackPtr < $numTokens; $stackPtr++) {
            ...
            if ($token['code'] === T_BITWISE_XOR && $tokens[$stackPtr + 1]['content'] === 'PHPCS_CSS_T_OPEN_TAG') {
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    ...
                    $cleanContent = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($content);
            ...
            switch ($token['code']) {
                    ...
                    if ($finalTokens[$stackPtr + 1]['code'] === \T_STRING) {
                        if ($finalTokens[$stackPtr - 1]['code'] === \T_STRING) {
                            ...
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                ...
                                $old = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($finalTokens[$stackPtr + 1]['content']);
                                $new = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($newContent);
                        ...
                        if ($finalTokens[$stackPtr + 1]['code'] === \T_LNUMBER) {
                            ...
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                ...
                                $content = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($finalTokens[$stackPtr + 1]['content']);
                ...
                case T_COLON:
                    ...
                    for ($x = $stackPtr - 1; $x >= 0; $x--) {
                        if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$finalTokens[$x]['code']]) === \false) {
                            break;
                        }
                    }
                    if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                case \T_STRING:
                    if (\strtolower($token['content']) === 'url') {
                        ...
                        for ($y = $x + 1; $y < $numTokens; $y++) {
                            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$finalTokens[$y]['code']]) === \false) {
                                break;
                            }
                        }
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            for ($i = $stackPtr + 1; $i <= $y; $i++) {
                                ...
                                $content = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($finalTokens[$i]['content']);
                        ...
                        if ($newContent !== '') {
                            ...
                            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                                $content = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($finalTokens[$x + 1]['content']);
