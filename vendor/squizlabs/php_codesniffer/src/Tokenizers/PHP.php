 ...
 */
namespace MolliePrefix\PHP_CodeSniffer\Tokenizers;

use MolliePrefix\PHP_CodeSniffer\Util;
class PHP extends \MolliePrefix\PHP_CodeSniffer\Tokenizers\Tokenizer
{
    /**
     ...
     * @var array
     */
    public $endScopeTokens = [T_CLOSE_CURLY_BRACKET => T_CLOSE_CURLY_BRACKET, \T_ENDIF => \T_ENDIF, \T_ENDFOR => \T_ENDFOR, \T_ENDFOREACH => \T_ENDFOREACH, \T_ENDWHILE => \T_ENDWHILE, \T_ENDSWITCH => \T_ENDSWITCH, \T_BREAK => \T_BREAK, \T_END_HEREDOC => \T_END_HEREDOC];
    /**
    ...
    protected function tokenize($string)
    {
        ...
        $commentTokenizer = new \MolliePrefix\PHP_CodeSniffer\Tokenizers\Comment();
        for ($stackPtr = 0; $stackPtr < $numTokens; $stackPtr++) {
            ...
            if (PHP_CODESNIFFER_VERBOSITY > 1) {
                if ($tokenIsArray === \true) {
                    $type = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($token[0]);
                    $content = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($token[1]);
                } else {
                    $newToken = self::resolveSimpleToken($token[0]);
                    $type = $newToken['type'];
                    $content = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($token[0]);
                }
                echo "\tProcess token ";
                if ($tokenIsArray === \true) {
                    echo "[{$stackPtr}]";
            ...
            //end if
            if ($newStackPtr > 0 && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$finalTokens[$newStackPtr - 1]['code']]) === \false) {
            ...
            */
            if (\PHP_VERSION_ID >= 80000 && $tokenIsArray === \true && ($token[0] === \T_COMMENT && \strpos($token[1], '//') === 0) && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE) {
            ...
            if (\PHP_VERSION_ID < 70000 && \PHP_VERSION_ID >= 50500 && $tokenIsArray === \true && $token[0] === \T_YIELD && isset($tokens[$stackPtr + 1]) === \true && isset($tokens[$stackPtr + 2]) === \true && $tokens[$stackPtr + 1][0] === \T_WHITESPACE && $tokens[$stackPtr + 2][0] === \T_STRING && \strtolower($tokens[$stackPtr + 2][1]) === 'from') {
                ...
                if (PHP_CODESNIFFER_VERBOSITY > 1) {
                    for ($i = $stackPtr + 1; $i <= $stackPtr + 2; $i++) {
                        $type = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($tokens[$i][0]);
                        $content = \MolliePrefix\PHP_CodeSniffer\Util\Common::prepareForOutput($tokens[$i][1]);
            ...
            */
            if (\PHP_VERSION_ID < 70400 && ($tokenIsArray === \true && ($token[0] === \T_LNUMBER || $token[0] === \T_DNUMBER) && isset($tokens[$stackPtr + 1]) === \true && \is_array($tokens[$stackPtr + 1]) === \true && $tokens[$stackPtr + 1][0] === \T_STRING && $tokens[$stackPtr + 1][1][0] === '_')) {
                ...
                $newToken['type'] = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($newType);
            ...
            */
            if ($tokenIsArray === \false && $token[0] === '?') {
                ...
                for ($i = $stackPtr + 1; $i < $numTokens; $i++) {
                    ...
                    if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \true) {
                        continue;
                    }
                    if ($tokenType === \T_STRING || $tokenType === \T_ARRAY || $tokenType === \T_NS_SEPARATOR) {
                        $lastRelevantNonEmpty = $tokenType;
                        continue;
                    }
                    if ($tokenType !== \T_CALLABLE && isset($lastRelevantNonEmpty) === \false || $lastRelevantNonEmpty === \T_ARRAY && $tokenType === '(' || $lastRelevantNonEmpty === \T_STRING && ($tokenType === \T_DOUBLE_COLON || $tokenType === '(' || $tokenType === ':')) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                ...
                for ($i = $stackPtr - 1; $i >= 0; $i--) {
                    ...
                    if ($prevNonEmpty === null && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \false) {
                    ...
                    if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokenType]) === \false) {
                        $lastSeenNonEmpty = $tokenType;
                    }
                }
                //end for
            ...
            */
            if ($stackPtr > 1 && (\is_array($tokens[$stackPtr - 1]) === \true && $tokens[$stackPtr - 1][0] === \T_PAAMAYIM_NEKUDOTAYIM) && $tokenIsArray === \true && $token[0] !== \T_STRING && $token[0] !== \T_VARIABLE && $token[0] !== T_DOLLAR && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$token[0]]) === \false) {
                $newToken = [];
            ...
            */
            if ($tokenIsArray === \true && ($token[0] === \T_FUNCTION || $token[0] === T_FN) && $finalTokens[$lastNotEmptyToken]['code'] !== \T_USE) {
                if ($token[0] === \T_FUNCTION) {
                    ...
                    if ($x < $numTokens && \is_array($tokens[$x]) === \true) {
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            $oldType = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($tokens[$x][0]);
                ...
                */
                ...
                if ($parenthesisCloser !== \false) {
                    ...
                    if (isset($tokens[$x]) === \true && \is_array($tokens[$x]) === \false && $tokens[$x] === ':') {
                        $allowed = [\T_STRING => \T_STRING, \T_ARRAY => \T_ARRAY, \T_CALLABLE => \T_CALLABLE, T_SELF => T_SELF, T_PARENT => T_PARENT, \T_NS_SEPARATOR => \T_NS_SEPARATOR];
                        $allowed += \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
                        // Find the start of the return type.
                        for ($x += 1; $x < $numTokens; $x++) {
                            if (\is_array($tokens[$x]) === \true && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$tokens[$x][0]]) === \true) {
            ...
            */
            if ($tokenIsArray === \true && \strpos($token[1], $this->eolChar) !== \false) {
                ...
                $numLines = \count($tokenLines);
                $newToken = ['type' => \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($token[0]), 'code' => $token[0], 'content' => ''];
                ...
                if ($tokenIsArray === \true && $token[0] === \T_STRING) {
                    ...
                    $context = [\T_OBJECT_OPERATOR => \true, \T_FUNCTION => \true, \T_CLASS => \true, \T_EXTENDS => \true, \T_IMPLEMENTS => \true, \T_NEW => \true, \T_CONST => \true, \T_NS_SEPARATOR => \true, \T_USE => \true, \T_NAMESPACE => \true, \T_PAAMAYIM_NEKUDOTAYIM => \true];
            ...
            if ($this->tokens[$i]['code'] === \T_FUNCTION) {
                ...
                */
                ...
                if ($this->tokens[$i]['code'] === \T_CLASS && isset($this->tokens[$i]['scope_opener']) === \true) {
                    ...
                    */
                    for ($x = $i + 1; $x < $numTokens; $x++) {
                        if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === \false) {
                            break;
                        }
                    }
                    ...
                    if ($this->tokens[$i]['code'] === T_FN && isset($this->tokens[$i + 1]) === \true) {
                        ...
                        if (isset($this->tokens[$x]) === \true && $this->tokens[$x]['code'] === T_OPEN_PARENTHESIS) {
                            $ignore = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens;
                            $ignore += [\T_STRING => \T_STRING, \T_ARRAY => \T_ARRAY, T_COLON => T_COLON, \T_NS_SEPARATOR => \T_NS_SEPARATOR, T_NULLABLE => T_NULLABLE, \T_CALLABLE => \T_CALLABLE, T_PARENT => T_PARENT, T_SELF => T_SELF, \T_STATIC => \T_STATIC];
                            $closer = $this->tokens[$x]['parenthesis_closer'];
                        ...
                        if ($this->tokens[$i]['code'] === T_OPEN_SQUARE_BRACKET) {
                            ...
                            $isShortArray = \false;
                            $allowed = [T_CLOSE_SQUARE_BRACKET => T_CLOSE_SQUARE_BRACKET, T_CLOSE_CURLY_BRACKET => T_CLOSE_CURLY_BRACKET, T_CLOSE_PARENTHESIS => T_CLOSE_PARENTHESIS, \T_VARIABLE => \T_VARIABLE, \T_OBJECT_OPERATOR => \T_OBJECT_OPERATOR, \T_STRING => \T_STRING, \T_CONSTANT_ENCAPSED_STRING => \T_CONSTANT_ENCAPSED_STRING];
                            $allowed += \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$magicConstants;
                            for ($x = $i - 1; $x >= 0; $x--) {
                            ...
                            if ($this->tokens[$i]['code'] === \T_STATIC) {
                                ...
                                if ($this->tokens[$i]['code'] === T_TRUE || $this->tokens[$i]['code'] === T_FALSE || $this->tokens[$i]['code'] === T_NULL) {
                                    ...
                                    $context = [\T_OBJECT_OPERATOR => \true, \T_NS_SEPARATOR => \true, \T_PAAMAYIM_NEKUDOTAYIM => \true];
                    ...
                    if ($oldCond === $this->tokens[$i]['code']) {
                        ...
                        if (PHP_CODESNIFFER_VERBOSITY > 1) {
                            ...
                            foreach ($oldConditions as $condition) {
                                $oldConds .= \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                            }
                            $oldConds = \rtrim($oldConds, ',');
                            $newConds = '';
                            foreach ($this->tokens[$x]['conditions'] as $condition) {
                                $newConds .= \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($condition) . ',';
                            }
    ...
    public static function standardiseToken($token)
    {
        ...
        if ($token[0] === \T_STRING) {
            ...
            if ($token[0] === \T_CURLY_OPEN) {
                ...
                $newToken = ['code' => $token[0], 'type' => \MolliePrefix\PHP_CodeSniffer\Util\Tokens::tokenName($token[0])];
