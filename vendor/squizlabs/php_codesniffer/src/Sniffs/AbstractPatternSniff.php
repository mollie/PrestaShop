...
use MolliePrefix\PHP_CodeSniffer\Tokenizers\PHP;
use MolliePrefix\PHP_CodeSniffer\Exceptions\RuntimeException;
abstract class AbstractPatternSniff implements \MolliePrefix\PHP_CodeSniffer\Sniffs\Sniff
{
    /**
    ...
    private function getListenerTokenPos($pattern)
    {
        ...
        $token = \MolliePrefix\PHP_CodeSniffer\Util\Tokens::getHighestWeightedToken($tokenCodes);
     ...
     * @see    register()
     */
    public final function process(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
     ...
     * @return array
     */
    protected function processPattern($patternInfo, \MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
        $tokens = $phpcsFile->getTokens();
        ...
        if ($this->ignoreComments === \true) {
            $ignoreTokens += \MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens;
        }
                    ...
                    if ($pattern[$i]['type'] === 'skip') {
                        ...
                        if ($pattern[$i]['type'] === 'string') {
                            ...
                            if ($pattern[$i]['type'] === 'newline') {
                                if ($this->ignoreComments === \true && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$stackPtr]['code']]) === \true) {
                                    $startComment = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens, $stackPtr - 1, null, \true);
                                ...
                                if ($tokens[$stackPtr]['code'] === \T_WHITESPACE) {
                                    if ($tokens[$stackPtr]['content'] !== $phpcsFile->eolChar) {
                                        ...
                                        if ($tokens[$stackPtr - 1]['content'] !== $phpcsFile->eolChar && ($this->ignoreComments === \true && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$stackPtr - 1]['code']]) === \false)) {
                                            $hasError = \true;
                                        } else {
                                            $stackPtr--;
                                        }
            ...
            if ($pattern[$i]['type'] === 'token') {
                if ($pattern[$i]['token'] === \T_WHITESPACE) {
                    if ($this->ignoreComments === \true) {
                        ...
                        if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$stackPtr]['code']]) === \true) {
                            continue;
                        }
                        // If the next token is a comment, the we need to skip the
                        // current token as we should allow a space before a
                        // comment for readability.
                        if (isset($tokens[$stackPtr + 1]) === \true && isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$stackPtr + 1]['code']]) === \true) {
                            continue;
                        }
                    }
                    $tokenContent = '';
                    if ($tokens[$stackPtr]['code'] === \T_WHITESPACE) {
                        if (isset($pattern[$i + 1]) === \false) {
                            ...
                            $next = $phpcsFile->findNext(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$emptyTokens, $stackPtr, null, \true);
                    ...
                    if ($next - $stackPtr > 0) {
                        ...
                        for ($j = $stackPtr; $j < $next; $j++) {
                            ...
                            if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$j]['code']]) === \true) {
                                $hasComment = \true;
                            }
                        }
                ...
                if ($pattern[$i]['type'] === 'skip') {
                    if ($pattern[$i]['to'] === 'unknown') {
                        ...
                        // Find the previous opener.
                        $next = $phpcsFile->findPrevious(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$blockOpeners, $stackPtr);
                        ...
                        if ($pattern[$i]['type'] === 'newline') {
                            ...
                            if ($newline === 0) {
                                ...
                                if ($this->ignoreComments === \false) {
                                    ...
                                    if (isset(\MolliePrefix\PHP_CodeSniffer\Util\Tokens::$commentTokens[$tokens[$newline]['code']]) === \true) {
                                        $hasError = \true;
                                    }
                                }
                                if ($newline === $stackPtr) {
     ...
     * @see    registerSupplementary()
     */
    protected function processSupplementary(\MolliePrefix\PHP_CodeSniffer\Files\File $phpcsFile, $stackPtr)
    {
    }
    ...
    private function createTokenPattern($str)
    {
        ...
        // whitespace token.
        $tokenizer = new \MolliePrefix\PHP_CodeSniffer\Tokenizers\PHP('<?php ' . $str . '?>', null);
