...
class Raven_Client
{
    ...
    public function __construct($options_or_dsn = null, $options = array())
    {
        ...
        $this->logger = \MolliePrefix\Raven_Util::get($options, 'logger', 'php');
        $this->server = \MolliePrefix\Raven_Util::get($options, 'server');
        $this->secret_key = \MolliePrefix\Raven_Util::get($options, 'secret_key');
        $this->public_key = \MolliePrefix\Raven_Util::get($options, 'public_key');
        $this->project = \MolliePrefix\Raven_Util::get($options, 'project', 1);
        $this->auto_log_stacks = (bool) \MolliePrefix\Raven_Util::get($options, 'auto_log_stacks', \false);
        $this->name = \MolliePrefix\Raven_Util::get($options, 'name', \MolliePrefix\Raven_Compat::gethostname());
        $this->site = \MolliePrefix\Raven_Util::get($options, 'site', self::_server_variable('SERVER_NAME'));
        $this->tags = \MolliePrefix\Raven_Util::get($options, 'tags', array());
        $this->release = \MolliePrefix\Raven_Util::get($options, 'release', null);
        $this->environment = \MolliePrefix\Raven_Util::get($options, 'environment', null);
        $this->sample_rate = \MolliePrefix\Raven_Util::get($options, 'sample_rate', 1);
        $this->trace = (bool) \MolliePrefix\Raven_Util::get($options, 'trace', \true);
        $this->timeout = \MolliePrefix\Raven_Util::get($options, 'timeout', 2);
        $this->message_limit = \MolliePrefix\Raven_Util::get($options, 'message_limit', self::MESSAGE_LIMIT);
        $this->exclude = \MolliePrefix\Raven_Util::get($options, 'exclude', array());
        $this->excluded_exceptions = \MolliePrefix\Raven_Util::get($options, 'excluded_exceptions', array());
        $this->severity_map = null;
        $this->http_proxy = \MolliePrefix\Raven_Util::get($options, 'http_proxy');
        $this->ignore_server_port = \MolliePrefix\Raven_Util::get($options, 'ignore_server_port', \false);
        $this->extra_data = \MolliePrefix\Raven_Util::get($options, 'extra', array());
        $this->send_callback = \MolliePrefix\Raven_Util::get($options, 'send_callback', null);
        $this->curl_method = \MolliePrefix\Raven_Util::get($options, 'curl_method', 'sync');
        $this->curl_path = \MolliePrefix\Raven_Util::get($options, 'curl_path', 'curl');
        $this->curl_ipv4 = \MolliePrefix\Raven_Util::get($options, 'curl_ipv4', \false);
        $this->ca_cert = \MolliePrefix\Raven_Util::get($options, 'ca_cert', static::get_default_ca_cert());
        $this->verify_ssl = \MolliePrefix\Raven_Util::get($options, 'verify_ssl', \true);
        $this->curl_ssl_version = \MolliePrefix\Raven_Util::get($options, 'curl_ssl_version');
        $this->trust_x_forwarded_proto = \MolliePrefix\Raven_Util::get($options, 'trust_x_forwarded_proto');
        $this->transport = \MolliePrefix\Raven_Util::get($options, 'transport', null);
        $this->mb_detect_order = \MolliePrefix\Raven_Util::get($options, 'mb_detect_order', null);
        $this->error_types = \MolliePrefix\Raven_Util::get($options, 'error_types', null);
        // app path is used to determine if code is part of your application
        $this->setAppPath(\MolliePrefix\Raven_Util::get($options, 'app_path', null));
        $this->setExcludedAppPaths(\MolliePrefix\Raven_Util::get($options, 'excluded_app_paths', null));
        // a list of prefixes used to coerce absolute paths into relative
        $this->setPrefixes(\MolliePrefix\Raven_Util::get($options, 'prefixes', static::getDefaultPrefixes()));
        ...
        $this->context = new \MolliePrefix\Raven_Context();
        $this->breadcrumbs = new \MolliePrefix\Raven_Breadcrumbs();
        $this->_shutdown_function_has_been_set = \false;
        $this->useCompression = \function_exists('gzcompress');
        $this->sdk = \MolliePrefix\Raven_Util::get($options, 'sdk', array('name' => 'sentry-php', 'version' => self::VERSION));
        $this->serializer = new \MolliePrefix\Raven_Serializer($this->mb_detect_order, $this->message_limit);
        $this->reprSerializer = new \MolliePrefix\Raven_ReprSerializer($this->mb_detect_order, $this->message_limit);
        if ($this->curl_method == 'async') {
            $this->_curl_handler = new \MolliePrefix\Raven_CurlHandler($this->get_curl_options());
        }
        $this->transaction = new \MolliePrefix\Raven_TransactionStack();
        if (static::is_http_request() && isset($_SERVER['PATH_INFO'])) {
            // @codeCoverageIgnoreStart
            $this->transaction->push($_SERVER['PATH_INFO']);
            // @codeCoverageIgnoreEnd
        }
        if (\MolliePrefix\Raven_Util::get($options, 'install_default_breadcrumb_handlers', \true)) {
            $this->registerDefaultBreadcrumbHandlers();
        }
        if (\MolliePrefix\Raven_Util::get($options, 'install_shutdown_handler', \true)) {
    ...
    public function install()
    {
        if ($this->error_handler) {
            throw new \MolliePrefix\Raven_Exception(\sprintf('%s->install() must only be called once', \get_class($this)));
        }
        $this->error_handler = new \MolliePrefix\Raven_ErrorHandler($this, \false, $this->error_types);
    ...
    {
        return array('MolliePrefix\\Raven_Processor_SanitizeDataProcessor');
    }
    /**
    ...
    {
        $processors = array();
        foreach (\MolliePrefix\Raven_Util::get($options, 'processors', static::getDefaultProcessors()) as $processor) {
            /**
    ...
    public static function parseDSN($dsn)
    {
        ...
        return array('server' => \sprintf('%s://%s%s/api/%s/store/', $scheme, $netloc, $path, $project), 'project' => $project, 'public_key' => $username, 'secret_key' => $password);
    }
    ...
    public function captureMessage($message, $params = array(), $data = array(), $stack = \false, $vars = null)
    {
        ...
        $data['sentry.interfaces.Message'] = array('message' => $message, 'params' => $params, 'formatted' => $formatted_message);
    ...
    public function captureException($exception, $data = null, $logger = null, $vars = null)
    {
        ...
        $exc = $exception;
        do {
            $exc_data = array('value' => $this->serializer->serialize($exc->getMessage()), 'type' => \get_class($exc));
            /**'exception'
             * Exception::getTrace doesn't store the point at where the exception
             * was thrown, so we have to stuff it in ourselves. Ugh.
             */
            $trace = $exc->getTrace();
            $frame_where_exception_thrown = array('file' => $exc->getFile(), 'line' => $exc->getLine());
            \array_unshift($trace, $frame_where_exception_thrown);
            $exc_data['stacktrace'] = array('frames' => \MolliePrefix\Raven_Stacktrace::get_stack_info($trace, $this->trace, $vars, $this->message_limit, $this->prefixes, $this->app_path, $this->excluded_app_paths, $this->serializer, $this->reprSerializer));
            $exceptions[] = $exc_data;
    ...
    {
        $data = array('message' => $query, 'level' => $level, 'sentry.interfaces.Query' => array('query' => $query));
        if ($engine !== '') {
    ...
    {
        $handler = new \MolliePrefix\Raven_Breadcrumbs_ErrorHandler($this);
        $handler->install();
    }
    protected function registerShutdownFunction()
    {
        if (!$this->_shutdown_function_has_been_set) {
            $this->_shutdown_function_has_been_set = \true;
    ...
    protected function get_http_data()
    {
        ...
        $result = array('method' => self::_server_variable('REQUEST_METHOD'), 'url' => $this->get_current_url(), 'query_string' => self::_server_variable('QUERY_STRING'));
        ...
        if (!empty($_POST)) {
            ...
            if ($raw_data !== \false) {
                $result['data'] = (array) \json_decode($raw_data, \true) ?: null;
            }
        }
        if (!empty($_COOKIE)) {
    ...
    {
        return array('server_name' => $this->name, 'project' => $this->project, 'site' => $this->site, 'logger' => $this->logger, 'tags' => $this->tags, 'platform' => 'php', 'sdk' => $this->sdk, 'transaction' => $this->transaction->peek());
    }
    public function capture($data, $stack = null, $vars = null)
    {
        ...
        if (isset($data['message'])) {
            $data['message'] = \MolliePrefix\Raven_Compat::substr($data['message'], 0, $this->message_limit);
        }
        ...
        if (!$stack && $this->auto_log_stacks || $stack === \true) {
            $stack = \debug_backtrace();
            // Drop last stack
            \array_shift($stack);
        }
        if (!empty($stack) && !isset($data['stacktrace']) && !isset($data['exception'])) {
            $data['stacktrace'] = array('frames' => \MolliePrefix\Raven_Stacktrace::get_stack_info($stack, $this->trace, $vars, $this->message_limit, $this->prefixes, $this->app_path, $this->excluded_app_paths, $this->serializer, $this->reprSerializer));
        }
        $this->sanitize($data);
        $this->process($data);
    ...
    {
        $message = \MolliePrefix\Raven_Compat::json_encode($data);
        if ($message === \false) {
    ...
    public function send(&$data)
    {
        ...
        $headers = array('User-Agent' => static::getUserAgent(), 'X-Sentry-Auth' => $this->getAuthHeader(), 'Content-Type' => 'application/octet-stream');
    ...
    {
        $options = array(\CURLOPT_VERBOSE => \false, \CURLOPT_SSL_VERIFYHOST => 2, \CURLOPT_SSL_VERIFYPEER => $this->verify_ssl, \CURLOPT_CAINFO => $this->ca_cert, \CURLOPT_USERAGENT => 'sentry-php/' . self::VERSION);
        if ($this->http_proxy) {
    ...
    protected function buildCurlCommand($url, $data, $headers)
    {
        ...
        $cmd .= '-m 5 ';
        // 5 second timeout for the whole process (connect + send)
        if (!$this->verify_ssl) {
            $cmd .= '-k ';
        }
        $cmd .= '> /dev/null 2>&1 &';
        // ensure exec returns immediately while curl runs in the background
        return $cmd;
    }
    /**
    ...
    protected function send_http_asynchronous_curl_exec($url, $data, $headers)
    {
        ...
        return \true;
        // The exec method is just fire and forget, so just assume it always works
    }
    /**
    ...
    protected function send_http_synchronous($url, $data, $headers)
    {
        ...
        \curl_setopt($this->_curl_instance, \CURLOPT_RETURNTRANSFER, \true);
    ...
    {
        $header = array(\sprintf('sentry_timestamp=%F', $timestamp), "sentry_client={$client}", \sprintf('sentry_version=%s', self::PROTOCOL));
        if ($api_key) {
    ...
    protected function get_current_url()
    {
        ...
        $host = !empty($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : (!empty($_SERVER['LOCAL_ADDR']) ? $_SERVER['LOCAL_ADDR'] : (!empty($_SERVER['SERVER_ADDR']) ? $_SERVER['SERVER_ADDR'] : ''));
    ...
    protected function isHttps()
    {
        ...
        if (!empty($this->trust_x_forwarded_proto) && !empty($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] === 'https') {
            return \true;
        }
        return \false;
    }
    ...
    public function translateSeverity($severity)
    {
        ...
        switch ($severity) {
            case \E_ERROR:
                return \MolliePrefix\Raven_Client::ERROR;
            case \E_WARNING:
                return \MolliePrefix\Raven_Client::WARN;
            case \E_PARSE:
                return \MolliePrefix\Raven_Client::ERROR;
            case \E_NOTICE:
                return \MolliePrefix\Raven_Client::INFO;
            case \E_CORE_ERROR:
                return \MolliePrefix\Raven_Client::ERROR;
            case \E_CORE_WARNING:
                return \MolliePrefix\Raven_Client::WARN;
    ...
    public function onShutdown()
    {
        if (!\defined('RAVEN_CLIENT_END_REACHED')) {
            \define('RAVEN_CLIENT_END_REACHED', \true);
        }
     ...
     */
    public function user_context($data, $merge = \true)
    {
     ...
     */
    public function setSerializer(\MolliePrefix\Raven_Serializer $serializer)
    {
     ...
     */
    public function setReprSerializer(\MolliePrefix\Raven_ReprSerializer $reprSerializer)
    {
    ...
    private function triggerAutoload()
    {
        ...
        if (!\class_exists('MolliePrefix\\Raven_Stacktrace')) {
 ...
 * @package raven
 */
\class_alias('MolliePrefix\\Raven_Client', 'Raven_Client', \false);
