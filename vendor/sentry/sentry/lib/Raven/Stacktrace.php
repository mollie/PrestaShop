...
{
    public static $statements = array('include', 'include_once', 'require', 'require_once');
    public static function get_stack_info($frames, $trace = \false, $errcontext = null, $frame_var_limit = \MolliePrefix\Raven_Client::MESSAGE_LIMIT, $strip_prefixes = null, $app_path = null, $excluded_app_paths = null, \MolliePrefix\Raven_Serializer $serializer = null, \MolliePrefix\Raven_ReprSerializer $reprSerializer = null)
    {
        $serializer = $serializer ?: new \MolliePrefix\Raven_Serializer();
        $reprSerializer = $reprSerializer ?: new \MolliePrefix\Raven_ReprSerializer();
        /**
        ...
        for ($i = 0; $i < \count($frames); $i++) {
            ...
            $data = array('filename' => $context['filename'], 'lineno' => (int) $context['lineno'], 'function' => isset($nextframe['function']) ? $nextframe['function'] : null, 'pre_context' => $serializer->serialize($context['prefix']), 'context_line' => $serializer->serialize($context['line']), 'post_context' => $serializer->serialize($context['suffix']));
            ...
            if (!empty($vars)) {
                ...
                foreach ($vars as $key => $value) {
                    ...
                    if (\is_string($value) || \is_numeric($value)) {
                        $cleanVars[(string) $key] = \MolliePrefix\Raven_Compat::substr($value, 0, $frame_var_limit);
                    } else {
    ...
    public static function get_default_context($frame, $frame_arg_limit = \MolliePrefix\Raven_Client::MESSAGE_LIMIT)
    {
        if (!isset($frame['args'])) {
            return array();
        }
    ...
    public static function get_frame_context($frame, $frame_arg_limit = \MolliePrefix\Raven_Client::MESSAGE_LIMIT)
    {
        if (!isset($frame['args'])) {
            return array();
        }
        ...
        if (\in_array($frame['function'], self::$statements)) {
            if (empty($frame['args'])) {
                ...
                return array('param1' => self::serialize_argument($frame['args'][0], $frame_arg_limit));
            }
        }
        try {
    ...
    private static function serialize_argument($arg, $frame_arg_limit)
    {
        if (\is_array($arg)) {
            ...
            foreach ($arg as $key => $value) {
                if (\is_string($value) || \is_numeric($value)) {
                    $_arg[$key] = \MolliePrefix\Raven_Compat::substr($value, 0, $frame_arg_limit);
                } else {
                    $_arg[$key] = $value;
                }
            }
            ...
            return \MolliePrefix\Raven_Compat::substr($arg, 0, $frame_arg_limit);
        } else {
            return $arg;
        }
    }
 ...
 * @package raven
 */
\class_alias('MolliePrefix\\Raven_Stacktrace', 'Raven_Stacktrace', \false);
