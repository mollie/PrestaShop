...
class Raven_Compat
{
    ...
    private static function _json_encode_lowlevel($value, $depth)
    {
        ...
        if (\is_scalar($value)) {
            ...
            if (\is_string($value)) {
                return \sprintf('"%s"', \str_replace($jsonReplaces[0], $jsonReplaces[1], $value));
            } else {
                return $value;
            }
        } elseif ($depth <= 1) {
        ...
        $isList = \true;
        for ($i = 0, \reset($value); $i < \count($value); $i++, \next($value)) {
            if (\key($value) !== $i) {
                $isList = \false;
                break;
            }
 ...
 * file that was distributed with this source code.
 */
\class_alias('MolliePrefix\\Raven_Compat', 'Raven_Compat', \false);
