...
class ConsumerFileTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    private $client;
    ...
    function setUp()
    {
        ...
        $this->client = new \MolliePrefix\Segment_Client("oq0vdlg7yi", array("consumer" => "file", "filename" => $this->filename));
    }
    function tearDown()
    {
        if (\file_exists($this->filename)) {
    ...
    function testTrack()
    {
        $this->assertTrue($this->client->track(array("userId" => "some-user", "event" => "File PHP Event - Microtime", "timestamp" => \microtime(\true))));
    ...
    function testScreen()
    {
        $this->assertTrue($this->client->screen(array("userId" => "userId", "name" => "grand theft auto", "category" => "analytics.log", "properties" => array())));
    }
    function testAlias()
    {
    ...
    function testProductionProblems()
    {
        ...
        $client = new \MolliePrefix\Segment_Client("oq0vdlg7yi", array("consumer" => "file", "filename" => "/dev/xxxxxxx"));
...
\class_alias('MolliePrefix\\ConsumerFileTest', 'ConsumerFileTest', \false);
