...
class ConsumerSocketTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    private $client;
    function setUp()
    {
        \date_default_timezone_set("UTC");
        $this->client = new \MolliePrefix\Segment_Client("oq0vdlg7yi", array("consumer" => "socket"));
    }
    function testTrack()
    {
    ...
    function testScreen()
    {
        $this->assertTrue($this->client->screen(array("anonymousId" => "anonymousId", "name" => "grand theft auto", "category" => "socket", "properties" => array())));
    }
    function testAlias()
    {
    ...
    {
        $client = new \MolliePrefix\Segment_Client("oq0vdlg7yi", array("timeout" => 0.01, "consumer" => "socket"));
    ...
    {
        $client = new \MolliePrefix\Segment_Client("x", array("consumer" => "socket", "error_handler" => function () {
    ...
    function testDebugProblems()
    {
        ...
        $client = new \MolliePrefix\Segment_Client("x", $options);
    ...
    function testLargeMessage()
    {
        ...
        $client = new \MolliePrefix\Segment_Client("testsecret", $options);
        $big_property = "";
        for ($i = 0; $i < 10000; $i++) {
            $big_property .= "a";
        }
    ...
    {
        $client = new \MolliePrefix\Segment_Client("x", array("consumer" => "socket", "host" => "api.segment.ioooooo", "error_handler" => function ($errno, $errmsg) {
...
\class_alias('MolliePrefix\\ConsumerSocketTest', 'ConsumerSocketTest', \false);
