 ...
 */
final class BracesFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface, \MolliePrefix\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    /**
     * @internal
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('The body of each structure MUST be enclosed by braces. Braces should be properly placed. Body of braces should be properly indented.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
$negative = function ($item) {
                return $item < 0; };
', ['allow_single_line_closure' => \true]), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
', ['position_after_functions_and_oop_constructs' => self::LINE_SAME])]);
        ...
        for ($index = $tokens->count() - 1; 0 <= $index; --$index) {
            ...
            for ($i = $braceIndex; $i > $newBraceIndex; --$i) {
                ...
                if ($tokens[$i]->isWhitespace() && $tokens[$i + 1]->isWhitespace()) {
                    $tokens[$i] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $tokens[$i]->getContent() . $tokens[$i + 1]->getContent()]);
                    $tokens->clearAt($i + 1);
                }
            }
            ...
            if (\substr_count($c, "\n") > 1) {
                ...
                $tokens[$braceIndex] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, \substr($c, \strrpos($c, "\n"))]);
            }
        }
    }
    private function fixControlContinuationBraces(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
    ...
    private function fixIndents(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $classyTokens = \MolliePrefix\PhpCsFixer\Tokenizer\Token::getClassyTokenKinds();
        ...
        for ($index = 0, $limit = \count($tokens); $index < $limit; ++$index) {
            ...
            for ($nestIndex = $lastCommaIndex; $nestIndex >= $startBraceIndex; --$nestIndex) {
                ...
                if ($nestToken->equalsAny([')', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_BRACE_CLASS_INSTANTIATION_CLOSE]])) {
                    $nestIndex = $tokens->findBlockStart($nestToken->equals(')') ? \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE : \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_BRACE_CLASS_INSTANTIATION, $nestIndex);
                    continue;
                }
                if (1 === $nestLevel) {
                    ...
                    if ($nestToken->equalsAny([';', '}'])) {
                        ...
                        if ($nextLineCanBeIndented || $i === $startBraceIndex) {
                            ...
                            $nextLineCanBeIndented = $nextToken->isWhitespace() && 1 === \MolliePrefix\PhpCsFixer\Preg::match('/\\R/', $nextToken->getContent());
                        }
                    }
                    ...
                    if (!($nextNonWhitespaceNestToken->isComment() && (!$tokens[$nextNonWhitespaceNestIndex - 1]->isWhitespace() || !\MolliePrefix\PhpCsFixer\Preg::match('/\\R/', $tokens[$nextNonWhitespaceNestIndex - 1]->getContent()))) && !($nestToken->equals('}') && $nextNonWhitespaceNestToken->equalsAny([';', ',', ']', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_CLOSE]])) && !($nestToken->equals('}') && $nextNonWhitespaceNestToken->equals('(')) && !($nestToken->equals('}') && $tokens[$nestIndex - 1]->equalsAny(['"', "'", [\T_CONSTANT_ENCAPSED_STRING], [\T_VARIABLE]])) && !($tokens[$nestIndex - 1]->isGivenKind(\T_END_HEREDOC) && $nextNonWhitespaceNestToken->isGivenKind(\T_CLOSE_TAG))) {
                        if (self::LINE_NEXT !== $this->configuration['position_after_control_structures'] && $nextNonWhitespaceNestToken->isGivenKind($this->getControlContinuationTokens()) && !$tokens[$tokens->getPrevNonWhitespace($nextNonWhitespaceNestIndex)]->isComment() || $nextNonWhitespaceNestToken->isGivenKind(\T_CLOSE_TAG) || self::LINE_NEXT !== $this->configuration['position_after_control_structures'] && $nextNonWhitespaceNestToken->isGivenKind(\T_WHILE) && $tokensAnalyzer->isWhilePartOfDoWhile($nextNonWhitespaceNestIndex)) {
                            ...
                            if ($nextToken->isWhitespace()) {
                                ...
                                if ('' !== $nextWhitespace) {
                                    $nextWhitespace = \MolliePrefix\PhpCsFixer\Preg::replace(\sprintf('/%s$/', $this->whitespacesConfig->getLineEnding()), '', $nextWhitespace, 1);
                                }
                            }
    ...
    private function fixMissingControlBraces(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $controlTokens = $this->getControlTokens();
        for ($index = $tokens->count() - 1; 0 <= $index; --$index) {
            $token = $tokens[$index];
            ...
            if ($tokenAfterParenthesis->isGivenKind([\T_FOR, \T_FOREACH, \T_SWITCH, \T_WHILE])) {
                $tokenAfterParenthesisBlockEnd = $tokens->findBlockEnd(
                    // go to ')'
                    \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE,
            ...
            $tokens->insertAt($statementEndIndex + 1, [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token('}')]);
            // insert missing `;` if needed
            if (!$tokens[$statementEndIndex]->equalsAny([';', '}'])) {
                $tokens->insertAt($statementEndIndex + 1, new \MolliePrefix\PhpCsFixer\Tokenizer\Token(';'));
            }
            // insert opening brace
            $tokens->insertAt($parenthesisEndIndex + 1, new \MolliePrefix\PhpCsFixer\Tokenizer\Token('{'));
            $tokens->ensureWhitespaceAtIndex($parenthesisEndIndex + 1, 0, ' ');
        }
    }
    private function fixSpaceAroundToken(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $controlTokens = $this->getControlTokens();
        for ($index = $tokens->count() - 1; 0 <= $index; --$index) {
            $token = $tokens[$index];
     ...
     * @return int
     */
    private function findStatementEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $parenthesisEndIndex)
    {
        ...
        if ($nextToken->equals('{')) {
            return $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $nextIndex);
        }
     ...
     * @param int $index
     */
    private function fixDeclareStatement(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     */
    private function fixSingleLineWhitespaceForDeclare(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startBraceIndex)
    {
     ...
     */
    private function ensureWhitespaceAtIndexAndIndentMultilineComment(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $whitespace)
    {
     ...
     * @return bool
     */
    private function isMultilined(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startParenthesisIndex, $endParenthesisIndex)
    {
     ...
     * @return bool
     */
    private function isCommentWithFixableIndentation(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     * @return null|int
     */
    private function getSiblingContinuousSingleLineComment(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $after)
    {
        $siblingIndex = $index;
        do {
        ...
        for ($i = \min($siblingIndex, $index) + 1, $max = \max($siblingIndex, $index); $i < $max; ++$i) {
            if ($tokens[$i]->isWhitespace() && \MolliePrefix\PhpCsFixer\Preg::match('/\\R/', $tokens[$i]->getContent())) {
                if (1 === $newLines || \MolliePrefix\PhpCsFixer\Preg::match('/\\R.*\\R/', $tokens[$i]->getContent())) {
                    return null;
                }
                ++$newLines;
            }
