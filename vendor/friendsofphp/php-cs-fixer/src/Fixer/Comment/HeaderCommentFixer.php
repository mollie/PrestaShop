 ...
 * @author SpacePossum
 */
final class HeaderCommentFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface, \MolliePrefix\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Add, replace or remove header comment.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
declare(strict_types=1);

namespace A\\B;

echo 1;
', ['header' => 'Made with love.']), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
declare(strict_types=1);

namespace A\\B;

echo 1;
', ['header' => 'Made with love.', 'comment_type' => 'PHPDoc', 'location' => 'after_open', 'separate' => 'bottom']), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
declare(strict_types=1);
...
', ['header' => 'Made with love.', 'comment_type' => 'comment', 'location' => 'after_declare_strict'])]);
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('header', 'Proper header content.'))->setAllowedTypes(['string'])->setNormalizer(static function (\MolliePrefix\Symfony\Component\OptionsResolver\Options $options, $value) use($fixerName) {
            if ('' === \trim($value)) {
                return '';
            }
            if (\false !== \strpos($value, '*/')) {
                throw new \MolliePrefix\PhpCsFixer\ConfigurationException\InvalidFixerConfigurationException($fixerName, 'Cannot use \'*/\' in header.');
            }
            return $value;
        })->getOption(), (new \MolliePrefix\PhpCsFixer\FixerConfiguration\AliasedFixerOptionBuilder(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('comment_type', 'Comment syntax type.'), 'commentType'))->setAllowedValues([self::HEADER_PHPDOC, self::HEADER_COMMENT])->setDefault(self::HEADER_COMMENT)->getOption(), (new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('location', 'The location of the inserted header.'))->setAllowedValues(['after_open', 'after_declare_strict'])->setDefault('after_declare_strict')->getOption(), (new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('separate', 'Whether the header should be separated from the file content with a new line.'))->setAllowedValues(['both', 'top', 'bottom', 'none'])->setDefault('both')->getOption()]);
    }
    /**
     ...
     * @return null|int
     */
    private function findHeaderCommentCurrentIndex(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $headerNewIndex)
    {
        ...
        if ($tokens[$next]->isWhitespace()) {
            if (!\MolliePrefix\PhpCsFixer\Preg::match('/^\\h*\\R\\h*$/D', $tokens[$next]->getContent())) {
                return $index;
            }
            ++$next;
        }
     ...
     * @return int
     */
    private function findHeaderCommentInsertionIndex(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $location)
    {
     ...
     */
    private function fixWhiteSpaceAroundHeader(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $headerIndex)
    {
        $lineEnding = $this->whitespacesConfig->getLineEnding();
        ...
        if ($headerIndex === \count($tokens) - 1) {
            $tokens->insertAt($headerIndex + 1, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, \str_repeat($lineEnding, $expectedLineCount)]));
        } else {
            ...
            if ($lineBreakCount < $expectedLineCount) {
                ...
                if ($tokens[$headerIndex + 1]->isWhitespace()) {
                    $tokens[$headerIndex + 1] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $missing . $tokens[$headerIndex + 1]->getContent()]);
                } else {
                    $tokens->insertAt($headerIndex + 1, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $missing]));
                }
                ...
                $tokens[$headerIndex + 1] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, \MolliePrefix\PhpCsFixer\Preg::replace("/^\\R{{$newLinesToRemove}}/", '', $tokens[$headerIndex + 1]->getContent())]);
            }
        }
        ...
        $regex = '/\\h$/';
        if ($tokens[$prev]->isGivenKind(\T_OPEN_TAG) && \MolliePrefix\PhpCsFixer\Preg::match($regex, $tokens[$prev]->getContent())) {
            $tokens[$prev] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_OPEN_TAG, \MolliePrefix\PhpCsFixer\Preg::replace($regex, $lineEnding, $tokens[$prev]->getContent())]);
        }
        $lineBreakCount = $this->getLineBreakCount($tokens, $headerIndex, -1);
        if ($lineBreakCount < $expectedLineCount) {
            // because of the way the insert index was determined for header comment there cannot be an empty token here
            $tokens->insertAt($headerIndex, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, \str_repeat($lineEnding, $expectedLineCount - $lineBreakCount)]));
        }
    }
    /**
     * @param int $index
     ...
     * @return int
     */
    private function getLineBreakCount(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $direction)
    {
        $whitespace = '';
    ...
    private function removeHeader(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $prevIndex = $index - 1;
     ...
     * @param int $index
     */
    private function insertHeader(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $tokens->insertAt($index, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([self::HEADER_COMMENT === $this->configuration['comment_type'] ? \T_COMMENT : \T_DOC_COMMENT, $this->getHeaderAsComment()]));
