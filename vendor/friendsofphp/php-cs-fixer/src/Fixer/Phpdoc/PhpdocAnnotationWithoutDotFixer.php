 ...
 */
final class PhpdocAnnotationWithoutDotFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('PHPDoc annotation descriptions should not be a sentence.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
/**
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        foreach ($tokens as $index => $token) {
            ...
            foreach ($annotations as $annotation) {
                ...
                if (1 !== \MolliePrefix\PhpCsFixer\Preg::match('/[.。]\\h*$/u', $content) || 0 !== \MolliePrefix\PhpCsFixer\Preg::match('/[.。](?!\\h*$)/u', $content, $matches)) {
                    continue;
                }
                $endLine = $doc->getLine($annotation->getEnd());
                $endLine->setContent(\MolliePrefix\PhpCsFixer\Preg::replace('/(?<![.。])[.。]\\h*(\\H+)$/u', '\\1', $endLine->getContent()));
