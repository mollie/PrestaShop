 ...
 */
final class StrictParamFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        static $map = null;
        if (null === $map) {
            $trueToken = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'true']);
            $map = ['array_keys' => [null, null, $trueToken], 'array_search' => [null, null, $trueToken], 'base64_decode' => [null, $trueToken], 'in_array' => [null, null, $trueToken], 'mb_detect_encoding' => [null, [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'mb_detect_order']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token('('), new \MolliePrefix\PhpCsFixer\Tokenizer\Token(')')], $trueToken]];
        }
    ...
    private function fixFunction(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $functionIndex, array $functionParams)
    {
        ...
        for ($i = $paramsQuantity; $i < $functionParamsQuantity; ++$i) {
            ...
            $tokensToInsert[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']);
