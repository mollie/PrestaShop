...
final class ArrayIndentationFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        foreach ($this->findArrays($tokens) as $array) {
            $indentLevel = 1;
            $scopes = [['opening_braces' => $array['start_braces']['opening'], 'unindented' => \false]];
            $currentScope = 0;
            ...
            foreach ($array['braces'] as $index => $braces) {
                ...
                if ($braces['starts_with_closing'] && !$scopes[$currentScope]['unindented'] && !$this->isClosingLineWithMeaningfulContent($tokens, $index)) {
                    --$currentIndentLevel;
                }
                $token = $tokens[$index];
                if ($this->newlineIsInArrayScope($tokens, $index, $array)) {
                    $content = \MolliePrefix\PhpCsFixer\Preg::replace('/(\\R+)\\h*$/', '$1' . $arrayIndent . \str_repeat($this->whitespacesConfig->getIndent(), $currentIndentLevel), $token->getContent());
                    $previousLineInitialIndent = $this->extractIndent($token->getContent());
                    $previousLineNewIndent = $this->extractIndent($content);
                } else {
                    $content = \MolliePrefix\PhpCsFixer\Preg::replace('/(\\R)' . \preg_quote($previousLineInitialIndent, '/') . '(\\h*)$/', '$1' . $previousLineNewIndent . '$2', $token->getContent());
                }
                $closingBraces = $braces['closing'];
                while ($closingBraces-- > 0) {
                    if (!$scopes[$currentScope]['unindented']) {
                        --$indentLevel;
                        $scopes[$currentScope]['unindented'] = \true;
                    }
                ...
                if ($braces['opening'] > 0) {
                    $scopes[] = ['opening_braces' => $braces['opening'], 'unindented' => \false];
                    ++$indentLevel;
                    ++$currentScope;
                }
    ...
    private function findArrays(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $arrays = [];
        foreach ($this->findArrayTokenRanges($tokens, 0, \count($tokens) - 1) as $arrayTokenRanges) {
            $array = ['start' => $arrayTokenRanges[0][0], 'end' => $arrayTokenRanges[\count($arrayTokenRanges) - 1][1], 'token_ranges' => $arrayTokenRanges];
    ...
    private function findArrayTokenRanges(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $from, $to)
    {
        $arrayTokenRanges = [];
        $currentArray = null;
        ...
        for ($index = $from; $index <= $to; ++$index) {
            ...
            if ($token->isGivenKind([\T_ARRAY, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_OPEN])) {
                ...
                $endIndex = $tokens->findBlockEnd($tokens[$index]->equals('(') ? \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE : \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_ARRAY_SQUARE_BRACE, $index);
                if (null === $currentArray) {
                    $currentArray = ['start' => $index, 'end' => $endIndex, 'ignored_tokens_ranges' => []];
                } else {
            ...
            if ($token->equals('(') && !$tokens[$tokens->getPrevMeaningfulToken($index)]->isGivenKind(\T_ARRAY) || $token->equals('{')) {
                $index = $tokens->findBlockEnd($token->equals('{') ? \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE : \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $index);
            }
            if ($token->equals(',')) {
                $currentArray['ignored_tokens_ranges'][] = [$valueSinceIndex, $tokens->getPrevMeaningfulToken($index)];
                $valueSinceIndex = null;
    ...
    private function computeArrayLineSignificantBraces(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, array $array)
    {
        $braces = [];
    ...
    private function getLineSignificantBraces(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, array $array)
    {
        $deltas = [];
        for (++$index; $index <= $array['end']; ++$index) {
            ...
            if ($token->equals(')')) {
                $openBraceIndex = $tokens->findBlockStart(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $index);
                if (!$tokens[$tokens->getPrevMeaningfulToken($openBraceIndex)]->isGivenKind(\T_ARRAY)) {
                    continue;
                }
            }
            if ($token->equalsAny(['(', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_OPEN]])) {
                $deltas[] = 1;
                continue;
            }
        ...
        $braces = ['opening' => 0, 'closing' => 0, 'starts_with_closing' => -1 === \reset($deltas)];
    ...
    private function isClosingLineWithMeaningfulContent(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $newLineIndex)
    {
        $nextMeaningfulIndex = $tokens->getNextMeaningfulToken($newLineIndex);
        return !$tokens[$nextMeaningfulIndex]->equalsAny([')', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_CLOSE]]);
    }
    ...
    private function newlineIsInArrayScope(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, array $array)
    {
