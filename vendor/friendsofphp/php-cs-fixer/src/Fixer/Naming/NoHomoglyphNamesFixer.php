 ...
 */
namespace MolliePrefix\PhpCsFixer\Fixer\Naming;
 ...
 */
final class NoHomoglyphNamesFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Replace accidental usage of homoglyphs (non ascii characters) in names.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php \$nаmе = 'wrong \"a\" character';\n")], null, 'Renames classes and cannot rename the files. You might have string references to renamed code (`$$name`).');
    }
    /**
     * {@inheritdoc}
     */
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        foreach ($tokens as $index => $token) {
            ...
            $replaced = \MolliePrefix\PhpCsFixer\Preg::replaceCallback('/[^[:ascii:]]/u', static function ($matches) {
                return isset(self::$replacements[$matches[0]]) ? self::$replacements[$matches[0]] : $matches[0];
            }, $token->getContent(), -1, $count);
            if ($count) {
                $tokens->offsetSet($index, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([$token->getId(), $replaced]));
            }
        }
    }
}
