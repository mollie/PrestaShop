...
final class ReturnAssignmentFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Local, dynamic and directly referenced variables should not be assigned and directly returned by a function or method.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php\nfunction a() {\n    \$a = 1;\n    return \$a;\n}\n")]);
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        for ($index = 1; $index < $tokenCount; ++$index) {
            ...
            $functionCloseIndex = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $functionOpenIndex);
            $totalTokensAdded = 0;
            do {
     ...
     */
    private function fixFunction(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $functionIndex, $functionOpenIndex, $functionCloseIndex)
    {
        static $riskyKinds = [
        ...
        for ($index = $functionOpenIndex + 1; $index < $functionCloseIndex; ++$index) {
            if ($tokens[$index]->isGivenKind(\T_FUNCTION)) {
                ...
                $nestedFunctionCloseIndex = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $nestedFunctionOpenIndex);
        ...
        for ($i = \count($candidates) - 1; $i >= 0; --$i) {
            ...
            do {
                ...
                $assignVarEndIndex = $tokens->findBlockStart(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $prevMeaningFul);
            } while (\true);
     ...
     * @return int >= 0 number of tokens inserted into the Tokens collection
     */
    private function simplifyReturnStatement(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $assignVarIndex, $assignVarOperatorIndex, $returnIndex, $returnVarEndIndex)
    {
        $inserted = 0;
        ...
        if ($tokens[$returnIndex - 1]->isWhitespace()) {
            ...
            $tokens[$returnIndex - 1] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $content]);
        }
        ...
        $tokens->insertAt($assignVarIndex, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_RETURN, 'return']));
        ++$inserted;
        ...
        if (null !== $originalIndent && $tokens[$assignVarIndex - 1]->isWhitespace() && $originalIndent !== $tokens[$assignVarIndex - 1]->getContent()) {
            $tokens[$assignVarIndex - 1] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $originalIndent]);
        }
    ...
    private function clearIfSave(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
