...
use MolliePrefix\PhpCsFixer\PregException;
 ...
 */
final class EregToPregFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Replace deprecated `ereg` regular expression functions with `preg`.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php \$x = ereg('[A-Z]');\n")], null, 'Risky if the `ereg` function is overridden.');
    }
    /**
     * {@inheritdoc}
     */
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        foreach (self::$functions as $map) {
            ...
            while (null !== $currIndex) {
                ...
                $tokens[$match[0]] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, $map[1]]);
                $tokens[$match[2]] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_CONSTANT_ENCAPSED_STRING, $quote . $preg . $quote]);
            }
        }
    }
    /**
    ...
    {
        try {
            \MolliePrefix\PhpCsFixer\Preg::match($pattern, '');
            return \true;
        } catch (\MolliePrefix\PhpCsFixer\PregException $e) {
            return \false;
        }
    }
    /**
    ...
    private function getBestDelimiter($pattern)
    {
        ...
        \uasort($delimiters, static function ($a, $b) {
            if ($a[0] === $b[0]) {
                return \MolliePrefix\PhpCsFixer\Utils::cmpInt($a, $b);
            }
            return $a[0] < $b[0] ? -1 : 1;
        });
