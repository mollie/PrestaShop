 ...
 * @author SpacePossum
 */
final class ArrayPushFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Converts simple usages of `array_push($x, $y);` to `$x[] = $y;`.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample("<?php\narray_push(\$x, \$y);\n", new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecification(70000))], null, 'Risky when the function `array_push` is overridden.');
    }
    /**
     * {@inheritdoc}
     */
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        for ($index = $tokens->count() - 7; $index > 0; --$index) {
            ...
            $blockType = \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::detectBlockType($tokens[$openBraceIndex]);
            if (null === $blockType || \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE !== $blockType['type']) {
                continue;
            }
            // figure out where the arguments list closes
            $closeBraceIndex = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $openBraceIndex);
            ...
            $tokens->insertAt($firstArgumentStop, [new \MolliePrefix\PhpCsFixer\Tokenizer\Token('['), new \MolliePrefix\PhpCsFixer\Tokenizer\Token(']'), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token('=')]);
     ...
     * @return int
     */
    private function getFirstArgumentEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $nextIndex = $tokens->getNextMeaningfulToken($index);
        $nextToken = $tokens[$nextIndex];
        while ($nextToken->equalsAny(['$', '[', '(', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_OPEN], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_PROP_BRACE_OPEN], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_VAR_BRACE_OPEN], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR], [\T_NS_SEPARATOR], [\T_STATIC], [\T_STRING], [\T_VARIABLE]])) {
     ...
     * @return null|int
     */
    private function getSecondArgumentEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $endIndex)
    {
