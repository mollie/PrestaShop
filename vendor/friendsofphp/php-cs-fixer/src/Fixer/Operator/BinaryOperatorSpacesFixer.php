 ...
 * @author SpacePossum
 */
final class BinaryOperatorSpacesFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * @internal
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Binary operators should be surrounded by space as configured.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php\n\$a= 1  + \$b^ \$d !==  \$e or   \$f;\n"), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
$aa=  1;
$b=2;

$c = $d    xor    $e;
$f    -=  1;
', ['operators' => ['=' => 'align', 'xor' => null]]), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
$a = $b +=$c;
$d = $ee+=$f;

$g = $b     +=$c;
$h = $ee+=$f;
', ['operators' => ['+=' => 'align_single_space']]), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
$a = $b===$c;
$d = $f   ===  $g;
$h = $i===  $j;
', ['operators' => ['===' => 'align_single_space_minimal']]), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
', ['operators' => ['|' => 'no_space']])]);
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([
            (new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('default', 'Default fix strategy.'))->setDefault(self::SINGLE_SPACE)->setAllowedValues(self::$allowedValues)->getOption(),
            (new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('operators', 'Dictionary of `binary operator` => `fix strategy` values that differ from the default strategy.'))->setAllowedTypes(['array'])->setAllowedValues([static function ($option) {
                foreach ($option as $operator => $value) {
                    if (!\in_array($operator, self::$supportedOperators, \true)) {
                        throw new \MolliePrefix\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Unexpected "operators" key, expected any of "%s", got "%s".', \implode('", "', self::$supportedOperators), \is_object($operator) ? \get_class($operator) : \gettype($operator) . '#' . $operator));
                    }
                    if (!\in_array($value, self::$allowedValues, \true)) {
                        throw new \MolliePrefix\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException(\sprintf('Unexpected value for operator "%s", expected any of "%s", got "%s".', $operator, \implode('", "', self::$allowedValues), \is_object($value) ? \get_class($value) : (null === $value ? 'null' : \gettype($value) . '#' . $value)));
                    }
                }
                return \true;
            }])->setDefault([])->getOption(),
            // add deprecated options as BC layer
            (new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('align_double_arrow', 'Whether to apply, remove or ignore double arrows alignment.'))->setDefault(\false)->setAllowedValues([\true, \false, null])->setDeprecationMessage('Use options `operators` and `default` instead.')->getOption(),
            (new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('align_equals', 'Whether to apply, remove or ignore equals alignment.'))->setDefault(\false)->setAllowedValues([\true, \false, null])->setDeprecationMessage('Use options `operators` and `default` instead.')->getOption(),
        ]);
    }
    /**
     * @param int $index
     */
    private function fixWhiteSpaceAroundOperator(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     * @param int $index
     */
    private function fixWhiteSpaceAroundOperatorToSingleSpace(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        // fix white space after operator
        if ($tokens[$index + 1]->isWhitespace()) {
     ...
     * @param int $index
     */
    private function fixWhiteSpaceAroundOperatorToNoSpace(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        // fix white space after operator
        if ($tokens[$index + 1]->isWhitespace()) {
            $content = $tokens[$index + 1]->getContent();
     ...
     */
    private function isEqualPartOfDeclareStatement(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        foreach ($configuration as $name => $setting) {
            if ('align_double_arrow' === $name) {
                if (\true === $configuration[$name]) {
                    ...
                    throw new \MolliePrefix\PhpCsFixer\ConfigurationException\InvalidFixerConfigurationException($this->getName(), \sprintf('Invalid configuration: The option "align_double_arrow" with value %s is invalid. Accepted values are: true, false, null.', $configuration[$name]));
                }
                ...
                if (\true === $configuration[$name]) {
                    ...
                    throw new \MolliePrefix\PhpCsFixer\ConfigurationException\InvalidFixerConfigurationException($this->getName(), \sprintf('Invalid configuration: The option "align_equals" with value %s is invalid. Accepted values are: true, false, null.', $configuration[$name]));
                }
            } else {
                throw new \MolliePrefix\PhpCsFixer\ConfigurationException\InvalidFixerConfigurationException($this->getName(), 'Mixing old configuration with new configuration is not allowed.');
            }
        }
        $message = \sprintf('Given configuration is deprecated and will be removed in 3.0. Use configuration %s as replacement for %s.', \MolliePrefix\PhpCsFixer\Console\Command\HelpCommand::toString($newConfig), \MolliePrefix\PhpCsFixer\Console\Command\HelpCommand::toString($configuration));
        if (\getenv('PHP_CS_FIXER_FUTURE_MODE')) {
            throw new \MolliePrefix\PhpCsFixer\ConfigurationException\InvalidFixerConfigurationException($this->getName(), "{$message} This check was performed as `PHP_CS_FIXER_FUTURE_MODE` env var is set.");
        }
     ...
     */
    private function fixAlignment(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, array $toAlign)
    {
        $this->deepestLevel = 0;
        $this->currentLevel = 0;
     ...
     */
    private function injectAlignmentPlaceholders(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startAt, $endAt, $tokenContent)
    {
        for ($index = $startAt; $index < $endAt; ++$index) {
            ...
            if (\strtolower($content) === $tokenContent && $this->tokensAnalyzer->isBinaryOperator($index) && ('=' !== $content || !$this->isEqualPartOfDeclareStatement($tokens, $index))) {
                $tokens[$index] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token(\sprintf(self::ALIGN_PLACEHOLDER, $this->deepestLevel) . $content);
                continue;
            }
     ...
     * @param int $endAt
     */
    private function injectAlignmentPlaceholdersForArrow(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startAt, $endAt)
    {
        for ($index = $startAt; $index < $endAt; ++$index) {
            $token = $tokens[$index];
     ...
     */
    private function fixWhiteSpaceBeforeOperator(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $alignStrategy)
    {
     ...
     * @return string
     */
    private function replacePlaceholders(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $alignStrategy)
    {
                ...
                if (self::ALIGN !== $alignStrategy) {
                    ...
                    foreach ($group as $index) {
                        ...
                        if (self::ALIGN_SINGLE_SPACE === $alignStrategy) {
                            ...
                            if (1 !== \MolliePrefix\PhpCsFixer\Preg::match('/^\\h+$/', $before)) {
