 ...
 * @author SpacePossum
 */
final class TernaryToElvisOperatorFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
     * @internal
     *
    ...
    const VALID_BEFORE_ENDTYPES = [
        ...
        '{',
        '}',
        [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_OPEN],
        [\T_AND_EQUAL],
        // &=
        [\T_CONCAT_EQUAL],
        // .=
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Use the Elvis operator `?:` where possible.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php\n\$foo = \$foo ? \$foo : 1;\n"), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php \$foo = \$bar[a()] ? \$bar[a()] : 1; # \"risky\" sample, \"a()\" only gets called once after fixing\n")], null, 'Risky when relying on functions called on both sides of the `?` operator.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    {
        $blockEdgeDefinitions = \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::getBlockEdgeDefinitions();
     ...
     */
    private function getBeforeOperator(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, array $blockEdgeDefinitions)
    {
     ...
     * @return array
     */
    private function getAfterOperator(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     * @return bool
     */
    private function rangeEqualsRange(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, array $range1, array $range2)
    {
        ...
        while ($leftStart <= $leftEnd && $rightStart <= $rightEnd) {
            if (!$tokens[$leftStart]->equals($tokens[$rightStart]) && !($tokens[$leftStart]->equalsAny(['[', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_OPEN]]) && $tokens[$rightStart]->equalsAny(['[', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_OPEN]])) && !($tokens[$leftStart]->equalsAny([']', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE]]) && $tokens[$rightStart]->equalsAny([']', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE]]))) {
                return \false;
            }
            $leftStart = $tokens->getNextMeaningfulToken($leftStart);
            $rightStart = $tokens->getNextMeaningfulToken($rightStart);
    ...
    private function clearMeaningfulFromRange(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, array $range)
    {
