 ...
 */
final class ModernizeTypesCastingFixer extends \MolliePrefix\PhpCsFixer\AbstractFunctionReferenceFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Replaces `intval`, `floatval`, `doubleval`, `strval` and `boolval` function calls with according type casting operator.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
    $a = intval($b);
    $a = floatval($b);
    $a = doubleval($b);
    $a = strval ($b);
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        foreach ($replacement as $functionIdentity => $newToken) {
            ...
            while (null !== $currIndex) {
                ...
                $replacementSequence = [new \MolliePrefix\PhpCsFixer\Tokenizer\Token($newToken), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' '])];
                if ($wrapInParenthesises) {
                    \array_unshift($replacementSequence, new \MolliePrefix\PhpCsFixer\Tokenizer\Token('('));
                }
                ...
                if ($wrapInParenthesises) {
                    $tokens->insertAt($closeParenthesis, new \MolliePrefix\PhpCsFixer\Tokenizer\Token(')'));
                }
