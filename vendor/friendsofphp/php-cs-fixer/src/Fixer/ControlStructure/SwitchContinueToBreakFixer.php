...
final class SwitchContinueToBreakFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Switch case must not be ended with `continue` but with `break`.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
switch ($foo) {
    case 1:
        continue;
}
...
')]);
     ...
     * @return int
     */
    private function doFix(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $depth, $isInSwitch)
    {
        $token = $tokens[$index];
     ...
     * @return int
     */
    private function fixInSwitch(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $switchIndex, $depth)
    {
     ...
     * @return int
     */
    private function fixInLoop(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $openIndex, $depth)
    {
        $openCount = 1;
        do {
            ++$openIndex;
     ...
     * @return int
     */
    private function fixContinueWhenActsAsBreak(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $continueIndex, $isInSwitch, $depth)
    {
        ...
        if (\strlen($jump) > 2 && 'x' === $jump[1]) {
            ...
            $jump = \octdec($jump);
            // octal 01
        } elseif (1 === \MolliePrefix\PhpCsFixer\Preg::match('#^\\d+$#', $jump)) {
            // positive int
            $jump = (float) $jump;
     ...
     * @param int $index
     */
    private function replaceContinueWithBreakToken(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $tokens[$index] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_BREAK, 'break']);
    }
}
