 ...
 * @author SpacePossum
 */
final class FunctionToConstantFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
    ...
    public function __construct()
    {
        if (null === self::$availableFunctions) {
            self::$availableFunctions = ['get_called_class' => [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STATIC, 'static']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_DOUBLE_COLON, '::']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_CLASS_CONSTANT, 'class'])], 'get_class' => [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_CLASS_C, '__CLASS__'])], 'get_class_this' => [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STATIC, 'static']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_DOUBLE_COLON, '::']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_CLASS_CONSTANT, 'class'])], 'php_sapi_name' => [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'PHP_SAPI'])], 'phpversion' => [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'PHP_VERSION'])], 'pi' => [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'M_PI'])]];
        }
        parent::__construct();
    }
    /**
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Replace core functions calls returning constants with the constants.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php\necho phpversion();\necho pi();\necho php_sapi_name();\nclass Foo\n{\n    public function Bar()\n    {\n        echo get_class();\n        echo get_called_class();\n    }\n}\n"), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php\necho phpversion();\necho pi();\nclass Foo\n{\n    public function Bar()\n    {\n        echo get_class();\n        get_class(\$this);\n        echo get_called_class();\n    }\n}\n", ['functions' => ['get_called_class', 'get_class_this', 'phpversion']])], null, 'Risky when any of the configured functions to replace are overridden.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('functions', 'List of function names to fix.'))->setAllowedTypes(['array'])->setAllowedValues([new \MolliePrefix\PhpCsFixer\FixerConfiguration\AllowedValueSubset($functionNames)])->setDefault(['get_class', 'php_sapi_name', 'phpversion', 'pi'])->getOption()]);
    }
    /**
     * @param int     $index
     ...
     */
    private function fixFunctionCallToConstant(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $braceOpenIndex, $braceCloseIndex, array $replacements)
    {
     ...
     * @return null|array
     */
    private function getReplaceCandidate(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\FunctionsAnalyzer $functionAnalyzer, $index)
    {
     ...
     * @return null|array
     */
    private function fixGetClassCall(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\FunctionsAnalyzer $functionAnalyzer, $index)
    {
