 ...
 */
final class IsNullFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Replaces `is_null($var)` expression with `null === $var`.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample("<?php\n\$a = is_null(\$b);\n")], null, 'Risky when the function `is_null` is overridden.');
    }
    /**
     * {@inheritdoc}
     *
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        while (null !== $currIndex) {
            ...
            $referenceEnd = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $matches[1]);
            ...
            $replacement = [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'null']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token($isInvertedNullCheck ? [\T_IS_NOT_IDENTICAL, '!=='] : [\T_IS_IDENTICAL, '===']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' '])];
            if (\true === $this->configuration['use_yoda_style']) {
                if ($wrapIntoParentheses) {
                    \array_unshift($replacement, new \MolliePrefix\PhpCsFixer\Tokenizer\Token('('));
                    $tokens->insertAt($referenceEnd + 1, new \MolliePrefix\PhpCsFixer\Tokenizer\Token(')'));
                }
                $tokens->overrideRange($isNullIndex, $isNullIndex, $replacement);
            } else {
                $replacement = \array_reverse($replacement);
                if ($wrapIntoParentheses) {
                    $replacement[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token(')');
                    $tokens[$isNullIndex] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token('(');
                } else {
    ...
    protected function createConfigurationDefinition()
    {
        ...
        return new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('use_yoda_style', 'Whether Yoda style conditions should be used.'))->setAllowedTypes(['bool'])->setDefault(\true)->setDeprecationMessage('Use `yoda_style` fixer instead.')->getOption()]);
    }
}
