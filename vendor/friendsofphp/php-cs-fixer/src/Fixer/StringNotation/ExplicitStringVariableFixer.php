 ...
 */
final class ExplicitStringVariableFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Converts implicit variables into explicit ones in double-quoted strings or heredoc syntax.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample(<<<'EOT'
<?php
...
EOT
        ...
        for ($index = \count($tokens) - 1; $index > 0; --$index) {
            ...
            while (!$this->isStringPartToken($tokens[$nextIndex])) {
                if ($tokens[$nextIndex]->isGivenKind(\T_CURLY_OPEN)) {
                    $nextIndex = $tokens->getNextTokenOfKind($nextIndex, [[\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_CURLY_CLOSE]]);
            ...
            foreach ($variableTokens as $distinctVariableSet) {
                if (1 === \count($distinctVariableSet['tokens'])) {
                    ...
                    $tokens->overrideRange($singleVariableIndex, $singleVariableIndex, [new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_DOLLAR_OPEN_CURLY_BRACES, '${']), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING_VARNAME, \substr($singleVariableToken->getContent(), 1)]), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DOLLAR_CLOSE_CURLY_BRACES, '}'])]);
                } else {
                    foreach ($distinctVariableSet['tokens'] as $variablePartIndex => $variablePartToken) {
                        if ($variablePartToken->isGivenKind(\T_NUM_STRING)) {
                            $tokens[$variablePartIndex] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_LNUMBER, $variablePartToken->getContent()]);
                            continue;
                        }
                        if ($variablePartToken->isGivenKind(\T_STRING) && $tokens[$variablePartIndex + 1]->equals(']')) {
                            $tokens[$variablePartIndex] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_CONSTANT_ENCAPSED_STRING, "'" . $variablePartToken->getContent() . "'"]);
                        }
                    }
                    $tokens->insertAt($distinctVariableSet['lastVariableTokenIndex'] + 1, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_CURLY_CLOSE, '}']));
                    $tokens->insertAt($distinctVariableSet['firstVariableTokenIndex'], new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_CURLY_OPEN, '{']));
                }
            }
        }
    }
     ...
     * @return bool
     */
    private function isStringPartToken(\MolliePrefix\PhpCsFixer\Tokenizer\Token $token)
    {
