...
final class PhpdocToReturnTypeFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('EXPERIMENTAL: Takes `@return` annotation of non-mixed types and adjusts accordingly the function signature. Requires PHP >= 7.0.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
...
function my_foo()
{}
', new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecification(70200)), new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
/** @return Foo */
function foo() {}
/** @return string */
function bar() {}
    ...
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        for ($index = $tokens->count() - 1; 0 < $index; --$index) {
            ...
            if (isset($this->scalarTypes[$returnType])) {
                ...
                if (1 !== \MolliePrefix\PhpCsFixer\Preg::match($this->classRegex, $returnType, $matches)) {
                    continue;
                }
