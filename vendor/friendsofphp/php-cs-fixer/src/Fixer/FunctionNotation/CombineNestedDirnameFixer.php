 ...
 */
final class CombineNestedDirnameFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Replace multiple nested calls of `dirname` by only one call with second `$level` parameter. Requires PHP >= 7.0.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample("<?php\ndirname(dirname(dirname(\$path)));\n", new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecification(70000))], null, 'Risky when the function `dirname` is overridden.');
    }
    /**
     * {@inheritdoc}
     */
     ...
     */
    private function getDirnameInfo(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $firstArgumentEndIndex = null)
    {
        if (!$tokens[$index]->equals([\T_STRING, 'dirname'], \false)) {
            return \false;
        }
        if (!(new \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\FunctionsAnalyzer())->isGlobalFunctionCall($tokens, $index)) {
            return \false;
        }
        $info['indexes'] = [];
        ...
        if (null !== $firstArgumentEndIndex) {
            ...
            while (!$tokens[$next]->equalsAny([',', ')'])) {
                $blockType = \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::detectBlockType($tokens[$next]);
                if ($blockType) {
    ...
    private function combineDirnames(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, array $dirnameInfoArray)
    {
        ...
        $levelsToken = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_LNUMBER, (string) $levels]);
        if (isset($outerDirnameInfo['secondArgument'])) {
            ...
            $items = [];
            if (!$tokens[$prev]->equals(',')) {
                $items = [new \MolliePrefix\PhpCsFixer\Tokenizer\Token(','), new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' '])];
            }
            $items[] = $levelsToken;
