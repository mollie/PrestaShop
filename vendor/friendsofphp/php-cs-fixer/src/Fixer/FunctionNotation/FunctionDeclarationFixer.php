 ...
 */
final class FunctionDeclarationFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface
{
    /**
     * @internal
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Spaces should be properly placed in a function declaration.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
$f = function () {};
', ['closure_function_spacing' => self::SPACING_NONE]), new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
$f = fn () => null;
', new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecification(70400), ['closure_function_spacing' => self::SPACING_NONE])]);
    }
    /**
     * {@inheritdoc}
     */
    protected function applyFix(\SplFileInfo $file, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        for ($index = $tokens->count() - 1; $index >= 0; --$index) {
            ...
            if ($afterParenthesisToken->isGivenKind(\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_USE_LAMBDA)) {
                ...
                $useEndParenthesisIndex = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $useStartParenthesisIndex);
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('closure_function_spacing', 'Spacing to use before open parenthesis for closures.'))->setDefault(self::SPACING_ONE)->setAllowedValues($this->supportedSpacings)->getOption()]);
    }
    private function fixParenthesisInnerEdge(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $start, $end)
    {
