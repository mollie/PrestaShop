...
final class PhpdocToParamTypeFixer extends \MolliePrefix\PhpCsFixer\AbstractPhpdocToTypeDeclarationFixer
{
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('EXPERIMENTAL: Takes `@param` annotations of non-mixed types and adjusts accordingly the function signature. Requires PHP >= 7.0.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecificCodeSample('<?php
...
function my_foo($bar)
{}
', new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecification(70100))], null, 'This rule is EXPERIMENTAL and [1] is not covered with backward compatibility promise. [2] `@param` annotation is mandatory for the fixer to make changes, signatures of methods without it (no docblock, inheritdocs) will not be fixed. [3] Manual actions are required if inherited signatures are not properly documented.');
    }
    /**
     * {@inheritdoc}
     */
        ...
        for ($index = $tokens->count() - 1; 0 < $index; --$index) {
            ...
            foreach ($paramTypeAnnotations as $paramTypeAnnotation) {
                ...
                foreach ($types as $key => $type) {
                    if (1 !== \MolliePrefix\PhpCsFixer\Preg::match(self::CLASS_REGEX, $type, $matches)) {
                        continue;
                    }
     ...
     * @return Annotation[]
     */
    private function findParamAnnotations(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        do {
     ...
     * @return null|int
     */
    private function findCorrectVariable(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $paramTypeAnnotation)
    {
        ...
        \MolliePrefix\PhpCsFixer\Preg::match('/@param\\s*[^\\s!<]+\\s*([^\\s]+)/', $paramTypeAnnotation->getContent(), $paramVariable);
     ...
     * @return bool
     */
    private function hasParamTypeHint(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        return $tokens[$index]->isGivenKind([\T_STRING, \T_NS_SEPARATOR, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_TYPEHINT, \T_CALLABLE, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NULLABLE_TYPE]);
    }
    /**
     ...
     */
    private function fixFunctionDefinition($paramType, \MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $hasNull, $hasArray, $hasIterable, $hasString, $hasInt, $hasFloat, $hasBool, $hasCallable, $hasObject)
    {
        $newTokens = [];
        if (\true === $hasIterable && \true === $hasArray) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_TYPEHINT, 'array']);
        } elseif (\true === $hasIterable) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'iterable']);
        } elseif (\true === $hasArray) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_TYPEHINT, 'array']);
        } elseif (\true === $hasString) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'string']);
        } elseif (\true === $hasInt) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'int']);
        } elseif (\true === $hasFloat) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'float']);
        } elseif (\true === $hasBool) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'bool']);
        } elseif (\true === $hasCallable) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_CALLABLE, 'callable']);
        } elseif (\true === $hasObject) {
            $newTokens[] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_STRING, 'object']);
        }
        ...
        if (\true === $hasNull) {
            \array_unshift($newTokens, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NULLABLE_TYPE, '?']));
        }
