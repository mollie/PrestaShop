 ...
 * @author SpacePossum
 */
final class ClassDefinitionFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer implements \MolliePrefix\PhpCsFixer\Fixer\ConfigurationDefinitionFixerInterface, \MolliePrefix\PhpCsFixer\Fixer\WhitespacesAwareFixerInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerDefinition\FixerDefinition('Whitespace around the keywords of a class, trait or interfaces definition should be one space.', [new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
', new \MolliePrefix\PhpCsFixer\FixerDefinition\VersionSpecification(70100)), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php

class Foo
extends Bar
implements Baz, BarBaz
{}
', ['single_line' => \true]), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php

class Foo
extends Bar
implements Baz
{}
', ['single_item_single_line' => \true]), new \MolliePrefix\PhpCsFixer\FixerDefinition\CodeSample('<?php
...
', ['multi_line_extends_each_single_line' => \true])]);
    ...
    {
        return new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerConfigurationResolver([(new \MolliePrefix\PhpCsFixer\FixerConfiguration\AliasedFixerOptionBuilder(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('multi_line_extends_each_single_line', 'Whether definitions should be multiline.'), 'multiLineExtendsEachSingleLine'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption(), (new \MolliePrefix\PhpCsFixer\FixerConfiguration\AliasedFixerOptionBuilder(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('single_item_single_line', 'Whether definitions should be single line when including a single item.'), 'singleItemSingleLine'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption(), (new \MolliePrefix\PhpCsFixer\FixerConfiguration\AliasedFixerOptionBuilder(new \MolliePrefix\PhpCsFixer\FixerConfiguration\FixerOptionBuilder('single_line', 'Whether definitions should be single line.'), 'singleLine'))->setAllowedTypes(['bool'])->setDefault(\false)->getOption()]);
    }
    /**
     * @param int $classyIndex Class definition token start index
     */
    private function fixClassyDefinition(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $classyIndex)
    {
     ...
     * @return array
     */
    private function fixClassyDefinitionExtends(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $classOpenIndex, array $classExtendsInfo)
    {
     ...
     * @return array
     */
    private function fixClassyDefinitionImplements(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $classOpenIndex, array $classImplementsInfo)
    {
        $endIndex = $tokens->getPrevNonWhitespace($classOpenIndex);
     ...
     * @return int
     */
    private function fixClassyDefinitionOpenSpacing(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, array $classDefInfo)
    {
        ...
        if ($tokens[$openIndex - 1]->isWhitespace()) {
            if (' ' !== $spacing || !$tokens[$tokens->getPrevNonWhitespace($openIndex - 1)]->isComment()) {
                $tokens[$openIndex - 1] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $spacing]);
            }
            return $openIndex;
        }
        $tokens->insertAt($openIndex, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $spacing]));
        return $openIndex + 1;
    }
    /**
     ...
     * @return array
     */
    private function getClassyDefinitionInfo(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $classyIndex)
    {
     ...
     * @return array
     */
    private function getClassyInheritanceInfo(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startIndex, $label)
    {
     ...
     * @param int $endIndex
     */
    private function makeClassyDefinitionSingleLine(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startIndex, $endIndex)
    {
        for ($i = $endIndex; $i >= $startIndex; --$i) {
            if ($tokens[$i]->isWhitespace()) {
                ...
                if ($tokens[$prevNonWhite]->isComment() || $tokens[$nextNonWhite]->isComment()) {
                    ...
                    if (!('#' === $content || '//' === \substr($content, 0, 2))) {
                        ...
                        if (!('#' === $content || '//' === \substr($content, 0, 2))) {
                            $tokens[$i] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']);
                        }
                    }
                    continue;
                }
            ...
            if (!$tokens[$i - 1]->isWhitespace() && !$tokens[$i - 1]->isComment()) {
                $tokens->insertAt($i, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, ' ']));
            }
        }
    }
    /**
     ...
     * @param int $endIndex
     */
    private function makeClassyInheritancePartMultiLine(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $startIndex, $endIndex)
    {
        for ($i = $endIndex; $i > $startIndex; --$i) {
            ...
            if (!$isOnOwnLine) {
                if ($tokens[$breakAtIndex - 1]->isWhitespace()) {
                    $tokens[$breakAtIndex - 1] = new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $this->whitespacesConfig->getLineEnding() . $this->whitespacesConfig->getIndent()]);
                } else {
                    $tokens->insertAt($breakAtIndex, new \MolliePrefix\PhpCsFixer\Tokenizer\Token([\T_WHITESPACE, $this->whitespacesConfig->getLineEnding() . $this->whitespacesConfig->getIndent()]));
                }
            }
