...
final class TokensAnalyzer
{
     ...
     */
    private $tokens;
    public function __construct(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        $this->tokens = $tokens;
    }
    /**
    ...
    {
        return $this->tokens[$index]->isGivenKind([\T_ARRAY, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_OPEN]);
    }
    /**
    ...
    public function isArrayMultiLine($index)
    {
        ...
        $endIndex = $tokens[$index]->equals('(') ? $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $index) : $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_ARRAY_SQUARE_BRACE, $index);
    ...
    public function isLambda($index)
    {
        ...
        if ($startParenthesisToken->isGivenKind(\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_RETURN_REF)) {
    ...
    public function isConstantInvocation($index)
    {
        ...
        if ($this->tokens[$nextIndex]->equalsAny(['(', '{']) || $this->tokens[$nextIndex]->isGivenKind([\T_AS, \T_DOUBLE_COLON, \T_ELLIPSIS, \T_NS_SEPARATOR, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_RETURN_REF, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_TYPE_ALTERNATION, \T_VARIABLE])) {
            return \false;
        }
        $prevIndex = $this->tokens->getPrevMeaningfulToken($index);
        if ($this->tokens[$prevIndex]->isGivenKind([\T_AS, \T_CLASS, \T_CONST, \T_DOUBLE_COLON, \T_FUNCTION, \T_GOTO, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_OPEN, \T_INTERFACE, \T_OBJECT_OPERATOR, \T_TRAIT, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_TYPE_COLON])) {
            return \false;
        }
        while ($this->tokens[$prevIndex]->isGivenKind([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR, \T_NS_SEPARATOR, \T_STRING])) {
            $prevIndex = $this->tokens->getPrevMeaningfulToken($prevIndex);
        }
        if ($this->tokens[$prevIndex]->isGivenKind([\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_CONST_IMPORT, \T_EXTENDS, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_FUNCTION_IMPORT, \T_IMPLEMENTS, \T_INSTANCEOF, \T_INSTEADOF, \T_NAMESPACE, \T_NEW, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NULLABLE_TYPE, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_TYPE_COLON, \T_USE, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_USE_TRAIT])) {
            return \false;
        }
        // `FOO & $bar` could be:
        ...
        if ($this->tokens[$prevIndex]->equals(',')) {
            ...
            while ($this->tokens[$checkIndex]->equalsAny([',', [\T_AS], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR], [\T_NS_SEPARATOR], [\T_STRING]])) {
                $checkIndex = $this->tokens->getPrevMeaningfulToken($checkIndex);
            }
            if ($this->tokens[$checkIndex]->isGivenKind([\T_EXTENDS, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_GROUP_IMPORT_BRACE_OPEN, \T_IMPLEMENTS, \T_USE, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_USE_TRAIT])) {
                return \false;
            }
        }
    ...
    {
        static $allowedPrevToken = [']', [\T_STRING], [\T_VARIABLE], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_PROP_BRACE_CLOSE], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_VAR_BRACE_CLOSE]];
        $tokens = $this->tokens;
        $token = $tokens[$index];
    ...
    public function isUnaryPredecessorOperator($index)
    {
        ...
        static $potentialBinaryOperator = ['+', '-', '&', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_RETURN_REF]];
        ...
        if (null === $disallowedPrevTokens) {
            $disallowedPrevTokens = [']', '}', ')', '"', '`', [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_SQUARE_BRACE_CLOSE], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_ARRAY_INDEX_CURLY_BRACE_CLOSE], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_PROP_BRACE_CLOSE], [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DYNAMIC_VAR_BRACE_CLOSE], [\T_CLASS_C], [\T_CONSTANT_ENCAPSED_STRING], [\T_DEC], [\T_DIR], [\T_DNUMBER], [\T_FILE], [\T_FUNC_C], [\T_INC], [\T_LINE], [\T_LNUMBER], [\T_METHOD_C], [\T_NS_C], [\T_STRING], [\T_TRAIT_C], [\T_VARIABLE]];
        }
        $tokens = $this->tokens;
        $token = $tokens[$index];
    ...
    public function isBinaryOperator($index)
    {
        ...
        if (null === $arrayOperators) {
            $arrayOperators = [
                ...
                \T_XOR_EQUAL => \true,
                // ^=
                \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_TYPE_ALTERNATION => \true,
            ];
