...
final class FunctionsAnalyzer
{
     ...
     * @return bool
     */
    public function isGlobalFunctionCall(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        if ($tokens[$prevIndex]->isGivenKind([\T_DOUBLE_COLON, \T_FUNCTION, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_NAMESPACE_OPERATOR, \T_NEW, \T_OBJECT_OPERATOR, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_RETURN_REF, \T_STRING])) {
            return \false;
        }
        ...
        $namespaceAnalyzer = new \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\NamespacesAnalyzer();
     ...
     */
    public function getFunctionArguments(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $methodIndex)
    {
        $argumentsStart = $tokens->getNextTokenOfKind($methodIndex, ['(']);
        $argumentsEnd = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $argumentsStart);
        $argumentAnalyzer = new \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\ArgumentsAnalyzer();
        $arguments = [];
     ...
     */
    public function getFunctionReturnType(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $methodIndex)
    {
        $argumentsStart = $tokens->getNextTokenOfKind($methodIndex, ['(']);
        $argumentsEnd = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $argumentsStart);
        ...
        return new \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\Analysis\TypeAnalysis($type, $typeStartIndex, $typeEndIndex);
    }
    /**
     * @param int $index
     *
     * @return bool
     */
    public function isTheSameClassCall(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
    ...
    private function buildFunctionsAnalysis(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens)
    {
        ...
        if ($tokens->isTokenKindFound(\T_FUNCTION)) {
            ...
            for ($i = 0; $i < $end; ++$i) {
                ...
                if ($tokens[$i]->isGivenKind(\MolliePrefix\PhpCsFixer\Tokenizer\Token::getClassyTokenKinds())) {
                    $i = $tokens->getNextTokenOfKind($i, ['(', '{']);
                    if ($tokens[$i]->equals('(')) {
                        // anonymous class
                        $i = $tokens->findBlockEnd(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $i);
                        $i = $tokens->getNextTokenOfKind($i, ['{']);
                    }
                ...
                if ($tokens[$i]->isGivenKind(\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_RETURN_REF)) {
                    $i = $tokens->getNextMeaningfulToken($i);
                }
        ...
        $namespaceUsesAnalyzer = new \MolliePrefix\PhpCsFixer\Tokenizer\Analyzer\NamespaceUsesAnalyzer();
