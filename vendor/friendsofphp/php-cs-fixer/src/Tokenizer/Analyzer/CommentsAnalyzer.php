...
final class CommentsAnalyzer
{
     ...
     * @return bool
     */
    public function isHeaderComment(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        ...
        if ($tokens[$prevIndex]->equals(';')) {
            ...
            $braceOpenIndex = $tokens->findBlockStart(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $braceCloseIndex);
     ...
     * @return bool
     */
    public function isBeforeStructuralElement(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $token = $tokens[$index];
     ...
     * @return null|array
     */
    public function getCommentBlockIndices(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     * @return bool
     */
    private function isStructuralElement(\MolliePrefix\PhpCsFixer\Tokenizer\Token $token)
    {
     ...
     * @return bool
     */
    private function isValidControl(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, \MolliePrefix\PhpCsFixer\Tokenizer\Token $docsToken, $controlIndex)
    {
     ...
     * @return bool
     */
    private function isValidLanguageConstruct(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, \MolliePrefix\PhpCsFixer\Tokenizer\Token $docsToken, $languageConstructIndex)
    {
        static $languageStructures = [\T_LIST, \T_PRINT, \T_ECHO, \MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DESTRUCTURING_SQUARE_BRACE_OPEN];
        if (!$tokens[$languageConstructIndex]->isGivenKind($languageStructures)) {
            return \false;
        }
        $endKind = $tokens[$languageConstructIndex]->isGivenKind(\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DESTRUCTURING_SQUARE_BRACE_OPEN) ? [\MolliePrefix\PhpCsFixer\Tokenizer\CT::T_DESTRUCTURING_SQUARE_BRACE_CLOSE] : ')';
     ...
     * @return bool
     */
    private function isValidVariable(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
    ...
    private function getLineBreakCount(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $whiteStart, $whiteEnd)
    {
        ...
        for ($i = $whiteStart; $i < $whiteEnd; ++$i) {
            $lineCount += \MolliePrefix\PhpCsFixer\Preg::matchAll('/\\R/u', $tokens[$i]->getContent());
        }
        return $lineCount;
    }
}
