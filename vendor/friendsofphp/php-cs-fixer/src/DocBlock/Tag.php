...
class Tag
{
     ...
     */
    public function __construct(\MolliePrefix\PhpCsFixer\DocBlock\Line $line)
    {
        $this->line = $line;
    }
    /**
    ...
    {
        if (null === $this->name) {
            \MolliePrefix\PhpCsFixer\Preg::matchAll('/@[a-zA-Z0-9_-]+(?=\\s|$)/', $this->line->getContent(), $matches);
    ...
    public function setName($name)
    {
        ...
        $this->line->setContent(\MolliePrefix\PhpCsFixer\Preg::replace("/@{$current}/", "@{$name}", $this->line->getContent(), 1));
        $this->name = $name;
    }
    /**
