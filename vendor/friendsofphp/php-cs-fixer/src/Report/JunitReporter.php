 ...
 * @internal
 */
final class JunitReporter implements \MolliePrefix\PhpCsFixer\Report\ReporterInterface
{
    /**
     * {@inheritdoc}
     */
    ...
    private function createFailedTestCases(\DOMDocument $dom, \DOMElement $testsuite, \MolliePrefix\PhpCsFixer\Report\ReportSummary $reportSummary)
    {
        $assertionsCount = 0;
    ...
    private function createFailedTestCase(\DOMDocument $dom, $file, array $fixResult, $shouldAddAppliedFixers)
    {
        ...
        $testName = \str_replace('.', '_DOT_', \MolliePrefix\PhpCsFixer\Preg::replace('@\\.' . \pathinfo($file, \PATHINFO_EXTENSION) . '$@', '', $file));
        $testcase = $dom->createElement('testcase');
