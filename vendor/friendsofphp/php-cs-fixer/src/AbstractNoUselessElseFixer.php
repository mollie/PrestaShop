 ...
 * @author SpacePossum
 */
abstract class AbstractNoUselessElseFixer extends \MolliePrefix\PhpCsFixer\AbstractFixer
{
    /**
     * {@inheritdoc}
     */
     ...
     * @return bool
     */
    protected function isSuperfluousElse(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
     ...
     * @return int[]
     */
    private function getPreviousBlock(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index)
    {
        $close = $previous = $tokens->getPrevMeaningfulToken($index);
        // short 'if' detection
        if ($tokens[$close]->equals('}')) {
            $previous = $tokens->findBlockStart(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_CURLY_BRACE, $close);
        }
     ...
     * @return bool
     */
    private function isInConditional(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $lowerLimitIndex)
    {
        ...
        $open = $tokens->findBlockStart(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens::BLOCK_TYPE_PARENTHESIS_BRACE, $candidateIndex);
     ...
     * @return bool
     */
    private function isInConditionWithoutBraces(\MolliePrefix\PhpCsFixer\Tokenizer\Tokens $tokens, $index, $lowerLimitIndex)
    {
        do {
            ...
            if ($token->equals('{')) {
                ...
                $type = \MolliePrefix\PhpCsFixer\Tokenizer\Tokens::detectBlockType($token);
