...
final class ErrorOutput
{
    ...
    public function listErrors($process, array $errors)
    {
        ...
        $showDetails = $this->output->getVerbosity() >= \MolliePrefix\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_VERY_VERBOSE;
        $showTrace = $this->output->getVerbosity() >= \MolliePrefix\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_DEBUG;
        foreach ($errors as $i => $error) {
            ...
            if ($showTrace && !$e instanceof \MolliePrefix\PhpCsFixer\Linter\LintingException) {
                // stack trace of lint exception is of no interest
                $this->output->writeln('');
                $stackTrace = $e->getTrace();
                foreach ($stackTrace as $trace) {
                    if (isset($trace['class'], $trace['function']) && \MolliePrefix\Symfony\Component\Console\Command\Command::class === $trace['class'] && 'run' === $trace['function']) {
                        $this->output->writeln('      [ ... ]');
                        break;
                    }
            ...
            if (\MolliePrefix\PhpCsFixer\Error\Error::TYPE_LINT === $error->getType() && 0 < \count($error->getAppliedFixers())) {
                $this->output->writeln('');
                $this->output->writeln(\sprintf('      Applied fixers: <comment>%s</comment>', \implode(', ', $error->getAppliedFixers())));
                $diff = $error->getDiff();
                if (!empty($diff)) {
                    $diffFormatter = new \MolliePrefix\PhpCsFixer\Differ\DiffConsoleFormatter($this->isDecorated, \sprintf('<comment>      ---------- begin diff ----------</comment>%s%%s%s<comment>      ----------- end diff -----------</comment>', \PHP_EOL, \PHP_EOL));
    ...
    {
        return $this->isDecorated ? \MolliePrefix\Symfony\Component\Console\Formatter\OutputFormatter::escape($string) : $string;
    }
}
