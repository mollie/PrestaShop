...
final class WarningsDetector
{
    ...
    public function detectOldVendor()
    {
        if ($this->toolInfo->isInstalledByComposer()) {
            ...
            if (\MolliePrefix\PhpCsFixer\ToolInfo::COMPOSER_LEGACY_PACKAGE_NAME === $details['name']) {
                $this->warnings[] = \sprintf('You are running PHP CS Fixer installed with old vendor `%s`. Please update to `%s`.', \MolliePrefix\PhpCsFixer\ToolInfo::COMPOSER_LEGACY_PACKAGE_NAME, \MolliePrefix\PhpCsFixer\ToolInfo::COMPOSER_PACKAGE_NAME);
            }
        }
    }
    /**
