 ...
 * @internal
 */
abstract class AbstractIntegrationCaseFactory implements \MolliePrefix\PhpCsFixer\Tests\Test\IntegrationCaseFactoryInterface
{
    /**
    ...
    public function create(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file)
    {
        try {
            ...
            return new \MolliePrefix\PhpCsFixer\Tests\Test\IntegrationCase($file->getRelativePathname(), $this->determineTitle($file, $match['title']), $this->determineSettings($file, $match['settings']), $this->determineRequirements($file, $match['requirements']), $this->determineConfig($file, $match['config']), $this->determineRuleset($file, $match['ruleset']), $this->determineExpectedCode($file, $match['expect']), $this->determineInputCode($file, $match['input']));
     ...
     * @return array
     */
    protected function determineConfig(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file, $config)
    {
     ...
     * @return array
     */
    protected function determineRequirements(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file, $config)
    {
     ...
     * @return string
     */
    protected function determineTitle(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file, $config)
    {
        return $config;
    }
    /**
     ...
     * @return string
     */
    protected function determineExpectedCode(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file, $code)
    {
     ...
     * @return null|string
     */
    protected function determineInputCode(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file, $code)
    {
     ...
     * @return null|string
     */
    private function determineCode(\MolliePrefix\Symfony\Component\Finder\SplFileInfo $file, $code, $suffix)
    {
        if (null !== $code) {
            return $code;
        }
        $candidateFile = new \MolliePrefix\Symfony\Component\Finder\SplFileInfo($file->getPathname() . $suffix, '', '');
