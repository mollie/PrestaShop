...
abstract class LongestCommonSubsequenceTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    /**
    ...
    {
        $from = array(\false, 0, 0.0, '', null, array(), \true, 1, 1.0, 'foo', array('foo', 'bar'), array('foo' => 'bar'));
        $to = $from;
        $common = $this->implementation->calculate($from, $to);
        $this->assertEquals($from, $common);
        $to = array(\false, \false, \false, \false, \false, \false, \true, \true, \true, \true, \true, \true);
        $expected = array(\false, \true);
        $common = $this->implementation->calculate($from, $to);
