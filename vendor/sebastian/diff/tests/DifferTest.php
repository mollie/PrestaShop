...
use MolliePrefix\SebastianBergmann\Diff\LCS\MemoryEfficientImplementation;
use MolliePrefix\SebastianBergmann\Diff\LCS\TimeEfficientImplementation;
 ...
 */
class DifferTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    const REMOVED = 2;
    const ADDED = 1;
    const OLD = 0;
    ...
    {
        $this->differ = new \MolliePrefix\SebastianBergmann\Diff\Differ();
    }
    /**
    ...
    {
        $this->assertEquals($expected, $this->differ->diffToArray($from, $to, new \MolliePrefix\SebastianBergmann\Diff\LCS\TimeEfficientImplementation()));
    }
    /**
    ...
    {
        $this->assertEquals($expected, $this->differ->diff($from, $to, new \MolliePrefix\SebastianBergmann\Diff\LCS\TimeEfficientImplementation()));
    }
    /**
     * @param array        $expected
     * @param string|array $from
    ...
    {
        $this->assertEquals($expected, $this->differ->diffToArray($from, $to, new \MolliePrefix\SebastianBergmann\Diff\LCS\MemoryEfficientImplementation()));
    }
    /**
     * @param string $expected
     * @param string $from
    ...
    {
        $this->assertEquals($expected, $this->differ->diff($from, $to, new \MolliePrefix\SebastianBergmann\Diff\LCS\MemoryEfficientImplementation()));
    }
    public function testCustomHeaderCanBeUsed()
    {
        $differ = new \MolliePrefix\SebastianBergmann\Diff\Differ('CUSTOM HEADER');
    ...
    {
        $parser = new \MolliePrefix\SebastianBergmann\Diff\Parser();
...
EOL;
    ...
    {
        $differ = new \MolliePrefix\SebastianBergmann\Diff\Differ('', \false);
        $this->assertSame($expected, $differ->diff($from, $to));
    }
    public function textForNoNonDiffLinesProvider()
    {
        return array(array('', 'a', 'a'), array("-A\n+C\n", "A\n\n\nB", "C\n\n\nB"));
    }
    /**
