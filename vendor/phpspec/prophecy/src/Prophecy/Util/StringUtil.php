 ...
 */
namespace MolliePrefix\Prophecy\Util;
...
class StringUtil
{
    ...
    public function stringify($value, $exportObject = \true)
    {
        ...
        if (\is_object($value)) {
            return $exportObject ? \MolliePrefix\Prophecy\Util\ExportUtil::export($value) : \sprintf('%s:%s', \get_class($value), \spl_object_hash($value));
        }
    ...
    {
        $self = $this;
        return \implode(\PHP_EOL, \array_map(function (\MolliePrefix\Prophecy\Call\Call $call) use($self) {
