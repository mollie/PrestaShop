...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\MolliePrefix\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \MolliePrefix\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\MolliePrefix\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    /**
     * Add expect header
     *
     * @return void
     */
    private function addExpectHeader(\MolliePrefix\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
