...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\_PhpScoper5eddef0da618a\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \_PhpScoper5eddef0da618a\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
        ...
        return $fn(\_PhpScoper5eddef0da618a\GuzzleHttp\Psr7\modify_request($request, $modify), $options);
    }
    /**
     * Add expect header
     *
     * @return void
     */
    private function addExpectHeader(\_PhpScoper5eddef0da618a\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
