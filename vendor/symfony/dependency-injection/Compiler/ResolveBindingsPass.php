 ...
 */
class ResolveBindingsPass extends \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process(\_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        try {
            ...
            foreach ($this->unusedBindings as list($key, $serviceId)) {
                ...
                throw new \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException($message);
            }
        } finally {
            $this->usedBindings = [];
            $this->unusedBindings = [];
    ...
    {
        if ($value instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\TypedReference && $value->getType() === $this->container->normalizeId($value)) {
            // Already checked
        ...
        if (!$value instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Definition || !($bindings = $value->getBindings())) {
            return parent::processValue($value, $isRoot);
        }
        foreach ($bindings as $key => $binding) {
            ...
            if (null !== $bindingValue && !$bindingValue instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Reference && !$bindingValue instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Definition) {
                throw new \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value for binding key "%s" for service "%s": expected null, an instance of "%s" or an instance of "%s", "%s" given.', $key, $this->currentId, \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Reference::class, \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Definition::class, \gettype($bindingValue)));
            }
        }
        if ($value->isAbstract()) {
            return parent::processValue($value, $isRoot);
        ...
        foreach ($calls as $i => $call) {
            ...
            foreach ($reflectionMethod->getParameters() as $key => $parameter) {
                ...
                $typeHint = \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \true);
    ...
    private function getBindingValue(\_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Argument\BoundArgument $binding)
    {
