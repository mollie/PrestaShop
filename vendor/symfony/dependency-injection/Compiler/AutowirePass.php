...
use _PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Config\AutowireServiceResource;
 ...
 */
class AutowirePass extends \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public static function createResourceForClass(\ReflectionClass $reflectionClass)
    {
        ...
        return new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Config\AutowireServiceResource($reflectionClass->name, $reflectionClass->getFileName(), $metadata);
    }
    /**
     * {@inheritdoc}
     */
    ...
    private function doProcessValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Definition || !$value->isAutowired() || $value->isAbstract() || !$value->getClass()) {
            return $value;
        }
        ...
        try {
            ...
            throw new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $e->getMessage(), 0, $e);
        }
        if ($constructor) {
    ...
    private function autowireCalls(\ReflectionClass $reflectionClass, array $methodCalls)
    {
        foreach ($methodCalls as $i => $call) {
            ...
            if ($method instanceof \ReflectionFunctionAbstract) {
                ...
                $definition = new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Definition($reflectionClass->name);
                try {
    ...
    private function autowireMethod(\ReflectionFunctionAbstract $reflectionMethod, array $arguments)
    {
        ...
        foreach ($parameters as $index => $parameter) {
            ...
            $type = \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \true);
            if (!$type) {
                ...
                if (!$parameter->isDefaultValueAvailable()) {
                    ...
                    $type = \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter, \false);
                    $type = $type ? \sprintf('is type-hinted "%s"', $type) : 'has no type-hint';
                    throw new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, \sprintf('Cannot autowire service "%s": argument "$%s" of method "%s()" %s, you should configure its value explicitly.', $this->currentId, $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method, $type));
                }
                // specifically pass the default value
                $arguments[$index] = $parameter->getDefaultValue();
                continue;
            }
            if (!($value = $this->getAutowiredReference($ref = new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference($type, $type, !$parameter->isOptional() ? $class : ''), 'for ' . \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class . '::' . $method)))) {
                $failureMessage = $this->createTypeNotFoundMessage($ref, \sprintf('argument "$%s" of method "%s()"', $parameter->name, $class !== $this->currentId ? $class . '::' . $method : $method));
                if ($parameter->isDefaultValueAvailable()) {
                    $value = $parameter->getDefaultValue();
                } elseif (!$parameter->allowsNull()) {
                    throw new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Exception\AutowiringFailedException($this->currentId, $failureMessage);
                }
     ...
     */
    private function getAutowiredReference(\_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference $reference, $deprecationMessage)
    {
        ...
        if (isset($this->definedTypes[$type])) {
            return new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference($this->types[$type], $type);
        }
        ...
        if (isset($this->autowired[$type])) {
            return $this->autowired[$type] ? new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference($this->autowired[$type], $type) : null;
        }
        if (!$this->strictMode) {
     ...
     * @param string $id
     */
    private function populateAvailableType($id, \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Definition $definition, $onlyAutowiringTypes)
    {
    ...
    private function createAutowiredDefinition($type)
    {
        ...
        $argumentDefinition = new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\Definition($type);
        ...
        return new \_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference($argumentId, $type);
    }
    private function createTypeNotFoundMessage(\_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference $reference, $label)
    {
        ...
        if (!$r) {
            ...
            try {
                $resource = new \_PhpScoper5ece82d7231e4\Symfony\Component\Config\Resource\ClassExistenceResource($type, \false);
    ...
    private function createTypeAlternatives(\_PhpScoper5ece82d7231e4\Symfony\Component\DependencyInjection\TypedReference $reference)
    {
