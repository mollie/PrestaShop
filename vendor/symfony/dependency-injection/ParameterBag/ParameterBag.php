 ...
 */
class ParameterBag implements \MolliePrefix\Symfony\Component\DependencyInjection\ParameterBag\ParameterBagInterface
{
    ...
    public function get($name)
    {
        ...
        if (!\array_key_exists($name, $this->parameters)) {
            if (!$name) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\ParameterNotFoundException($name);
            }
            $alternatives = [];
            ...
            throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\ParameterNotFoundException($name, null, null, null, $alternatives, $nonNestedAlternative);
        }
    ...
    public function resolveString($value, array $resolving = [])
    {
        ...
        if (\preg_match('/^%([^%\\s]+)%$/', $value, $match)) {
            ...
            if (isset($resolving[$lcKey])) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\ParameterCircularReferenceException(\array_keys($resolving));
            }
            $resolving[$lcKey] = \true;
        ...
        return \preg_replace_callback('/%%|%([^%\\s]+)%/', function ($match) use($resolving, $value) {
            ...
            if (!\is_string($resolved) && !\is_numeric($resolved)) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('A string value must be composed of strings and/or numbers, but found parameter "%s" of type "%s" inside string value "%s".', $key, \gettype($resolved), $value));
            }
