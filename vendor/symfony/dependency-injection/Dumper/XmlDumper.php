 ...
 */
class XmlDumper extends \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
     * @var \DOMDocument
     */
    ...
    private function addService($definition, $id, \DOMElement $parent)
    {
        ...
        if ($callable = $definition->getFactory()) {
            ...
            if (\is_array($callable) && $callable[0] instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Definition) {
                $this->addService($callable[0], null, $factory);
                $factory->setAttribute('method', $callable[1]);
            } elseif (\is_array($callable)) {
                if (null !== $callable[0]) {
                    $factory->setAttribute($callable[0] instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Reference ? 'service' : 'class', $callable[0]);
                }
                $factory->setAttribute('method', $callable[1]);
            } else {
        ...
        if ($callable = $definition->getConfigurator()) {
            ...
            if (\is_array($callable) && $callable[0] instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Definition) {
                ...
                $configurator->setAttribute($callable[0] instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Reference ? 'service' : 'class', $callable[0]);
                $configurator->setAttribute('method', $callable[1]);
            } else {
     ...
     * @param string $alias
     */
    private function addServiceAlias($alias, \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Alias $id, \DOMElement $parent)
    {
        $service = $this->document->createElement('service');
    ...
    private function convertParameters(array $parameters, $type, \DOMElement $parent, $keyAttribute = 'key')
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            if (\is_array($value)) {
            ...
            } elseif ($value instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument) {
                ...
                if (\_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE == $behavior) {
                    $element->setAttribute('on-invalid', 'null');
                } elseif (\_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_INVALID_REFERENCE == $behavior) {
                    $element->setAttribute('on-invalid', 'ignore');
                } elseif (\_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE == $behavior) {
    ...
    public static function phpToXml($value)
    {
        switch (\true) {
            ...
            case \false === $value:
                return 'false';
            case $value instanceof \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Parameter:
                return '%' . $value . '%';
