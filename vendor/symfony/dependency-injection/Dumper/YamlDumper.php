...
use MolliePrefix\Symfony\Component\Yaml\Dumper as YmlDumper;
 ...
 */
class YamlDumper extends \MolliePrefix\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    private $dumper;
    /**
    ...
    {
        if (!\class_exists('MolliePrefix\\Symfony\\Component\\Yaml\\Dumper')) {
            throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('Unable to dump the container as the Symfony Yaml Component is not installed.');
        }
        if (null === $this->dumper) {
     ...
     * @return string
     */
    private function addService($id, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition)
    {
        $code = "    {$id}:\n";
     ...
     * @return string
     */
    private function addServiceAlias($alias, \MolliePrefix\Symfony\Component\DependencyInjection\Alias $id)
    {
        if ($id->isPrivate()) {
            return \sprintf("    %s: '@%s'\n", $alias, $id);
        }
    ...
    private function dumpValue($value)
    {
        ...
        if ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Argument\ArgumentInterface) {
            if ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument) {
                return new \MolliePrefix\Symfony\Component\Yaml\Tag\TaggedValue('tagged', $value->getTag());
            }
            if ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $tag = 'iterator';
            } else {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Unspecified Yaml tag for type "%s".', \get_class($value)));
            }
            return new \MolliePrefix\Symfony\Component\Yaml\Tag\TaggedValue($tag, $this->dumpValue($value->getValues()));
        }
        if (\is_array($value)) {
            $code = [];
            ...
            return new \MolliePrefix\Symfony\Component\Yaml\Tag\TaggedValue('service', (new \MolliePrefix\Symfony\Component\Yaml\Parser())->parse("_:\n" . $this->addService('_', $value), \MolliePrefix\Symfony\Component\Yaml\Yaml::PARSE_CUSTOM_TAGS)['_']['_']);
    ...
    private function getServiceCall($id, \MolliePrefix\Symfony\Component\DependencyInjection\Reference $reference = null)
    {
        if (null !== $reference) {
            switch ($reference->getInvalidBehavior()) {
                case \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE:
                    break;
                case \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE:
                    return \sprintf('@!%s', $id);
                default:
                    return \sprintf('@?%s', $id);
            }
    ...
    private function prepareParameters(array $parameters, $escape = \true)
    {
        ...
        foreach ($parameters as $key => $value) {
            ...
            } elseif ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference || \is_string($value) && 0 === \strpos($value, '@')) {
                $value = '@' . $value;
            }
            $filtered[$key] = $value;
        }
