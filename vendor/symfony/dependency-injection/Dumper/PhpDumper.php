...
use MolliePrefix\Symfony\Component\DependencyInjection\Exception\EnvParameterException;
...
use MolliePrefix\Symfony\Component\HttpKernel\Kernel;
/**
 ...
 */
class PhpDumper extends \MolliePrefix\Symfony\Component\DependencyInjection\Dumper\Dumper
{
    /**
     ...
     */
    public function setProxyDumper(\MolliePrefix\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\DumperInterface $proxyDumper)
    {
    ...
    public function dump(array $options = [])
    {
        ...
        if (0 !== \strpos($baseClass = $options['base_class'], '\\') && 'Container' !== $baseClass) {
            ...
            $baseClassWithNamespace = \MolliePrefix\Symfony\Component\DependencyInjection\Container::class;
        } else {
            $baseClassWithNamespace = $baseClass;
        }
        $this->initializeMethodNamesMap('Container' === $baseClass ? \MolliePrefix\Symfony\Component\DependencyInjection\Container::class : $baseClass);
        if ($this->getProxyDumper() instanceof \MolliePrefix\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper) {
            (new \MolliePrefix\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\true, \false))->process($this->container);
            try {
                (new \MolliePrefix\Symfony\Component\DependencyInjection\Compiler\CheckCircularReferencesPass())->process($this->container);
                ...
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($e->getServiceId(), $path);
            }
        }
        (new \MolliePrefix\Symfony\Component\DependencyInjection\Compiler\AnalyzeServiceReferencesPass(\false, !$this->getProxyDumper() instanceof \MolliePrefix\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper))->process($this->container);
        $checkedNodes = [];
        $this->circularReferences = [];
        foreach ($this->container->getCompiler()->getServiceReferenceGraph()->getNodes() as $id => $node) {
            if (!$node->getValue() instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition) {
                continue;
            }
...
EOF;
            ...
            $hash = \ucfirst(\strtr(\MolliePrefix\Symfony\Component\DependencyInjection\ContainerBuilder::hash($files), '._', 'xx'));
            $code = [];
...
EOF;
        ...
        if ($unusedEnvs) {
            throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\EnvParameterException($unusedEnvs, null, 'Environment variables "%s" are never used. Please, check your container\'s configuration.');
        }
        return $code;
    }
    /**
    ...
    {
        if (!$this->proxyDumper) {
            $this->proxyDumper = new \MolliePrefix\Symfony\Component\DependencyInjection\LazyProxy\PhpDumper\NullDumper();
        }
    ...
    private function analyzeCircularReferences($sourceId, array $edges, &$checkedNodes, &$currentPath = [], $byConstructor = \true)
    {
        ...
        foreach ($edges as $edge) {
            ...
            $id = $node->getId();
            if (!$node->getValue() instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition || $sourceId === $id || $edge->isLazy() || $edge->isWeak()) {
                // no-op
    ...
    private function generateProxyClasses()
    {
        ...
        $strip = '' === $this->docStar && \method_exists('MolliePrefix\\Symfony\\Component\\HttpKernel\\Kernel', 'stripComments');
        ...
        foreach ($definitions as $definition) {
            ...
            if ($strip) {
                $proxyCode = "<?php\n" . $proxyCode;
                $proxyCode = \substr(\MolliePrefix\Symfony\Component\HttpKernel\Kernel::stripComments($proxyCode), 5);
            }
     ...
     * @return string
     */
    private function addServiceInclude($cId, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition)
    {
        $code = '';
     ...
     */
    private function addServiceInstance($id, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, $isSimpleInstance)
    {
        ...
        if (0 === \strpos($class, "'") && \false === \strpos($class, '$') && !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid class name for the "%s" service.', $class, $id));
        }
     ...
     * @return bool
     */
    private function isTrivialInstance(\MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition)
    {
        ...
        foreach ($definition->getArguments() as $arg) {
            if (!$arg || $arg instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Parameter) {
                continue;
            }
            if (\is_array($arg) && 3 >= \count($arg)) {
                foreach ($arg as $k => $v) {
                    ...
                    if (!$v || $v instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Parameter) {
                        continue;
                    }
                    if ($v instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $v) && $this->container->findDefinition($id)->isSynthetic()) {
                        continue;
                    }
            ...
            } elseif ($arg instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference && $this->container->has($id = (string) $arg) && $this->container->findDefinition($id)->isSynthetic()) {
                continue;
     ...
     * @return string
     */
    private function addServiceMethodCalls(\MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, $variableName = 'instance')
    {
        $calls = '';
    ...
    private function addServiceProperties(\MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, $variableName = 'instance')
    {
        $code = '';
     ...
     * @return string
     */
    private function addServiceConfigurator(\MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, $variableName = 'instance')
    {
        if (!($callable = $definition->getConfigurator())) {
            return '';
        }
        if (\is_array($callable)) {
            if ($callable[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
     ...
     * @return string
     */
    private function addService($id, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, &$file = null)
    {
        $this->definitionVariables = new \SplObjectStorage();
        $this->referenceVariables = [];
        $this->variableCount = 0;
        $this->referenceVariables[$id] = new \MolliePrefix\Symfony\Component\DependencyInjection\Variable('instance');
        $return = [];
        if ($class = $definition->getClass()) {
            $class = $class instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
            $return[] = \sprintf(0 === \strpos($class, '%') ? '@return object A %1$s instance' : '@return \\%s', \ltrim($class, '\\'));
        } elseif ($definition->getFactory()) {
            $factory = $definition->getFactory();
            if (\is_string($factory)) {
                $return[] = \sprintf('@return object An instance returned by %s()', $factory);
            } elseif (\is_array($factory) && (\is_string($factory[0]) || $factory[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition || $factory[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference)) {
                $class = $factory[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition ? $factory[0]->getClass() : (string) $factory[0];
                $class = $class instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Parameter ? '%' . $class . '%' : $this->container->resolveEnvPlaceholders($class);
...
EOF;
    ...
    private function addInlineVariables($id, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, array $arguments, $forConstructor)
    {
        $code = '';
        foreach ($arguments as $argument) {
            if (\is_array($argument)) {
                $code .= $this->addInlineVariables($id, $definition, $argument, $forConstructor);
            } elseif ($argument instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference) {
                $code .= $this->addInlineReference($id, $definition, $this->container->normalizeId($argument), $forConstructor);
            } elseif ($argument instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition) {
                $code .= $this->addInlineService($id, $definition, $argument, $forConstructor);
            }
        }
        return $code;
    }
    private function addInlineReference($id, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, $targetId, $forConstructor)
    {
        ...
        $this->referenceVariables[$targetId] = new \MolliePrefix\Symfony\Component\DependencyInjection\Variable($name);
        $reference = \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= $behavior ? new \MolliePrefix\Symfony\Component\DependencyInjection\Reference($targetId, $behavior) : null;
...
, 'services', $this->doExport($id));
    ...
    private function addInlineService($id, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, \MolliePrefix\Symfony\Component\DependencyInjection\Definition $inlineDef = null, $forConstructor = \true)
    {
        $code = '';
        ...
        if (isset($this->definitionVariables[$inlineDef])) {
            ...
            $this->definitionVariables[$inlineDef] = new \MolliePrefix\Symfony\Component\DependencyInjection\Variable($name);
            $code .= '' !== $code ? "\n" : '';
            if ('instance' === $name) {
    ...
    private function addNewInstance(\MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition, $return, $instantiation, $id)
    {
        $class = $this->dumpValue($definition->getClass());
        ...
        if (null !== $definition->getFactory()) {
            ...
            if (\is_array($callable)) {
                if (!\preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $callable[1])) {
                    throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid factory method (%s).', $callable[1] ?: 'n/a'));
                }
                if ($callable[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference || $callable[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition && $this->definitionVariables->contains($callable[0])) {
                ...
                if (0 === \strpos($class, "'") && \false === \strpos($class, '$')) {
                    if ("''" === $class) {
                        throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition: The "%s" service is defined to be created by a factory but is missing the service reference, did you forget to define the factory service id or class?', $id));
                    }
    ...
    {
        $bagClass = $this->container->isCompiled() ? 'use MolliePrefix\\Symfony\\Component\\DependencyInjection\\ParameterBag\\FrozenParameterBag;' : 'use MolliePrefix\\Symfony\\Component\\DependencyInjection\\ParameterBag\\ParameterBag;';
...
EOF;
        ...
        if ($this->container->isCompiled()) {
            if (\MolliePrefix\Symfony\Component\DependencyInjection\Container::class !== $baseClassWithNamespace) {
                $r = $this->container->getReflectionClass($baseClassWithNamespace, \false);
                if (null !== $r && null !== ($constructor = $r->getConstructor()) && 0 === $constructor->getNumberOfRequiredParameters() && \MolliePrefix\Symfony\Component\DependencyInjection\Container::class !== $constructor->getDeclaringClass()->name) {
    ...
    private function addDefaultParametersMethod()
    {
        ...
        foreach ($this->container->getParameterBag()->all() as $key => $value) {
            if ($key !== ($resolvedKey = $this->container->resolveEnvPlaceholders($key))) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Parameter name cannot use env parameters: "%s".', $resolvedKey));
            }
...
EOF;
            ...
            throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('You cannot dump a not-frozen container with dynamic parameters.');
        }
        $code .= <<<EOF
    ...
    private function exportParameters(array $parameters, $path = '', $indent = 12)
    {
        ...
        foreach ($parameters as $key => $value) {
            if (\is_array($value)) {
                ...
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain special arguments. "%s" found in "%s".', \get_class($value), $path . '/' . $key));
            } elseif ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Variable) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain variable references. Variable "%s" found in "%s".', $value, $path . '/' . $key));
            } elseif ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain service definitions. Definition for "%s" found in "%s".', $value->getClass(), $path . '/' . $key));
            } elseif ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Reference) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain references to other services (reference to service "%s" found in "%s").', $value, $path . '/' . $key));
            } elseif ($value instanceof \MolliePrefix\Symfony\Component\ExpressionLanguage\Expression) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('You cannot dump a container with parameters that contain expressions. Expression "%s" found in "%s".', $value, $path . '/' . $key));
            } else {
...
EOF;
    ...
    {
        $conditions = [];
        foreach (\MolliePrefix\Symfony\Component\DependencyInjection\ContainerBuilder::getInitializedConditionals($value) as $service) {
            if (!$this->container->hasDefinition($service)) {
                return 'false';
            }
            $conditions[] = \sprintf('isset($this->services[%s])', $this->doExport($service));
        }
        foreach (\MolliePrefix\Symfony\Component\DependencyInjection\ContainerBuilder::getServiceConditionals($value) as $service) {
    ...
    private function dumpValue($value, $interpolate = \true)
    {
        if (\is_array($value)) {
            ...
            try {
                if ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Argument\ServiceClosureArgument) {
                    ...
                    if ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\TypedReference) {
                        $code = \sprintf('$f = function (\\%s $v%s) { return $v; }; return $f(%s);', $value->getType(), \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE !== $value->getInvalidBehavior() ? ' = null' : '', $code);
                    } else {
                        $code = \sprintf('return %s;', $code);
                    }
                    return \sprintf("function () {\n            %s\n        }", $code);
                }
                if ($value instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                    $operands = [0];
                    $code = [];
            ...
            if ($value->getMethodCalls()) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have method calls.');
            }
            if ($value->getProperties()) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have properties.');
            }
            if (null !== $value->getConfigurator()) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have a configurator.');
            }
            $arguments = [];
            ...
            if (null !== $value->getFactory()) {
                ...
                if (\is_array($factory)) {
                    if (!\preg_match('/^[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*$/', $factory[1])) {
                        throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid factory method (%s).', $factory[1] ?: 'n/a'));
                    }
                    $class = $this->dumpValue($factory[0]);
                    if (\is_string($factory[0])) {
                        return \sprintf('%s::%s(%s)', $this->dumpLiteralClass($class), $factory[1], \implode(', ', $arguments));
                    }
                    if ($factory[0] instanceof \MolliePrefix\Symfony\Component\DependencyInjection\Definition) {
                        if (0 === \strpos($class, 'new ')) {
                ...
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definition because of invalid factory.');
            }
            $class = $value->getClass();
            if (null === $class) {
                throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException('Cannot dump definitions which have no class nor factory.');
            }
    ...
    private function dumpLiteralClass($class)
    {
        ...
        if (0 !== \strpos($class, "'") || !\preg_match('/^\'(?:\\\\{2})?[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*(?:\\\\{2}[a-zA-Z_\\x7f-\\xff][a-zA-Z0-9_\\x7f-\\xff]*)*\'$/', $class)) {
            throw new \MolliePrefix\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('Cannot dump definition because of invalid class name (%s).', $class ?: 'n/a'));
        }
     ...
     * @return string
     */
    private function getServiceCall($id, \MolliePrefix\Symfony\Component\DependencyInjection\Reference $reference = null)
    {
        while ($this->container->hasAlias($id)) {
            $id = (string) $this->container->getAlias($id);
        }
        ...
        if ($this->container->hasDefinition($id) && ($definition = $this->container->getDefinition($id))) {
            ...
            } elseif (null !== $reference && \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::IGNORE_ON_UNINITIALIZED_REFERENCE === $reference->getInvalidBehavior()) {
                $code = 'null';
            ...
            return 'null';
        } elseif (null !== $reference && \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE !== $reference->getInvalidBehavior()) {
            $code = \sprintf('$this->get(%s, /* ContainerInterface::NULL_ON_INVALID_REFERENCE */ %d)', $this->doExport($id), \MolliePrefix\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE);
        } else {
    ...
    private function generateMethodName($id)
    {
        ...
        $i = \strrpos($id, '\\');
        $name = \MolliePrefix\Symfony\Component\DependencyInjection\Container::camelize(\false !== $i && isset($id[1 + $i]) ? \substr($id, 1 + $i) : $id);
    ...
    private function isHotPath(\MolliePrefix\Symfony\Component\DependencyInjection\Definition $definition)
    {
