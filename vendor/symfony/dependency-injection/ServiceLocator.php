 ...
 */
class ServiceLocator implements \_PhpScoper5eddef0da618a\Psr\Container\ContainerInterface
{
    private $factories;
    private $loading = [];
    private $externalId;
    ...
    public function get($id)
    {
        if (!isset($this->factories[$id])) {
            throw new \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException($id, \end($this->loading) ?: null, null, [], $this->createServiceNotFoundMessage($id));
        }
        if (isset($this->loading[$id])) {
            $ids = \array_values($this->loading);
            $ids = \array_slice($this->loading, \array_search($id, $ids));
            $ids[] = $id;
            throw new \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Exception\ServiceCircularReferenceException($id, $ids);
        }
        $this->loading[$id] = $id;
        try {
     ...
     * @internal
     */
    public function withContext($externalId, \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\Container $container)
    {
        $locator = clone $this;
    ...
    private function createServiceNotFoundMessage($id)
    {
        ...
        if (!$class) {
            // no-op
        } elseif (\is_subclass_of($class, \_PhpScoper5eddef0da618a\Symfony\Component\DependencyInjection\ServiceSubscriberInterface::class)) {
