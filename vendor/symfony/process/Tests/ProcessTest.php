...
use MolliePrefix\Symfony\Component\Process\InputStream;
 ...
 */
class ProcessTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    ...
    {
        $phpBin = new \MolliePrefix\Symfony\Component\Process\PhpExecutableFinder();
    ...
    public function testInvalidCwd()
    {
        ...
        $cmd = new \MolliePrefix\Symfony\Component\Process\Process('echo test', __DIR__);
        $cmd->run();
        $cmd = new \MolliePrefix\Symfony\Component\Process\Process('echo test', __DIR__ . '/notfound/');
        $cmd->run();
    }
    ...
    public function testAllOutputIsActuallyReadOnTermination()
    {
        ...
        $expectedOutputSize = \MolliePrefix\Symfony\Component\Process\Pipes\PipesInterface::CHUNK_SIZE * 2 + 2;
    ...
    public function testSetInputWhileRunningThrowsAnException()
    {
        ...
        } catch (\MolliePrefix\Symfony\Component\Process\Exception\LogicException $e) {
        }
        $process->stop();
        throw $e;
    }
    ...
    {
        return [[[]], [new \MolliePrefix\Symfony\Component\Process\Tests\NonStringifiable()]];
    }
    /**
    ...
    {
        return [['getOutput', 'getIncrementalOutput', 'php://stdout'], ['getErrorOutput', 'getIncrementalErrorOutput', 'php://stderr']];
    }
    ...
    public function testTTYCommand()
    {
        ...
        $process->wait();
        $this->assertSame(\MolliePrefix\Symfony\Component\Process\Process::STATUS_TERMINATED, $process->getStatus());
    }
    ...
    {
        if (!\MolliePrefix\Symfony\Component\Process\Process::isPtySupported()) {
    ...
    {
        $this->expectException('MolliePrefix\\Symfony\\Component\\Process\\Exception\\ProcessFailedException');
        $this->skipIfNotEnhancedSigchild();
    ...
    public function testStatus()
    {
        ...
        $this->assertSame(\MolliePrefix\Symfony\Component\Process\Process::STATUS_READY, $process->getStatus());
        $process->start();
        $this->assertTrue($process->isRunning());
        $this->assertTrue($process->isStarted());
        $this->assertFalse($process->isTerminated());
        $this->assertSame(\MolliePrefix\Symfony\Component\Process\Process::STATUS_STARTED, $process->getStatus());
        $process->wait();
        $this->assertFalse($process->isRunning());
        $this->assertTrue($process->isStarted());
    ...
    {
        $this->expectException('MolliePrefix\\Symfony\\Component\\Process\\Exception\\ProcessTimedOutException');
        ...
        } catch (\MolliePrefix\Symfony\Component\Process\Exception\RuntimeException $e) {
        }
        $this->assertLessThan(15, \microtime(\true) - $start);
        throw $e;
    }
    ...
    public function testCheckTimeoutOnStartedProcess()
    {
        ...
        } catch (\MolliePrefix\Symfony\Component\Process\Exception\ProcessTimedOutException $e) {
        }
        $this->assertLessThan(15, \microtime(\true) - $start);
        throw $e;
    }
    ...
    {
        return [['getOutput'], ['getIncrementalOutput'], ['getErrorOutput'], ['getIncrementalErrorOutput'], ['wait']];
    }
    /**
    ...
    {
        return [['hasBeenSignaled'], ['getTermSignal'], ['hasBeenStopped'], ['getStopSignal']];
    }
    /**
    ...
    public function testWrongSignal($signal)
    {
        ...
        if (\PHP_VERSION_ID < 80000 || \is_int($signal)) {
            $this->expectException(\MolliePrefix\Symfony\Component\Process\Exception\RuntimeException::class);
        } else {
    ...
    {
        return [['getOutput'], ['getIncrementalOutput'], ['getErrorOutput'], ['getIncrementalErrorOutput']];
    }
    ...
    {
        $variations = ['fwrite(STDOUT, $in = file_get_contents(\'php://stdin\')); fwrite(STDERR, $in);', 'include \'' . __DIR__ . '/PipeStdinInStdoutStdErrStreamSelect.php\';'];
        if ('\\' === \DIRECTORY_SEPARATOR) {
    ...
    {
        return [['php://stdout', 'getIncrementalOutput'], ['php://stderr', 'getIncrementalErrorOutput']];
    }
    ...
    {
        $input = new \MolliePrefix\Symfony\Component\Process\InputStream();
    ...
    public function testIteratorOutput()
    {
        ...
        $this->assertFalse($process->isRunning());
        $expectedOutput = [[$process::OUT, '123'], [$process::ERR, '234'], [$process::OUT, '345'], [$process::ERR, '456']];
        $this->assertSame($expectedOutput, $output);
    }
    ...
    {
        $p = new \MolliePrefix\Symfony\Component\Process\Process(['/usr/bin/php']);
    ...
    {
        $p = new \MolliePrefix\Symfony\Component\Process\Process([self::$phpBin, '-r', 'echo $argv[1];', $arg]);
        $p->run();
    ...
    {
        $p = new \MolliePrefix\Symfony\Component\Process\Process([self::$phpBin, '-r', 'echo $argv[1];', $arg], null, ['BAR' => 'BAZ']);
    ...
    {
        $p = new \MolliePrefix\Symfony\Component\Process\Process(\sprintf('"%s" -r %s "a" "" "b"', self::$phpBin, \escapeshellarg('print_r($argv);')));
        $p->run();
        $expected = <<<EOTXT
Array
(
...
EOTXT;
    ...
    public function testEnvArgument()
    {
        ...
        $p = new \MolliePrefix\Symfony\Component\Process\Process($cmd, null, $env);
    ...
    {
        $process = new \MolliePrefix\Symfony\Component\Process\Process($commandline, $cwd, $env, $input, $timeout);
        $process->inheritEnvironmentVariables();
