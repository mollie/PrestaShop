...
final class ProgressBar
{
     ...
     */
    public function __construct(\MolliePrefix\Symfony\Component\Console\Output\OutputInterface $output, $max = 0)
    {
    ...
    private function setMaxSteps($max)
    {
        ...
        $this->stepWidth = $this->max ? \MolliePrefix\Symfony\Component\Console\Helper\Helper::strlen($this->max) : 4;
    }
    /**
    ...
    {
        return ['bar' => function (self $bar, \MolliePrefix\Symfony\Component\Console\Output\OutputInterface $output) {
            $completeBars = \floor($bar->getMaxSteps() > 0 ? $bar->getProgressPercent() * $bar->getBarWidth() : $bar->getProgress() % $bar->getBarWidth());
            $display = \str_repeat($bar->getBarCharacter(), $completeBars);
            if ($completeBars < $bar->getBarWidth()) {
                $emptyBars = $bar->getBarWidth() - $completeBars - \MolliePrefix\Symfony\Component\Console\Helper\Helper::strlenWithoutDecoration($output->getFormatter(), $bar->getProgressCharacter());
                $display .= $bar->getProgressCharacter() . \str_repeat($bar->getEmptyBarCharacter(), $emptyBars);
            }
            return $display;
        }, 'elapsed' => function (self $bar) {
            return \MolliePrefix\Symfony\Component\Console\Helper\Helper::formatTime(\time() - $bar->getStartTime());
        }, 'remaining' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new \MolliePrefix\Symfony\Component\Console\Exception\LogicException('Unable to display the remaining time if the maximum number of steps is not set.');
            }
            if (!$bar->getProgress()) {
                $remaining = 0;
            } else {
            ...
            return \MolliePrefix\Symfony\Component\Console\Helper\Helper::formatTime($remaining);
        }, 'estimated' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new \MolliePrefix\Symfony\Component\Console\Exception\LogicException('Unable to display the estimated time if the maximum number of steps is not set.');
            }
            if (!$bar->getProgress()) {
                $estimated = 0;
            } else {
            ...
            return \MolliePrefix\Symfony\Component\Console\Helper\Helper::formatTime($estimated);
    ...
    private function buildLine()
    {
        ...
        $linesLength = \array_map(function ($subLine) {
            return \MolliePrefix\Symfony\Component\Console\Helper\Helper::strlenWithoutDecoration($this->output->getFormatter(), \rtrim($subLine, "\r"));
        }, \explode("\n", $line));
