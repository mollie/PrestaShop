 ...
 * @group tty
 */
class SymfonyQuestionHelperTest extends \MolliePrefix\Symfony\Component\Console\Tests\Helper\AbstractQuestionHelperTest
{
    public function testAskChoice()
    {
        $questionHelper = new \MolliePrefix\Symfony\Component\Console\Helper\SymfonyQuestionHelper();
    ...
    public function testAskChoiceWithChoiceValueAsDefault()
    {
        ...
        $helperSet = new \MolliePrefix\Symfony\Component\Console\Helper\HelperSet([new \MolliePrefix\Symfony\Component\Console\Helper\FormatterHelper()]);
        $questionHelper->setHelperSet($helperSet);
        $question = new \MolliePrefix\Symfony\Component\Console\Question\ChoiceQuestion('What is your favorite superhero?', ['Superman', 'Batman', 'Spiderman'], 'Batman');
        $question->setMaxAttempts(1);
    ...
    {
        $questionHelper = new \MolliePrefix\Symfony\Component\Console\Helper\SymfonyQuestionHelper();
        $question = new \MolliePrefix\Symfony\Component\Console\Question\Question('What is your favorite superhero?');
    ...
    {
        $helper = new \MolliePrefix\Symfony\Component\Console\Helper\SymfonyQuestionHelper();
        $input = $this->createStreamableInputInterfaceMock($this->getInputStream('\\'));
        $helper->ask($input, $output = $this->createOutputInterface(), new \MolliePrefix\Symfony\Component\Console\Question\Question('Can I have a backslash?', '\\'));
        $this->assertOutputContains('Can I have a backslash? [\\]', $output);
    }
    public function testAskEscapeAndFormatLabel()
    {
        $helper = new \MolliePrefix\Symfony\Component\Console\Helper\SymfonyQuestionHelper();
        $input = $this->createStreamableInputInterfaceMock($this->getInputStream('MolliePrefix\\Foo\\Bar'));
        $helper->ask($input, $output = $this->createOutputInterface(), new \MolliePrefix\Symfony\Component\Console\Question\Question('Do you want to use Foo\\Bar <comment>or</comment> Foo\\Baz\\?', 'MolliePrefix\\Foo\\Baz'));
    ...
    public function testLabelTrailingBackslash()
    {
        ...
        $helper->ask($input, $output = $this->createOutputInterface(), new \MolliePrefix\Symfony\Component\Console\Question\Question('Question with a trailing \\'));
    ...
    public function testAskThrowsExceptionOnMissingInput()
    {
        ...
        $dialog = new \MolliePrefix\Symfony\Component\Console\Helper\SymfonyQuestionHelper();
    ...
    {
        $choiceQuestion = new \MolliePrefix\Symfony\Component\Console\Question\ChoiceQuestion('qqq', ['foo' => 'foo', 'żółw' => 'bar', 'łabądź' => 'baz']);
        (new \MolliePrefix\Symfony\Component\Console\Helper\SymfonyQuestionHelper())->ask($this->createStreamableInputInterfaceMock($this->getInputStream("foo\n")), $output = $this->createOutputInterface(), $choiceQuestion);
...
, $output, \true);
    ...
    {
        $choiceQuestion = new \MolliePrefix\Symfony\Component\Console\Question\ChoiceQuestion('qqq', ['foo']);
...
, $output, \true);
    ...
    {
        $output = new \MolliePrefix\Symfony\Component\Console\Output\StreamOutput(\fopen('php://memory', 'r+', \false));
    ...
    private function assertOutputContains($expected, \MolliePrefix\Symfony\Component\Console\Output\StreamOutput $output, $normalize = \false)
    {
