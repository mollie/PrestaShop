...
class ApplicationTesterTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    protected $application;
    protected $tester;
    protected function setUp()
    {
        $this->application = new \MolliePrefix\Symfony\Component\Console\Application();
        $this->application->setAutoExit(\false);
        $this->application->register('foo')->addArgument('foo')->setCode(function ($input, $output) {
            $output->writeln('foo');
        });
        $this->tester = new \MolliePrefix\Symfony\Component\Console\Tester\ApplicationTester($this->application);
        $this->tester->run(['command' => 'foo', 'foo' => 'bar'], ['interactive' => \false, 'decorated' => \false, 'verbosity' => \MolliePrefix\Symfony\Component\Console\Output\Output::VERBOSITY_VERBOSE]);
    }
