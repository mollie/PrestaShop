...
class TerminalTest extends \MolliePrefix\PHPUnit\Framework\TestCase
{
    private $colSize;
    private $lineSize;
    private $ansiCon;
    ...
    private function resetStatics()
    {
        foreach (['height', 'width', 'stty'] as $name) {
            $property = new \ReflectionProperty(\MolliePrefix\Symfony\Component\Console\Terminal::class, $name);
    ...
    public function test()
    {
        \putenv('COLUMNS=100');
        \putenv('LINES=50');
        $terminal = new \MolliePrefix\Symfony\Component\Console\Terminal();
