 ...
 */
class GlobResource implements \IteratorAggregate, \MolliePrefix\Symfony\Component\Config\Resource\SelfCheckingResourceInterface, \Serializable
{
    private $prefix;
    private $pattern;
    private $recursive;
    ...
    public function getIterator()
    {
        ...
        if (!\class_exists(\MolliePrefix\Symfony\Component\Finder\Finder::class)) {
            throw new \LogicException(\sprintf('Extended glob pattern "%s" cannot be used as the Finder component is not installed.', $this->pattern));
        }
        $finder = new \MolliePrefix\Symfony\Component\Finder\Finder();
        $regex = \MolliePrefix\Symfony\Component\Finder\Glob::toRegex($this->pattern);
        if ($this->recursive) {
