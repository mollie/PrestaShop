...
use MolliePrefix\Predis\Connection\Aggregate\ClusterInterface;
use MolliePrefix\Predis\Connection\Aggregate\RedisCluster;
use MolliePrefix\Predis\Connection\Factory;
use MolliePrefix\Predis\Response\Status;
...
trait RedisTrait
{
    ...
    public static function createConnection($dsn, array $options = [])
    {
        if (0 !== \strpos($dsn, 'redis://')) {
            throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Redis DSN: "%s" does not start with "redis://".', $dsn));
        }
        ...
        if (\false === ($params = \parse_url($params))) {
            throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Redis DSN: "%s".', $dsn));
        }
        ...
        if (\is_a($class, \MolliePrefix\Redis::class, \true)) {
            ...
            if ($params['lazy']) {
                $redis = new \MolliePrefix\Symfony\Component\Cache\Traits\RedisProxy($redis, $initializer);
            } else {
                $initializer($redis);
            }
        } elseif (\is_a($class, \MolliePrefix\Predis\Client::class, \true)) {
            $params['scheme'] = $scheme;
            $params['database'] = $params['dbindex'] ?: null;
            $params['password'] = $auth;
            $redis = new $class((new \MolliePrefix\Predis\Connection\Factory())->create($params));
        } elseif (\class_exists($class, \false)) {
            throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('"%s" is not a subclass of "Redis" or "Predis\\Client".', $class));
        } else {
            throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Class "%s" does not exist.', $class));
        }
        return $redis;
    }
    /**
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \MolliePrefix\Predis\Client && $this->redis->getConnection() instanceof \MolliePrefix\Predis\Connection\Aggregate\ClusterInterface) {
    ...
    protected function doClear($namespace)
    {
        ...
        if ($this->redis instanceof \MolliePrefix\Predis\Client) {
            $evalArgs = [0, $namespace];
            $connection = $this->redis->getConnection();
            if ($connection instanceof \MolliePrefix\Predis\Connection\Aggregate\ClusterInterface && $connection instanceof \Traversable) {
                $hosts = [];
        ...
        foreach ($hosts as $host) {
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \MolliePrefix\Predis\Client ? $host->scan($cursor, 'MATCH', $namespace . '*', 'COUNT', 1000) : $host->scan($cursor, $namespace . '*', 1000);
    ...
    protected function doSave(array $values, $lifetime)
    {
        ...
        foreach ($results as $id => $result) {
            if (\true !== $result && (!$result instanceof \MolliePrefix\Predis\Response\Status || $result !== \MolliePrefix\Predis\Response\Status::get('OK'))) {
                $failed[] = $id;
            }
        }
        return $failed;
    ...
    private function pipeline(\Closure $generator)
    {
        ...
        } elseif ($this->redis instanceof \MolliePrefix\Predis\Client) {
