    ...
    protected function doSave(array $values, $lifetime)
    {
        ...
        foreach ($values as $key => $value) {
            if (null === $value || \is_object($value)) {
                ...
                throw new \_PhpScoper5ece82d7231e4\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache key "%s" has non-serializable "%s" value.', $key, \gettype($value)));
            }
            $data[1] = $value;
        ...
        if (!$ok && !\is_writable($this->directory)) {
            throw new \_PhpScoper5ece82d7231e4\Symfony\Component\Cache\Exception\CacheException(\sprintf('Cache directory is not writable (%s).', $this->directory));
        }
        return $ok;
    }
}
