...
trait MemcachedTrait
{
    ...
    public static function createConnection($servers, array $options = [])
    {
        if (\is_string($servers)) {
            ...
            throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('MemcachedAdapter::createClient() expects array or string as first argument, "%s" given.', \gettype($servers)));
        }
        if (!static::isSupported()) {
            throw new \MolliePrefix\Symfony\Component\Cache\Exception\CacheException('Memcached >= 2.2.0 is required.');
        }
        ...
        try {
            ...
            foreach ($servers as $i => $dsn) {
                ...
                if (0 !== \strpos($dsn, 'memcached://')) {
                    throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Memcached DSN: "%s" does not start with "memcached://".', $dsn));
                }
                ...
                if (\false === ($params = \parse_url($params))) {
                    throw new \MolliePrefix\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Invalid Memcached DSN: "%s".', $dsn));
                }
