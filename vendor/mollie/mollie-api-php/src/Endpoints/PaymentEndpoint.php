...
class PaymentEndpoint extends \_PhpScoper5ece82d7231e4\Mollie\Api\Endpoints\CollectionEndpointAbstract
{
    ...
    public function get($paymentId, array $parameters = [])
    {
        if (empty($paymentId) || \strpos($paymentId, self::RESOURCE_ID_PREFIX) !== 0) {
            throw new \_PhpScoper5ece82d7231e4\Mollie\Api\Exceptions\ApiException("Invalid payment ID: '{$paymentId}'. A payment ID should start with '" . self::RESOURCE_ID_PREFIX . "'.");
        }
     ...
     * @throws ApiException
     */
    public function refund(\_PhpScoper5ece82d7231e4\Mollie\Api\Resources\Payment $payment, $data = [])
    {
