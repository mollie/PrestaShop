 ...
 */
class Extensions_PhptTestCaseTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
...
EOF;
    ...
    protected function setUp()
    {
        ...
        $this->testCase = new \MolliePrefix\PHPUnit_Extensions_PhptTestCase($this->filename, $this->phpUtil);
    }
    ...
    public function testShouldRunFileSectionAsTest()
    {
        ...
        $this->phpUtil->expects($this->once())->method('runJob')->with($fileSection)->will($this->returnValue(['stdout' => '', 'stderr' => '']));
        $this->testCase->run();
    }
    public function testShouldRunSkipifSectionWhenExists()
    {
        ...
        $this->phpUtil->expects($this->at(0))->method('runJob')->with($skipifSection)->will($this->returnValue(['stdout' => '', 'stderr' => '']));
        $this->testCase->run();
    }
    public function testShouldNotRunTestSectionIfSkipifSectionReturnsOutputWithSkipWord()
    {
        ...
        $phptContent .= $skipifSection;
        $this->setPhpContent($phptContent);
        $this->phpUtil->expects($this->once())->method('runJob')->with($skipifSection)->will($this->returnValue(['stdout' => 'skip: Reason', 'stderr' => '']));
        $this->testCase->run();
    }
    public function testShouldRunCleanSectionWhenDefined()
    {
        ...
        $this->setPhpContent($phptContent);
        $this->phpUtil->expects($this->at(1))->method('runJob')->with($cleanSection);
        $this->testCase->run();
    }
    /**
...
EOF
    ...
    {
        $this->setPhpContent(self::EXPECT_CONTENT);
        $this->phpUtil->expects($this->once())->method('runJob')->with(self::FILE_SECTION)->will($this->returnValue(['stdout' => 'Hello PHPUnit!', 'stderr' => '']));
    ...
    {
        $phptTestCase = new \MolliePrefix\PhpTestCaseProxy(__FILE__);
        $settings = $phptTestCase->parseIniSection("foo=1\nbar = 2\rbaz = 3\r\nempty=\nignore");
        $expected = ['foo=1', 'bar = 2', 'baz = 3', 'empty=', 'ignore'];
