 ...
 */
class PHPUnit_Util_Log_JUnit extends \MolliePrefix\PHPUnit_Util_Printer implements \MolliePrefix\PHPUnit_Framework_TestListener
{
    /**
     * @var DOMDocument
     */
    ...
    public function addIncompleteTest(\MolliePrefix\PHPUnit_Framework_Test $test, \Exception $e, $time)
    {
        if ($this->logIncompleteSkipped && $this->currentTestCase !== null) {
            $error = $this->document->createElement('error', \MolliePrefix\PHPUnit_Util_XML::prepareString("Incomplete Test\n" . \MolliePrefix\PHPUnit_Util_Filter::getFilteredStacktrace($e)));
    ...
    {
        if ($this->logIncompleteSkipped && $this->currentTestCase !== null) {
            $error = $this->document->createElement('error', \MolliePrefix\PHPUnit_Util_XML::prepareString("Risky Test\n" . \MolliePrefix\PHPUnit_Util_Filter::getFilteredStacktrace($e)));
            $error->setAttribute('type', \get_class($e));
            $this->currentTestCase->appendChild($error);
            $this->testSuiteErrors[$this->testSuiteLevel]++;
        } else {
    ...
    {
        if ($this->logIncompleteSkipped && $this->currentTestCase !== null) {
            $error = $this->document->createElement('error', \MolliePrefix\PHPUnit_Util_XML::prepareString("Skipped Test\n" . \MolliePrefix\PHPUnit_Util_Filter::getFilteredStacktrace($e)));
            $error->setAttribute('type', \get_class($e));
            $this->currentTestCase->appendChild($error);
            $this->testSuiteErrors[$this->testSuiteLevel]++;
        } else {
    ...
    public function endTest(\MolliePrefix\PHPUnit_Framework_Test $test, $time)
    {
        ...
        $this->attachCurrentTestCase = \true;
     ...
     * @param string                 $type
     */
    private function doAddFault(\MolliePrefix\PHPUnit_Framework_Test $test, \Exception $e, $time, $type)
    {
        ...
        $buffer .= \MolliePrefix\PHPUnit_Framework_TestFailure::exceptionToString($e) . \PHP_EOL . \MolliePrefix\PHPUnit_Util_Filter::getFilteredStacktrace($e);
        $fault = $this->document->createElement($type, \MolliePrefix\PHPUnit_Util_XML::prepareString($buffer));
        if ($e instanceof \MolliePrefix\PHPUnit_Framework_ExceptionWrapper) {
