...
class PHPUnit_Util_Test
{
     ...
     * @return mixed
     */
    public static function describe(\MolliePrefix\PHPUnit_Framework_Test $test, $asString = \true)
    {
        if ($asString) {
            if ($test instanceof \MolliePrefix\PHPUnit_Framework_SelfDescribing) {
            ...
            } elseif ($test instanceof \MolliePrefix\PHPUnit_Framework_SelfDescribing) {
                return ['', $test->toString()];
            } else {
                return ['', \get_class($test)];
            }
    ...
    private static function getLinesToBeCoveredOrUsed($className, $methodName, $mode)
    {
        ...
        if (!empty($annotations['class'][$mode . 'DefaultClass'])) {
            if (\count($annotations['class'][$mode . 'DefaultClass']) > 1) {
                throw new \MolliePrefix\PHPUnit_Framework_CodeCoverageException(\sprintf('More than one @%sClass annotation in class or interface "%s".', $mode, $className));
            }
    ...
    public static function getMissingRequirements($className, $methodName)
    {
        ...
        if (!empty($required['PHPUnit'])) {
            $phpunitVersion = \MolliePrefix\PHPUnit_Runner_Version::id();
    ...
    public static function getProvidedData($className, $methodName)
    {
        ...
        if (\is_array($data) && empty($data)) {
            throw new \MolliePrefix\PHPUnit_Framework_SkippedTestError();
        }
        if ($data !== null) {
            foreach ($data as $key => $value) {
                if (!\is_array($value)) {
                    throw new \MolliePrefix\PHPUnit_Framework_Exception(\sprintf('Data set %s is invalid.', \is_int($key) ? '#' . $key : '"' . $key . '"'));
                }
            }
        }
        return $data;
        ...
        if (\preg_match(self::REGEX_TEST_WITH, $docComment, $matches, \PREG_OFFSET_CAPTURE)) {
            ...
            foreach (\explode("\n", $annotationContent) as $candidateRow) {
                ...
                if (\json_last_error() != \JSON_ERROR_NONE) {
                    throw new \MolliePrefix\PHPUnit_Framework_Exception('The dataset for the @testWith annotation cannot be parsed: ' . \json_last_error_msg());
                }
                $data[] = $dataSet;
            }
            if (!$data) {
                throw new \MolliePrefix\PHPUnit_Framework_Exception('The dataset for the @testWith annotation cannot be parsed.');
            }
            return $data;
        }
    }
    ...
    public static function getSize($className, $methodName)
    {
        ...
        $size = self::UNKNOWN;
        $class = new \ReflectionClass($className);
        if (isset($groups['large']) || \class_exists('MolliePrefix\\PHPUnit_Extensions_Database_TestCase', \false) && $class->isSubclassOf('PHPUnit_Extensions_Database_TestCase')) {
            $size = self::LARGE;
    ...
    {
        return ['beforeClass' => ['setUpBeforeClass'], 'before' => ['setUp'], 'after' => ['tearDown'], 'afterClass' => ['tearDownAfterClass']];
    }
    /**
     * @param string $className
     * @param string $methodName
            ...
            if (isset($methodName[0]) && $methodName[0] == '<') {
                ...
                foreach ($classes as $className) {
                    if (!\class_exists($className) && !\interface_exists($className) && !\trait_exists($className)) {
                        throw new \MolliePrefix\PHPUnit_Framework_InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing class or ' . 'interface "%s".', $className));
                    }
                    $class = new \ReflectionClass($className);
                ...
                foreach ($classes as $className) {
                    if ($className == '' && \function_exists($methodName)) {
                        ...
                        if (!((\class_exists($className) || \interface_exists($className) || \trait_exists($className)) && \method_exists($className, $methodName))) {
                            throw new \MolliePrefix\PHPUnit_Framework_InvalidCoversTargetException(\sprintf('Trying to @cover or @use not existing method "%s::%s".', $className, $methodName));
                        }
