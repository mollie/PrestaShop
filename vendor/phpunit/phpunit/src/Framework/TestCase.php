 ...
 */
use MolliePrefix\SebastianBergmann\GlobalState\Snapshot;
use MolliePrefix\SebastianBergmann\GlobalState\Restorer;
use MolliePrefix\SebastianBergmann\GlobalState\Blacklist;
use MolliePrefix\SebastianBergmann\Diff\Differ;
use MolliePrefix\SebastianBergmann\Exporter\Exporter;
use MolliePrefix\SebastianBergmann\ObjectEnumerator\Enumerator;
 ...
 */
abstract class PHPUnit_Framework_TestCase extends \MolliePrefix\PHPUnit_Framework_Assert implements \MolliePrefix\PHPUnit_Framework_Test, \MolliePrefix\PHPUnit_Framework_SelfDescribing
{
    /**
    ...
    {
        return \MolliePrefix\PHPUnit_Util_Test::parseTestMethodAnnotations(\get_class($this), $this->name);
    }
    /**
    ...
    {
        return \MolliePrefix\PHPUnit_Util_Test::getSize(\get_class($this), $this->getName(\false));
    }
    /**
     * @return bool
     *
    ...
    {
        return $this->getSize() !== \MolliePrefix\PHPUnit_Util_Test::UNKNOWN;
    }
    /**
     * @return bool
     *
    ...
    {
        return $this->getSize() === \MolliePrefix\PHPUnit_Util_Test::SMALL;
    }
    /**
     * @return bool
     *
    ...
    {
        return $this->getSize() === \MolliePrefix\PHPUnit_Util_Test::MEDIUM;
    }
    /**
     * @return bool
     *
    ...
    {
        return $this->getSize() === \MolliePrefix\PHPUnit_Util_Test::LARGE;
    }
    /**
     * @return string
     *
    ...
    {
        try {
            $expectedException = \MolliePrefix\PHPUnit_Util_Test::getExpectedException(\get_class($this), $this->name);
    ...
    {
        try {
            $useErrorHandler = \MolliePrefix\PHPUnit_Util_Test::getErrorHandlerSettings(\get_class($this), $this->name);
    ...
    protected function checkRequirements()
    {
        ...
        $missingRequirements = \MolliePrefix\PHPUnit_Util_Test::getMissingRequirements(\get_class($this), $this->name);
    ...
    {
        $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_RISKY;
    }
    /**
    ...
    public function hasFailed()
    {
        ...
        return $status == \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_FAILURE || $status == \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_ERROR;
    }
    /**
    ...
    public function run(\MolliePrefix\PHPUnit_Framework_TestResult $result = null)
    {
        ...
        if (!$this instanceof \MolliePrefix\PHPUnit_Framework_WarningTestCase) {
        ...
        if (!$this instanceof \MolliePrefix\PHPUnit_Framework_WarningTestCase && !$this instanceof \MolliePrefix\PHPUnit_Framework_SkippedTestCase && !$this->handleDependencies()) {
            return;
        }
        if ($this->runTestInSeparateProcess === \true && $this->inIsolation !== \true && !$this instanceof \MolliePrefix\PHPUnit_Extensions_PhptTestCase) {
            $class = new \ReflectionClass($this);
            $template = new \MolliePrefix\Text_Template(__DIR__ . '/../Util/PHP/Template/TestCaseMethod.tpl');
            if ($this->preserveGlobalState) {
                $constants = \MolliePrefix\PHPUnit_Util_GlobalState::getConstantsAsString();
                $globals = \MolliePrefix\PHPUnit_Util_GlobalState::getGlobalsAsString();
                $includedFiles = \MolliePrefix\PHPUnit_Util_GlobalState::getIncludedFilesAsString();
                $iniSettings = \MolliePrefix\PHPUnit_Util_GlobalState::getIniSettingsAsString();
            } else {
                $constants = '';
    ...
    public function runBare()
    {
        ...
        $hookMethods = \MolliePrefix\PHPUnit_Util_Test::getHookMethods(\get_class($this));
        try {
            ...
            if (!empty($this->warnings)) {
                throw new \MolliePrefix\PHPUnit_Framework_Warning(\implode("\n", \array_unique($this->warnings)));
            }
            $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_PASSED;
        } catch (\MolliePrefix\PHPUnit_Framework_IncompleteTest $e) {
            $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_INCOMPLETE;
            $this->statusMessage = $e->getMessage();
        } catch (\MolliePrefix\PHPUnit_Framework_SkippedTest $e) {
            $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_SKIPPED;
            $this->statusMessage = $e->getMessage();
        } catch (\MolliePrefix\PHPUnit_Framework_Warning $e) {
            $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_WARNING;
            $this->statusMessage = $e->getMessage();
        } catch (\MolliePrefix\PHPUnit_Framework_AssertionFailedError $e) {
            $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_FAILURE;
            $this->statusMessage = $e->getMessage();
        ...
        if (isset($_e)) {
            $this->status = \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_ERROR;
        ...
        } catch (\MolliePrefix\PHPUnit_Framework_RiskyTestError $_e) {
            if (!isset($e)) {
                $e = $_e;
            }
        }
        ...
        if (isset($e)) {
            if ($e instanceof \MolliePrefix\Prophecy\Exception\Prediction\PredictionException) {
                $e = new \MolliePrefix\PHPUnit_Framework_AssertionFailedError($e->getMessage());
            }
    ...
    {
        if ($this->name === null) {
            throw new \MolliePrefix\PHPUnit_Framework_Exception('PHPUnit_Framework_TestCase::$name must not be null.');
        }
        try {
            $class = new \ReflectionClass($this);
        ...
        if (isset($e)) {
            ...
            if (!$e instanceof \MolliePrefix\PHPUnit_Framework_SkippedTestError && \is_string($this->expectedException)) {
                $checkException = \true;
                if ($e instanceof \MolliePrefix\PHPUnit_Framework_Exception) {
                    $checkException = \false;
                }
                $reflector = new \ReflectionClass($this->expectedException);
                if ($this->expectedException === 'PHPUnit_Framework_Exception' || $this->expectedException === '\\PHPUnit_Framework_Exception' || $reflector->isSubclassOf('PHPUnit_Framework_Exception')) {
                    $checkException = \true;
                }
            }
            if ($checkException) {
                $this->assertThat($e, new \MolliePrefix\PHPUnit_Framework_Constraint_Exception($this->expectedException));
                if (\is_string($this->expectedExceptionMessage) && !empty($this->expectedExceptionMessage)) {
                    $this->assertThat($e, new \MolliePrefix\PHPUnit_Framework_Constraint_ExceptionMessage($this->expectedExceptionMessage));
                }
                if (\is_string($this->expectedExceptionMessageRegExp) && !empty($this->expectedExceptionMessageRegExp)) {
                    $this->assertThat($e, new \MolliePrefix\PHPUnit_Framework_Constraint_ExceptionMessageRegExp($this->expectedExceptionMessageRegExp));
                }
                if ($this->expectedExceptionCode !== null) {
                    $this->assertThat($e, new \MolliePrefix\PHPUnit_Framework_Constraint_ExceptionCode($this->expectedExceptionCode));
                }
                return;
            } else {
                throw $e;
        ...
        if ($this->expectedException !== null) {
            $this->assertThat(null, new \MolliePrefix\PHPUnit_Framework_Constraint_Exception($this->expectedException));
        }
        return $testResult;
    }
    /**
    ...
    public function setOutputCallback($callback)
    {
        if (!\is_callable($callback)) {
            throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(1, 'callback');
        }
     ...
     * @since Method available since Release 3.6.0
     */
    public function setTestResultObject(\MolliePrefix\PHPUnit_Framework_TestResult $result)
    {
        $this->result = $result;
    }
    /**
     ...
     * @since Method available since Release 5.4.0
     */
    public function registerMockObject(\MolliePrefix\PHPUnit_Framework_MockObject_MockObject $mockObject)
    {
    ...
    protected function iniSet($varName, $newValue)
    {
        ...
        if ($currentValue !== \false) {
            ...
            throw new \MolliePrefix\PHPUnit_Framework_Exception(\sprintf('INI setting "%s" could not be set to "%s".', $varName, $newValue));
        }
    }
    /**
    ...
    protected function setLocale()
    {
        ...
        if ($result === \false) {
            throw new \MolliePrefix\PHPUnit_Framework_Exception('The locale functionality is not implemented on your platform, ' . 'the specified locale does not exist or the category name is ' . 'invalid.');
        }
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_MockBuilder($this, $className);
    }
    /**
     ...
     */
    protected function getMock($originalClassName, $methods = [], array $arguments = [], $mockClassName = '', $callOriginalConstructor = \true, $callOriginalClone = \true, $callAutoload = \true, $cloneArguments = \false, $callOriginalMethods = \false, $proxyTarget = null)
    {
        ...
        $mockObject = $this->getMockObjectGenerator()->getMock($originalClassName, $methods, $arguments, $mockClassName, $callOriginalConstructor, $callOriginalClone, $callAutoload, $cloneArguments, $callOriginalMethods, $proxyTarget);
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_AnyInvokedCount();
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedCount(0);
    }
    /**
     * Returns a matcher that matches when the method is executed
     * at least N times.
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedAtLeastCount($requiredInvocations);
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedAtLeastOnce();
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedCount(1);
    }
    /**
     * Returns a matcher that matches when the method is executed
     * exactly $count times.
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedCount($count);
    }
    /**
     * Returns a matcher that matches when the method is executed
     * at most N times.
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedAtMostCount($allowedInvocations);
    }
    /**
     * Returns a matcher that matches when the method is executed
     * at the given index.
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Matcher_InvokedAtIndex($index);
    }
    /**
     * @param mixed $value
     *
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_Return($value);
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_ReturnValueMap($valueMap);
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_ReturnArgument($argumentIndex);
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_ReturnCallback($callback);
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_ReturnSelf();
    }
    /**
    ...
    {
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_Exception($exception);
    }
    /**
     * @param mixed $value, ...
     *
    ...
    {
        $args = \func_get_args();
        return new \MolliePrefix\PHPUnit_Framework_MockObject_Stub_ConsecutiveCalls($args);
    }
    /**
     * @return bool
     *
    ...
    protected function getDataSetAsString($includeData = \true)
    {
        ...
        if (!empty($this->data)) {
            ...
            $exporter = new \MolliePrefix\SebastianBergmann\Exporter\Exporter();
            if ($includeData) {
        ...
        if (!empty($this->dependencies) && !$this->inIsolation) {
            ...
            foreach ($this->dependencies as $dependency) {
                ...
                if (!isset($passedKeys[$dependency])) {
                    ...
                    $this->result->addError($this, new \MolliePrefix\PHPUnit_Framework_SkippedTestError(\sprintf('This test depends on "%s" to pass.', $dependency)), 0);
                    $this->result->endTest($this, 0);
                    return \false;
                }
                if (isset($passed[$dependency])) {
                    if ($passed[$dependency]['size'] != \MolliePrefix\PHPUnit_Util_Test::UNKNOWN && $this->getSize() != \MolliePrefix\PHPUnit_Util_Test::UNKNOWN && $passed[$dependency]['size'] > $this->getSize()) {
                        $this->result->addError($this, new \MolliePrefix\PHPUnit_Framework_SkippedTestError('This test depends on a test that is larger than itself.'), 0);
                        return \false;
                    }
                    if ($clone) {
                        $deepCopy = new \MolliePrefix\DeepCopy\DeepCopy();
    ...
    protected function onNotSuccessfulTest($e)
    {
        ...
        throw \MolliePrefix\PHPUnit_Util_InvalidArgumentHelper::factory(1, 'Throwable or Exception');
    }
    /**
     ...
     * @since Method available since Release 3.4.0
     */
    protected function prepareTemplate(\MolliePrefix\Text_Template $template)
    {
    }
    /**
    ...
    protected function getMockObjectGenerator()
    {
        if (null === $this->mockObjectGenerator) {
            $this->mockObjectGenerator = new \MolliePrefix\PHPUnit_Framework_MockObject_Generator();
        }
    ...
    private function stopOutputBuffering()
    {
        if (\ob_get_level() != $this->outputBufferingLevel) {
            ...
            throw new \MolliePrefix\PHPUnit_Framework_RiskyTestError('Test code or tested code did not (only) close its own output buffers');
        }
    ...
    {
        if (!$this->snapshot instanceof \MolliePrefix\SebastianBergmann\GlobalState\Snapshot) {
            return;
        }
        $backupGlobals = $this->backupGlobals === null || $this->backupGlobals === \true;
        if ($this->beStrictAboutChangesToGlobalState) {
            ...
            } catch (\MolliePrefix\PHPUnit_Framework_RiskyTestError $rte) {
                // Intentionally left empty
            }
        }
        $restorer = new \MolliePrefix\SebastianBergmann\GlobalState\Restorer();
        if ($backupGlobals) {
    ...
    {
        $blacklist = new \MolliePrefix\SebastianBergmann\GlobalState\Blacklist();
        ...
        if (!\defined('PHPUNIT_TESTSUITE')) {
            ...
            $blacklist->addClassNamePrefix('MolliePrefix\\SebastianBergmann\\CodeCoverage');
            $blacklist->addClassNamePrefix('PHP_Invoker');
            $blacklist->addClassNamePrefix('PHP_Timer');
            $blacklist->addClassNamePrefix('PHP_Token');
            $blacklist->addClassNamePrefix('Symfony');
            $blacklist->addClassNamePrefix('Text_Template');
            $blacklist->addClassNamePrefix('MolliePrefix\\Doctrine\\Instantiator');
        ...
        return new \MolliePrefix\SebastianBergmann\GlobalState\Snapshot($blacklist, $backupGlobals, (bool) $this->backupStaticAttributes, \false, \false, \false, \false, \false, \false, \false);
    }
    /**
     ...
     */
    private function compareGlobalStateSnapshots(\MolliePrefix\SebastianBergmann\GlobalState\Snapshot $before, \MolliePrefix\SebastianBergmann\GlobalState\Snapshot $after)
    {
        $backupGlobals = $this->backupGlobals === null || $this->backupGlobals === \true;
        if ($backupGlobals) {
    ...
    {
        if ($before != $after) {
            $differ = new \MolliePrefix\SebastianBergmann\Diff\Differ($header);
            $exporter = new \MolliePrefix\SebastianBergmann\Exporter\Exporter();
            $diff = $differ->diff($exporter->export($before), $exporter->export($after));
            throw new \MolliePrefix\PHPUnit_Framework_RiskyTestError($diff);
        }
    }
    /**
    ...
    {
        if ($this->prophet === null) {
            $this->prophet = new \MolliePrefix\Prophecy\Prophet();
        }
        return $this->prophet;
    }
    /**
     ...
     * @since Method available since Release 5.0.0
     */
    private function shouldInvocationMockerBeReset(\MolliePrefix\PHPUnit_Framework_MockObject_MockObject $mock)
    {
        $enumerator = new \MolliePrefix\SebastianBergmann\ObjectEnumerator\Enumerator();
    ...
    private function registerMockObjectsFromTestArguments(array $testArguments)
    {
        if ($this->registerMockObjectsFromTestArgumentsRecursively) {
            ...
            foreach ($enumerator->enumerate($testArguments) as $object) {
                if ($object instanceof \MolliePrefix\PHPUnit_Framework_MockObject_MockObject) {
                    $this->registerMockObject($object);
                }
            }
        } else {
            foreach ($testArguments as $testArgument) {
                if ($testArgument instanceof \MolliePrefix\PHPUnit_Framework_MockObject_MockObject) {
     ...
     * @return bool
     */
    private function isCloneable(\MolliePrefix\PHPUnit_Framework_MockObject_MockObject $testArgument)
    {
