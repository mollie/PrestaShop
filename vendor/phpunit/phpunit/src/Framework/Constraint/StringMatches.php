 ...
 * ...
 */
class PHPUnit_Framework_Constraint_StringMatches extends \MolliePrefix\PHPUnit_Framework_Constraint_PCREMatch
{
    /**
     * @var string
     */
    ...
    protected function additionalFailureDescription($other)
    {
        ...
        $this->string = \implode("\n", $from);
        $other = \implode("\n", $to);
        $differ = new \MolliePrefix\SebastianBergmann\Diff\Differ("--- Expected\n+++ Actual\n");
        return $differ->diff($this->string, $other);
    }
    protected function createPatternFromFormat($string)
    {
        $string = \str_replace(['%e', '%s', '%S', '%a', '%A', '%w', '%i', '%d', '%x', '%f', '%c'], ['\\' . \DIRECTORY_SEPARATOR, '[^\\r\\n]+', '[^\\r\\n]*', '.+', '.*', '\\s*', '[+-]?\\d+', '\\d+', '[0-9a-fA-F]+', '[+-]?\\.?\\d+\\.?\\d*(?:[Ee][+-]?\\d+)?', '.'], \preg_quote($string, '/'));
        return '/^' . $string . '$/s';
    }
}
/**
