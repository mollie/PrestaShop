 ...
 */
abstract class PHPUnit_Extensions_TicketListener implements \MolliePrefix\PHPUnit_Framework_TestListener
{
    /**
     * @var array
     */
    ...
    public function startTest(\MolliePrefix\PHPUnit_Framework_Test $test)
    {
        if (!$test instanceof \MolliePrefix\PHPUnit_Framework_WarningTestCase) {
            ...
            $tickets = \MolliePrefix\PHPUnit_Util_Test::getTickets(\get_class($test), $name);
    ...
    {
        if (!$test instanceof \MolliePrefix\PHPUnit_Framework_WarningTestCase) {
            if ($test->getStatus() == \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_PASSED) {
                $ifStatus = ['assigned', 'new', 'reopened'];
                $newStatus = 'closed';
                $message = 'Automatically closed by PHPUnit (test passed).';
                $resolution = 'fixed';
                $cumulative = \true;
            } elseif ($test->getStatus() == \MolliePrefix\PHPUnit_Runner_BaseTestRunner::STATUS_FAILURE) {
                $ifStatus = ['closed'];
                $newStatus = 'reopened';
