 ...
 */
class PHPUnit_Extensions_PhptTestCase implements \MolliePrefix\PHPUnit_Framework_Test, \MolliePrefix\PHPUnit_Framework_SelfDescribing
{
    /**
     * @var string
     */
    ...
    public function __construct($filename, $phpUtil = null)
    {
        ...
        if (!\is_file($filename)) {
            throw new \MolliePrefix\PHPUnit_Framework_Exception(\sprintf('File "%s" does not exist.', $filename));
        }
        $this->filename = $filename;
        $this->phpUtil = $phpUtil ?: \MolliePrefix\PHPUnit_Util_PHP::factory();
    }
    /**
    ...
    {
        $assertions = ['EXPECT' => 'assertEquals', 'EXPECTF' => 'assertStringMatchesFormat', 'EXPECTREGEX' => 'assertRegExp'];
        $actual = \preg_replace('/\\r\\n/', "\n", \trim($output));
        ...
        \MolliePrefix\PHPUnit_Framework_Assert::$assertion($expected, $actual);
    }
    /**
    ...
    public function run(\MolliePrefix\PHPUnit_Framework_TestResult $result = null)
    {
        ...
        if (isset($sections['SKIPIF'])) {
            ...
            if (!\strncasecmp('skip', \ltrim($jobResult['stdout']), 4)) {
                ...
                $result->addFailure($this, new \MolliePrefix\PHPUnit_Framework_SkippedTestError($message), 0);
                $skip = \true;
            }
        }
        ...
        if (!$skip) {
            ...
            try {
                ...
                if ($xfail !== \false) {
                    $result->addFailure($this, new \MolliePrefix\PHPUnit_Framework_IncompleteTestError($xfail, 0, $e), $time);
                } else {
            ...
            if ($result->allCompletelyImplemented() && $xfail !== \false) {
                $result->addFailure($this, new \MolliePrefix\PHPUnit_Framework_IncompleteTestError('XFAIL section but test passes'), $time);
            }
    ...
    private function parse()
    {
        $sections = [];
        $section = '';
        $allowExternalSections = ['FILE', 'EXPECT', 'EXPECTF', 'EXPECTREGEX'];
        $requiredSections = ['FILE', ['EXPECT', 'EXPECTF', 'EXPECTREGEX']];
        $unsupportedSections = ['REDIRECTTEST', 'REQUEST', 'POST', 'PUT', 'POST_RAW', 'GZIP_POST', 'DEFLATE_POST', 'GET', 'COOKIE', 'HEADERS', 'CGI', 'EXPECTHEADERS', 'EXTENSIONS', 'PHPDBG'];
        foreach (\file($this->filename) as $line) {
            if (\preg_match('/^--([_A-Z]+)--/', $line, $result)) {
                ...
                throw new \MolliePrefix\PHPUnit_Framework_Exception('Invalid PHPT file');
            }
        ...
        foreach ($allowExternalSections as $section) {
            if (isset($sections[$section . '_EXTERNAL'])) {
                ...
                $externalFilename = \str_replace('..', '', \trim($sections[$section . '_EXTERNAL']));
                ...
                if (!\is_file($testDirectory . $externalFilename) || !\is_readable($testDirectory . $externalFilename)) {
                    throw new \MolliePrefix\PHPUnit_Framework_Exception(\sprintf('Could not load --%s-- %s for PHPT file', $section . '_EXTERNAL', $testDirectory . $externalFilename));
                }
        ...
        foreach ($unsupportedSections as $section) {
            if (isset($sections[$section])) {
                throw new \MolliePrefix\PHPUnit_Framework_Exception('PHPUnit does not support this PHPT file');
            }
        }
        return $sections;
    }
