 ...
 * </code>
 */
class PHPUnit_Extensions_GroupTestSuite extends \MolliePrefix\PHPUnit_Framework_TestSuite
{
    public function __construct(\MolliePrefix\PHPUnit_Framework_TestSuite $suite, array $groups)
    {
        $groupSuites = [];
        $name = $suite->getName();
        foreach ($groups as $group) {
            $groupSuites[$group] = new \MolliePrefix\PHPUnit_Framework_TestSuite($name . ' - ' . $group);
            $this->addTest($groupSuites[$group]);
        }
        $tests = new \RecursiveIteratorIterator(new \MolliePrefix\PHPUnit_Util_TestSuiteIterator($suite), \RecursiveIteratorIterator::LEAVES_ONLY);
        foreach ($tests as $test) {
            if ($test instanceof \MolliePrefix\PHPUnit_Framework_TestCase) {
                $testGroups = \MolliePrefix\PHPUnit_Util_Test::getGroups(\get_class($test), $test->getName(\false));
                foreach ($groups as $group) {
