...
use MolliePrefix\SebastianBergmann\CodeCoverage\Filter as CodeCoverageFilter;
use MolliePrefix\SebastianBergmann\CodeCoverage\Report\Clover as CloverReport;
use MolliePrefix\SebastianBergmann\CodeCoverage\Report\Crap4j as Crap4jReport;
use MolliePrefix\SebastianBergmann\CodeCoverage\Report\Html\Facade as HtmlReport;
use MolliePrefix\SebastianBergmann\CodeCoverage\Report\PHP as PhpReport;
use MolliePrefix\SebastianBergmann\CodeCoverage\Report\Text as TextReport;
use MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Facade as XmlReport;
use MolliePrefix\SebastianBergmann\Environment\Runtime;
/**
 * A TestRunner for the Command Line Interface (CLI)
 * PHP SAPI Module.
 */
class PHPUnit_TextUI_TestRunner extends \MolliePrefix\PHPUnit_Runner_BaseTestRunner
{
     ...
     */
    public function __construct(\MolliePrefix\PHPUnit_Runner_TestSuiteLoader $loader = null, \MolliePrefix\SebastianBergmann\CodeCoverage\Filter $filter = null)
    {
        if ($filter === null) {
            $filter = new \MolliePrefix\SebastianBergmann\CodeCoverage\Filter();
        }
    ...
    public static function run($test, array $arguments = [])
    {
        if ($test instanceof \ReflectionClass) {
            $test = new \MolliePrefix\PHPUnit_Framework_TestSuite($test);
        }
        if ($test instanceof \MolliePrefix\PHPUnit_Framework_Test) {
            $aTestRunner = new self();
            return $aTestRunner->doRun($test, $arguments);
        } else {
            throw new \MolliePrefix\PHPUnit_Framework_Exception('No test case or test suite found.');
        }
    }
    /**
     * @return PHPUnit_Framework_TestResult
     ...
     * @param array                       $arguments
     */
    private function processSuiteFilters(\MolliePrefix\PHPUnit_Framework_TestSuite $suite, array $arguments)
    {
        if (!$arguments['filter'] && empty($arguments['groups']) && empty($arguments['excludeGroups'])) {
            return;
        }
        $filterFactory = new \MolliePrefix\PHPUnit_Runner_Filter_Factory();
     ...
     * @return PHPUnit_Framework_TestResult
     */
    public function doRun(\MolliePrefix\PHPUnit_Framework_Test $suite, array $arguments = [], $exit = \true)
    {
        ...
        if (\is_int($arguments['repeat'])) {
            $test = new \MolliePrefix\PHPUnit_Extensions_RepeatedTest($suite, $arguments['repeat'], $arguments['processIsolation']);
        ...
        if (!$arguments['convertNoticesToExceptions']) {
            \MolliePrefix\PHPUnit_Framework_Error_Notice::$enabled = \false;
        }
        if (!$arguments['convertWarningsToExceptions']) {
            \MolliePrefix\PHPUnit_Framework_Error_Warning::$enabled = \false;
        }
        ...
        if ($this->printer === null) {
            if (isset($arguments['printer']) && $arguments['printer'] instanceof \MolliePrefix\PHPUnit_Util_Printer) {
        ...
        if (!$this->printer instanceof \MolliePrefix\PHPUnit_Util_Log_TAP) {
            $this->printer->write(\MolliePrefix\PHPUnit_Runner_Version::getVersionString() . "\n");
        ...
        if (isset($arguments['testdoxHTMLFile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_TestDox_ResultPrinter_HTML($arguments['testdoxHTMLFile'], $arguments['testdoxGroups'], $arguments['testdoxExcludeGroups']));
        }
        if (isset($arguments['testdoxTextFile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_TestDox_ResultPrinter_Text($arguments['testdoxTextFile'], $arguments['testdoxGroups'], $arguments['testdoxExcludeGroups']));
        }
        if (isset($arguments['testdoxXMLFile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_TestDox_ResultPrinter_XML($arguments['testdoxXMLFile']));
        }
        ...
        if ($codeCoverageReports > 0) {
            $codeCoverage = new \MolliePrefix\SebastianBergmann\CodeCoverage\CodeCoverage(null, $this->codeCoverageFilter);
            $codeCoverage->setUnintentionallyCoveredSubclassesWhitelist([\MolliePrefix\SebastianBergmann\Comparator\Comparator::class]);
        ...
        if (isset($arguments['jsonLogfile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_Log_JSON($arguments['jsonLogfile']));
        }
        if (isset($arguments['tapLogfile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_Log_TAP($arguments['tapLogfile']));
        }
        if (isset($arguments['teamcityLogfile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_Log_TeamCity($arguments['teamcityLogfile']));
        }
        if (isset($arguments['junitLogfile'])) {
            $result->addListener(new \MolliePrefix\PHPUnit_Util_Log_JUnit($arguments['junitLogfile'], $arguments['logIncompleteSkipped']));
        }
        ...
        if ($suite instanceof \MolliePrefix\PHPUnit_Framework_TestSuite) {
            $suite->setRunTestInSeparateProcess($arguments['processIsolation']);
        }
        $suite->run($result);
        unset($suite);
        $result->flushListeners();
        if ($this->printer instanceof \MolliePrefix\PHPUnit_TextUI_ResultPrinter) {
        ...
        if (isset($codeCoverage)) {
            if (isset($arguments['coverageClover'])) {
                ...
                try {
                    $writer = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Clover();
                    $writer->process($codeCoverage, $arguments['coverageClover']);
                    $this->printer->write(" done\n");
                    unset($writer);
                } catch (\MolliePrefix\SebastianBergmann\CodeCoverage\Exception $e) {
            ...
            if (isset($arguments['coverageCrap4J'])) {
                ...
                try {
                    $writer = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Crap4j($arguments['crap4jThreshold']);
            ...
            if (isset($arguments['coverageHtml'])) {
                ...
                try {
                    $writer = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Html\Facade($arguments['reportLowUpperBound'], $arguments['reportHighLowerBound'], \sprintf(' and <a href="https://phpunit.de/">PHPUnit %s</a>', \MolliePrefix\PHPUnit_Runner_Version::id()));
            ...
            if (isset($arguments['coveragePHP'])) {
                ...
                try {
                    $writer = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\PHP();
            ...
            if (isset($arguments['coverageText'])) {
                if ($arguments['coverageText'] == 'php://stdout') {
                    ...
                    $colors = $arguments['colors'] && $arguments['colors'] != \MolliePrefix\PHPUnit_TextUI_ResultPrinter::COLOR_NEVER;
                } else {
                    $outputStream = new \MolliePrefix\PHPUnit_Util_Printer($arguments['coverageText']);
                    $colors = \false;
                }
                $processor = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Text($arguments['reportLowUpperBound'], $arguments['reportHighLowerBound'], $arguments['coverageTextShowUncoveredFiles'], $arguments['coverageTextShowOnlySummary']);
                $outputStream->write($processor->process($codeCoverage, $colors));
            }
            if (isset($arguments['coverageXml'])) {
                $this->printer->write("\nGenerating code coverage report in PHPUnit XML format ...");
                try {
                    $writer = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Facade();
     ...
     */
    public function setPrinter(\MolliePrefix\PHPUnit_TextUI_ResultPrinter $resultPrinter)
    {
    ...
    {
        if ($this->loader === null) {
            $this->loader = new \MolliePrefix\PHPUnit_Runner_StandardTestSuiteLoader();
        }
        return $this->loader;
    }
    /**
    ...
    {
        if (isset($arguments['configuration']) && !$arguments['configuration'] instanceof \MolliePrefix\PHPUnit_Util_Configuration) {
            $arguments['configuration'] = \MolliePrefix\PHPUnit_Util_Configuration::getInstance($arguments['configuration']);
        }
        ...
        if (isset($arguments['configuration'])) {
            ...
            if (isset($phpunitConfiguration['deprecatedCheckForUnintentionallyCoveredCodeSettingUsed'])) {
                $arguments['deprecatedCheckForUnintentionallyCoveredCodeSettingUsed'] = \true;
            }
            ...
            foreach ($arguments['configuration']->getListenerConfiguration() as $listener) {
                ...
                if (!\class_exists($listener['class'])) {
                    throw new \MolliePrefix\PHPUnit_Framework_Exception(\sprintf('Class "%s" does not exist', $listener['class']));
                }
                $listenerClass = new \ReflectionClass($listener['class']);
                if (!$listenerClass->implementsInterface(\MolliePrefix\PHPUnit_Framework_TestListener::class)) {
                    throw new \MolliePrefix\PHPUnit_Framework_Exception(\sprintf('Class "%s" does not implement the PHPUnit_Framework_TestListener interface', $listener['class']));
                }
            ...
            if (isset($loggingConfiguration['plain'])) {
                $arguments['listeners'][] = new \MolliePrefix\PHPUnit_TextUI_ResultPrinter($loggingConfiguration['plain'], \true);
            }
        ...
        $arguments['addUncoveredFilesFromWhitelist'] = isset($arguments['addUncoveredFilesFromWhitelist']) ? $arguments['addUncoveredFilesFromWhitelist'] : \true;
        ...
        $arguments['colors'] = isset($arguments['colors']) ? $arguments['colors'] : \MolliePrefix\PHPUnit_TextUI_ResultPrinter::COLOR_DEFAULT;
        $arguments['convertErrorsToExceptions'] = isset($arguments['convertErrorsToExceptions']) ? $arguments['convertErrorsToExceptions'] : \true;
        $arguments['convertNoticesToExceptions'] = isset($arguments['convertNoticesToExceptions']) ? $arguments['convertNoticesToExceptions'] : \true;
        $arguments['convertWarningsToExceptions'] = isset($arguments['convertWarningsToExceptions']) ? $arguments['convertWarningsToExceptions'] : \true;
