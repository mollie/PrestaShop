...
class PHPUnit_TextUI_Command
{
     ...
     * @var array
     */
    protected $arguments = ['listGroups' => \false, 'listSuites' => \false, 'loader' => null, 'useDefaultConfiguration' => \true, 'loadedExtensions' => [], 'notLoadedExtensions' => []];
    /**
     * @var array
     */
    protected $options = [];
    ...
    public function run(array $argv, $exit = \true)
    {
        ...
        if (\is_object($this->arguments['test']) && $this->arguments['test'] instanceof \MolliePrefix\PHPUnit_Framework_Test) {
        ...
        if ($this->arguments['listGroups']) {
            ...
            if ($exit) {
                exit(\MolliePrefix\PHPUnit_TextUI_TestRunner::SUCCESS_EXIT);
            } else {
                return \MolliePrefix\PHPUnit_TextUI_TestRunner::SUCCESS_EXIT;
            }
        }
        if ($this->arguments['listSuites']) {
            $this->printVersionString();
            print "Available test suite(s):\n";
            $configuration = \MolliePrefix\PHPUnit_Util_Configuration::getInstance($this->arguments['configuration']);
        ...
        $return = \MolliePrefix\PHPUnit_TextUI_TestRunner::FAILURE_EXIT;
        if (isset($result) && $result->wasSuccessful(\false)) {
            $return = \MolliePrefix\PHPUnit_TextUI_TestRunner::SUCCESS_EXIT;
        } elseif (!isset($result) || $result->errorCount() > 0) {
            $return = \MolliePrefix\PHPUnit_TextUI_TestRunner::EXCEPTION_EXIT;
        }
        if ($exit) {
            exit($return);
        }
    ...
    {
        return new \MolliePrefix\PHPUnit_TextUI_TestRunner($this->arguments['loader']);
    }
    /**
    ...
    protected function handleArguments(array $argv)
    {
        ...
        try {
            $this->options = \MolliePrefix\PHPUnit_Util_Getopt::getopt($argv, 'd:c:hv', \array_keys($this->longOptions));
        } catch (\MolliePrefix\PHPUnit_Framework_Exception $e) {
        ...
        foreach ($this->options[0] as $option) {
            switch ($option[0]) {
                case '--colors':
                    $this->arguments['colors'] = $option[1] ?: \MolliePrefix\PHPUnit_TextUI_ResultPrinter::COLOR_AUTO;
                    break;
                case '--bootstrap':
                ...
                case '--generate-configuration':
                    ...
                    $generator = new \MolliePrefix\PHPUnit_Util_ConfigurationGenerator();
                    \file_put_contents('phpunit.xml', $generator->generateDefaultConfiguration(\MolliePrefix\PHPUnit_Runner_Version::series(), $bootstrapScript, $testsDirectory, $src));
                ...
                case '--include-path':
                    ...
                    break;
                case '--list-groups':
                    $this->arguments['listGroups'] = \true;
                    break;
                case '--list-suites':
                    $this->arguments['listSuites'] = \true;
                    break;
                case '--printer':
                ...
                case '--process-isolation':
                    $this->arguments['processIsolation'] = \true;
                    break;
                case '--repeat':
                    $this->arguments['repeat'] = (int) $option[1];
                    break;
                case '--stderr':
                    $this->arguments['stderr'] = \true;
                    break;
                case '--stop-on-error':
                    $this->arguments['stopOnError'] = \true;
                    break;
                case '--stop-on-failure':
                    $this->arguments['stopOnFailure'] = \true;
                    break;
                case '--stop-on-warning':
                    $this->arguments['stopOnWarning'] = \true;
                    break;
                case '--stop-on-incomplete':
                    $this->arguments['stopOnIncomplete'] = \true;
                    break;
                case '--stop-on-risky':
                    $this->arguments['stopOnRisky'] = \true;
                    break;
                case '--stop-on-skipped':
                    $this->arguments['stopOnSkipped'] = \true;
                    break;
                case '--fail-on-warning':
                    $this->arguments['failOnWarning'] = \true;
                    break;
                case '--fail-on-risky':
                    $this->arguments['failOnRisky'] = \true;
                    break;
                case '--tap':
                ...
                case '--no-configuration':
                    ...
                    break;
                case '--no-extensions':
                    $this->arguments['noExtensions'] = \true;
                    break;
                case '--no-coverage':
                    $this->arguments['noCoverage'] = \true;
                    break;
                case '--no-globals-backup':
                    $this->arguments['backupGlobals'] = \false;
                    break;
                case '--static-backup':
                    $this->arguments['backupStaticAttributes'] = \true;
                    break;
                case 'v':
                case '--verbose':
                    $this->arguments['verbose'] = \true;
                    break;
                case '--atleast-version':
                    exit(\version_compare(\MolliePrefix\PHPUnit_Runner_Version::id(), $option[1], '>=') ? \MolliePrefix\PHPUnit_TextUI_TestRunner::SUCCESS_EXIT : \MolliePrefix\PHPUnit_TextUI_TestRunner::FAILURE_EXIT);
                    break;
                case '--version':
                    $this->printVersionString();
                    exit(\MolliePrefix\PHPUnit_TextUI_TestRunner::SUCCESS_EXIT);
                ...
                case '--report-useless-tests':
                    $this->arguments['reportUselessTests'] = \true;
                    break;
                case '--strict-coverage':
                    $this->arguments['strictCoverage'] = \true;
                    break;
                case '--disable-coverage-ignore':
                    $this->arguments['disableCodeCoverageIgnore'] = \true;
                    break;
                case '--strict-global-state':
                    $this->arguments['beStrictAboutChangesToGlobalState'] = \true;
                    break;
                case '--disallow-test-output':
                    $this->arguments['disallowTestOutput'] = \true;
                    break;
                case '--disallow-resource-usage':
                    $this->arguments['beStrictAboutResourceUsageDuringSmallTests'] = \true;
                    break;
                case '--enforce-time-limit':
                    $this->arguments['enforceTimeLimit'] = \true;
                    break;
                case '--disallow-todo-tests':
                    $this->arguments['disallowTodoAnnotatedTests'] = \true;
                    break;
                case '--reverse-list':
                    $this->arguments['reverseList'] = \true;
                    break;
                case '--check-version':
        ...
        if (isset($this->arguments['configuration'])) {
            try {
                ...
                print $e->getMessage() . "\n";
                exit(\MolliePrefix\PHPUnit_TextUI_TestRunner::FAILURE_EXIT);
            } catch (\Exception $e) {
                print $e->getMessage() . "\n";
                exit(\MolliePrefix\PHPUnit_TextUI_TestRunner::FAILURE_EXIT);
            }
        ...
        if (isset($this->arguments['test']) && \is_string($this->arguments['test']) && \substr($this->arguments['test'], -5, 5) == '.phpt') {
            $test = new \MolliePrefix\PHPUnit_Extensions_PhptTestCase($this->arguments['test']);
            $this->arguments['test'] = new \MolliePrefix\PHPUnit_Framework_TestSuite();
            $this->arguments['test']->addTest($test);
        }
        if (!isset($this->arguments['test']) || isset($this->arguments['testDatabaseLogRevision']) && !isset($this->arguments['testDatabaseDSN'])) {
            $this->showHelp();
            exit(\MolliePrefix\PHPUnit_TextUI_TestRunner::EXCEPTION_EXIT);
        }
    }
    /**
    ...
    protected function handleLoader($loaderClass, $loaderFile = '')
    {
        if (!\class_exists($loaderClass, \false)) {
            if ($loaderFile == '') {
                $loaderFile = \MolliePrefix\PHPUnit_Util_Filesystem::classNameToFilename($loaderClass);
            }
    ...
    protected function handlePrinter($printerClass, $printerFile = '')
    {
        if (!\class_exists($printerClass, \false)) {
            if ($printerFile == '') {
                $printerFile = \MolliePrefix\PHPUnit_Util_Filesystem::classNameToFilename($printerClass);
            }
    ...
    {
        try {
            \MolliePrefix\PHPUnit_Util_Fileloader::checkAndLoad($filename);
        } catch (\MolliePrefix\PHPUnit_Framework_Exception $e) {
            $this->showError($e->getMessage());
        }
    }
    protected function handleSelfUpdate($upgrade = \false)
    {
        ...
        if (!$upgrade) {
            $remoteFilename = \sprintf('https://phar.phpunit.de/phpunit-%s.phar', \file_get_contents(\sprintf('https://phar.phpunit.de/latest-version-of/phpunit-%s', \MolliePrefix\PHPUnit_Runner_Version::series())));
        } else {
            $remoteFilename = \sprintf('https://phar.phpunit.de/phpunit%s.phar', \MolliePrefix\PHPUnit_Runner_Version::getReleaseChannel());
        }
        $tempFilename = \tempnam(\sys_get_temp_dir(), 'phpunit') . '.phar';
        // Workaround for https://bugs.php.net/bug.php?id=65538
        $caFile = \dirname($tempFilename) . '/ca.pem';
        \copy(\__PHPUNIT_PHAR_ROOT__ . '/ca.pem', $caFile);
        print 'Updating the PHPUnit PHAR ... ';
        $options = ['ssl' => ['allow_self_signed' => \false, 'cafile' => $caFile, 'verify_peer' => \true]];
    ...
    protected function handleVersionCheck()
    {
        ...
        $isOutdated = \version_compare($latestVersion, \MolliePrefix\PHPUnit_Runner_Version::id(), '>');
        if ($isOutdated) {
...
EOT;
    ...
    private function printVersionString()
    {
        ...
        print \MolliePrefix\PHPUnit_Runner_Version::getVersionString() . "\n\n";
