...
class PHPUnit_Runner_Filter_Test extends \RecursiveFilterIterator
{
    ...
    {
        if (\MolliePrefix\PHPUnit_Util_Regex::pregMatchSafe($filter, '') === \false) {
            // Handles:
    ...
    public function accept()
    {
        ...
        if ($test instanceof \MolliePrefix\PHPUnit_Framework_TestSuite) {
            return \true;
        }
        if ($test instanceof \MolliePrefix\PHPUnit_Framework_WarningTestCase) {
            $name = $test->getMessage();
        } else {
            $tmp = \MolliePrefix\PHPUnit_Util_Test::describe($test, \false);
            if ($tmp[0] != '') {
                $name = \implode('::', $tmp);
            } else {
                $name = $tmp[1];
