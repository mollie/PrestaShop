...
use MolliePrefix\SebastianBergmann\CodeCoverage\Node\File;
...
class Text
{
    ...
    private $colors = ['green' => "\33[30;42m", 'yellow' => "\33[30;43m", 'red' => "\33[37;41m", 'header' => "\33[1;37;40m", 'reset' => "\33[0m", 'eol' => "\33[2K"];
    /**
     ...
     * @return string
     */
    public function process(\MolliePrefix\SebastianBergmann\CodeCoverage\CodeCoverage $coverage, $showColors = \false)
    {
        ...
        $classes = \sprintf('  Classes: %6s (%d/%d)', \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($report->getNumTestedClassesAndTraits(), $report->getNumClassesAndTraits(), \true), $report->getNumTestedClassesAndTraits(), $report->getNumClassesAndTraits());
        $methods = \sprintf('  Methods: %6s (%d/%d)', \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($report->getNumTestedMethods(), $report->getNumMethods(), \true), $report->getNumTestedMethods(), $report->getNumMethods());
        $lines = \sprintf('  Lines:   %6s (%d/%d)', \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($report->getNumExecutedLines(), $report->getNumExecutableLines(), \true), $report->getNumExecutedLines(), $report->getNumExecutableLines());
        ...
        foreach ($report as $item) {
            if (!$item instanceof \MolliePrefix\SebastianBergmann\CodeCoverage\Node\File) {
                continue;
            }
    ...
    {
        $coverage = \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numberOfCoveredElements, $totalNumberOfElements);
    ...
    {
        $format = '%' . $precision . 's';
        return \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numberOfCoveredElements, $totalNumberOfElements, \true, \true) . ' (' . \sprintf($format, $numberOfCoveredElements) . '/' . \sprintf($format, $totalNumberOfElements) . ')';
    }
