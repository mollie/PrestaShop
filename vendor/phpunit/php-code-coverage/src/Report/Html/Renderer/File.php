...
use MolliePrefix\SebastianBergmann\CodeCoverage\Node\File as FileNode;
use MolliePrefix\SebastianBergmann\CodeCoverage\Util;
/**
 * Renders a file node.
 */
class File extends \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Html\Renderer
{
    /**
     * @var int
     */
     ...
     * @param string   $file
     */
    public function render(\MolliePrefix\SebastianBergmann\CodeCoverage\Node\File $node, $file)
    {
        $template = new \MolliePrefix\Text_Template($this->templatePath . 'file.html', '{{', '}}');
     ...
     * @return string
     */
    protected function renderItems(\MolliePrefix\SebastianBergmann\CodeCoverage\Node\File $node)
    {
        $template = new \MolliePrefix\Text_Template($this->templatePath . 'file_item.html', '{{', '}}');
        $methodItemTemplate = new \MolliePrefix\Text_Template($this->templatePath . 'method_item.html', '{{', '}}');
     ...
     * @return string
     */
    protected function renderTraitOrClassItems(array $items, \MolliePrefix\Text_Template $template, \MolliePrefix\Text_Template $methodItemTemplate)
    {
        if (empty($items)) {
            return '';
        }
        ...
        foreach ($items as $name => $item) {
            ...
            if ($item['executableLines'] > 0) {
                ...
                $linesExecutedPercentAsString = \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \true);
            } else {
                $numClasses = 'n/a';
                $numTestedClasses = 'n/a';
                $linesExecutedPercentAsString = 'n/a';
            }
            $buffer .= $this->renderItemTemplate($template, ['name' => $name, 'numClasses' => $numClasses, 'numTestedClasses' => $numTestedClasses, 'numMethods' => $numMethods, 'numTestedMethods' => $numTestedMethods, 'linesExecutedPercent' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \false), 'linesExecutedPercentAsString' => $linesExecutedPercentAsString, 'numExecutedLines' => $item['executedLines'], 'numExecutableLines' => $item['executableLines'], 'testedMethodsPercent' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods, $numMethods, \false), 'testedMethodsPercentAsString' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods, $numMethods, \true), 'testedClassesPercent' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods == $numMethods ? 1 : 0, 1, \false), 'testedClassesPercentAsString' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numTestedMethods == $numMethods ? 1 : 0, 1, \true), 'crap' => $item['crap']]);
            foreach ($item['methods'] as $method) {
     ...
     * @return string
     */
    protected function renderFunctionItems(array $functions, \MolliePrefix\Text_Template $template)
    {
        if (empty($functions)) {
            return '';
        }
     ...
     * @return string
     */
    protected function renderFunctionOrMethodItem(\MolliePrefix\Text_Template $template, array $item, $indent = '')
    {
        $numTestedItems = $item['executedLines'] == $item['executableLines'] ? 1 : 0;
        return $this->renderItemTemplate($template, ['name' => \sprintf('%s<a href="#%d"><abbr title="%s">%s</abbr></a>', $indent, $item['startLine'], \htmlspecialchars($item['signature']), isset($item['functionName']) ? $item['functionName'] : $item['methodName']), 'numMethods' => 1, 'numTestedMethods' => $numTestedItems, 'linesExecutedPercent' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \false), 'linesExecutedPercentAsString' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($item['executedLines'], $item['executableLines'], \true), 'numExecutedLines' => $item['executedLines'], 'numExecutableLines' => $item['executableLines'], 'testedMethodsPercent' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numTestedItems, 1, \false), 'testedMethodsPercentAsString' => \MolliePrefix\SebastianBergmann\CodeCoverage\Util::percent($numTestedItems, 1, \true), 'crap' => $item['crap']]);
    }
    /**
     * @param FileNode $node
     *
     * @return string
     */
    protected function renderSource(\MolliePrefix\SebastianBergmann\CodeCoverage\Node\File $node)
    {
