...
use MolliePrefix\SebastianBergmann\CodeCoverage\Node\AbstractNode;
...
class Facade
{
     ...
     */
    public function process(\MolliePrefix\SebastianBergmann\CodeCoverage\CodeCoverage $coverage, $target)
    {
        ...
        $this->project = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Project($coverage->getReport()->getName());
        $this->processTests($coverage->getTests());
        $this->processDirectory($report, $this->project);
        $index = $this->project->asDom();
        $index->formatOutput = \true;
    ...
    private function initTargetDirectory($directory)
    {
        if (\file_exists($directory)) {
            if (!\is_dir($directory)) {
                throw new \MolliePrefix\SebastianBergmann\CodeCoverage\RuntimeException("'{$directory}' exists but is not a directory.");
            }
            if (!\is_writable($directory)) {
                throw new \MolliePrefix\SebastianBergmann\CodeCoverage\RuntimeException("'{$directory}' exists but is not writable.");
            }
        } elseif (!@\mkdir($directory, 0777, \true)) {
            throw new \MolliePrefix\SebastianBergmann\CodeCoverage\RuntimeException("'{$directory}' could not be created.");
        }
    }
    private function processDirectory(\MolliePrefix\SebastianBergmann\CodeCoverage\Node\Directory $directory, \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Node $context)
    {
    ...
    private function processFile(\MolliePrefix\SebastianBergmann\CodeCoverage\Node\File $file, \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Directory $context)
    {
        $fileObject = $context->addFile($file->getName(), $file->getId() . '.xml');
        $this->setTotals($file, $fileObject->getTotals());
        $fileReport = new \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Report($file->getName());
        ...
        $fileDom->formatOutput = \true;
        $fileDom->preserveWhiteSpace = \false;
        $fileDom->save($this->target . $file->getId() . '.xml');
    }
    private function processUnit($unit, \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Report $report)
    {
    ...
    private function processFunction($function, \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Report $report)
    {
    ...
    private function setTotals(\MolliePrefix\SebastianBergmann\CodeCoverage\Node\AbstractNode $node, \MolliePrefix\SebastianBergmann\CodeCoverage\Report\Xml\Totals $totals)
    {
