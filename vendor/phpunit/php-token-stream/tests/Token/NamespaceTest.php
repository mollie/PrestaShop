 ...
 */
class PHP_Token_NamespaceTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    /**
    ...
    public function testGetName()
    {
        ...
        foreach ($tokenStream as $token) {
            if ($token instanceof \MolliePrefix\PHP_Token_NAMESPACE) {
                $this->assertSame('MolliePrefix\\Foo\\Bar', $token->getName());
            }
        }
    }
    ...
    {
        $tokenStream = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'classInScopedNamespace.php');
        foreach ($tokenStream as $token) {
            if ($token instanceof \MolliePrefix\PHP_Token_NAMESPACE) {
                $this->assertSame(2, $token->getLine());
            }
