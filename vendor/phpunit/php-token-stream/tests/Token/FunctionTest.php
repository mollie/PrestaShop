 ...
 */
class PHP_Token_FunctionTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    protected $functions;
    protected function setUp()
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'source.php');
        foreach ($ts as $token) {
    ...
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'source5.php');
        $f = $ts->getFunctions();
        $c = $ts->getClasses();
        $i = $ts->getInterfaces();
 ...
 * @since      Class available since Release 1.0.0
 */
\class_alias('MolliePrefix\\PHP_Token_FunctionTest', 'MolliePrefix\\PHP_Token_FunctionTest', \false);
