 ...
 */
class PHP_Token_InterfaceTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    protected $class;
    protected $interfaces;
    protected function setUp()
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'source4.php');
        $i = 0;
        foreach ($ts as $token) {
            if ($token instanceof \MolliePrefix\PHP_Token_CLASS) {
                $this->class = $token;
            } elseif ($token instanceof \MolliePrefix\PHP_Token_INTERFACE) {
    ...
    {
        $tokenStream = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'classInNamespace.php');
        foreach ($tokenStream as $token) {
            if ($token instanceof \MolliePrefix\PHP_Token_INTERFACE) {
                $package = $token->getPackage();
                $this->assertSame('MolliePrefix\\Foo\\Bar', $package['namespace']);
            }
        }
    }
    ...
    {
        $tokenStream = new \MolliePrefix\PHP_Token_Stream($filepath);
        $firstClassFound = \false;
        foreach ($tokenStream as $token) {
            if ($firstClassFound === \false && $token instanceof \MolliePrefix\PHP_Token_INTERFACE) {
                $package = $token->getPackage();
            ...
            if ($token instanceof \MolliePrefix\PHP_Token_INTERFACE) {
                ...
                $this->assertSame('MolliePrefix\\Foo\\Baz', $package['namespace']);
                return;
            }
        }
    ...
    {
        $tokenStream = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'classExtendsNamespacedClass.php');
        $firstClassFound = \false;
        foreach ($tokenStream as $token) {
            if ($firstClassFound === \false && $token instanceof \MolliePrefix\PHP_Token_INTERFACE) {
                $package = $token->getPackage();
            ...
            if ($token instanceof \MolliePrefix\PHP_Token_INTERFACE) {
                ...
                $this->assertSame('MolliePrefix\\Other\\Space', $package['namespace']);
                return;
            }
        }
