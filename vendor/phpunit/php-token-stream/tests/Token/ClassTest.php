 ...
 */
class PHP_Token_ClassTest extends \MolliePrefix\PHPUnit_Framework_TestCase
{
    protected $class;
    protected $function;
    protected function setUp()
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'source2.php');
        foreach ($ts as $token) {
    ...
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'issue19.php');
        foreach ($ts as $token) {
            if ($token instanceof \MolliePrefix\PHP_Token_CLASS) {
    ...
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'issue30.php');
    ...
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'class_with_method_that_declares_anonymous_class.php');
    ...
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'class_with_method_that_declares_anonymous_class2.php');
        $classes = $ts->getClasses();
    ...
    {
        $ts = new \MolliePrefix\PHP_Token_Stream(\TEST_FILES_PATH . 'classUsesNamespacedFunction.php');
        $this->assertEmpty($ts->getFunctions());
        $this->assertCount(1, $ts->getClasses());
    }
}
