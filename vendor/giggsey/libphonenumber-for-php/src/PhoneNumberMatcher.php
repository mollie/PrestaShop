...
class PhoneNumberMatcher implements \Iterator
{
    ...
    protected static function init()
    {
         ...
         */
        static::$matchingBrackets = '(?:[' . $openingParens . '])?' . '(?:' . $nonParens . '+' . '[' . $closingParens . '])?' . $nonParens . '+' . '(?:[' . $openingParens . ']' . $nonParens . '+[' . $closingParens . '])' . $bracketPairLimit . $nonParens . '*';
         ...
         * country code
         */
        $digitBlockLimit = \MolliePrefix\libphonenumber\PhoneNumberUtil::MAX_LENGTH_FOR_NSN + \MolliePrefix\libphonenumber\PhoneNumberUtil::MAX_LENGTH_COUNTRY_CODE;
        /*
        ...
        $punctuation = '[' . \MolliePrefix\libphonenumber\PhoneNumberUtil::VALID_PUNCTUATION . ']' . $punctuationLimit;
        // A digits block without punctuation.
        $digitSequence = "\\p{Nd}" . static::limit(1, $digitBlockLimit);
        $leadClassChars = $openingParens . \MolliePrefix\libphonenumber\PhoneNumberUtil::PLUS_CHARS;
        $leadClass = '[' . $leadClassChars . ']';
        static::$leadClass = $leadClass;
        // Init extension patterns from PhoneNumberUtil
        \MolliePrefix\libphonenumber\PhoneNumberUtil::initExtnPatterns();
        // Phone number pattern allowing optional punctuation.
        static::$pattern = '(?:' . $leadClass . $punctuation . ')' . $leadLimit . $digitSequence . '(?:' . $punctuation . $digitSequence . ')' . $blockLimit . '(?:' . \MolliePrefix\libphonenumber\PhoneNumberUtil::$EXTN_PATTERNS_FOR_MATCHING . ')?';
     ...
     */
    public function __construct(\MolliePrefix\libphonenumber\PhoneNumberUtil $util, $text, $country, \MolliePrefix\libphonenumber\Leniency\AbstractLeniency $leniency, $maxTries)
    {
        if ($maxTries < 0) {
            throw new \InvalidArgumentException();
        }
    ...
    {
        $matcher = new \MolliePrefix\libphonenumber\Matcher(static::$pattern, $this->text);
        while ($this->maxTries > 0 && $matcher->find($index)) {
            ...
            $candidate = static::trimAfterFirstMatch(\MolliePrefix\libphonenumber\PhoneNumberUtil::$SECOND_NUMBER_START_PATTERN, $candidate);
    ...
    {
        $trailingCharsMatcher = new \MolliePrefix\libphonenumber\Matcher($pattern, $candidate);
    ...
    public static function isLatinLetter($letter)
    {
        ...
        return \preg_match('/\\p{Latin}/u', $letter) === 1 || \preg_match('/\\pM+/u', $letter) === 1;
    }
    /**
    ...
    protected function extractMatch($candidate, $offset)
    {
        ...
        $dateMatcher = new \MolliePrefix\libphonenumber\Matcher(static::$slashSeparatedDates, $candidate);
        if ($dateMatcher->find()) {
            return null;
        }
        // Skip potential time-stamps.
        $timeStampMatcher = new \MolliePrefix\libphonenumber\Matcher(static::$timeStamps, $candidate);
        if ($timeStampMatcher->find()) {
            $followingText = \mb_substr($this->text, $offset + \mb_strlen($candidate));
            $timeStampSuffixMatcher = new \MolliePrefix\libphonenumber\Matcher(static::$timeStampsSuffix, $followingText);
    ...
    protected function extractInnerMatch($candidate, $offset)
    {
        foreach (static::$innerMatches as $possibleInnerMatch) {
            $groupMatcher = new \MolliePrefix\libphonenumber\Matcher($possibleInnerMatch, $candidate);
            $isFirstMatch = \true;
            while ($groupMatcher->find() && $this->maxTries > 0) {
                if ($isFirstMatch) {
                    // We should handle any group before this one too.
                    $group = static::trimAfterFirstMatch(\MolliePrefix\libphonenumber\PhoneNumberUtil::$UNWANTED_END_CHAR_PATTERN, \mb_substr($candidate, 0, $groupMatcher->start()));
                ...
                $group = static::trimAfterFirstMatch(\MolliePrefix\libphonenumber\PhoneNumberUtil::$UNWANTED_END_CHAR_PATTERN, $groupMatcher->group(1));
    ...
    protected function parseAndVerify($candidate, $offset)
    {
        try {
            ...
            $matchingBracketsMatcher = new \MolliePrefix\libphonenumber\Matcher(static::$matchingBrackets, $candidate);
            $pubPagesMatcher = new \MolliePrefix\libphonenumber\Matcher(static::$pubPages, $candidate);
            if (!$matchingBracketsMatcher->matches() || $pubPagesMatcher->find()) {
                return null;
            }
            // If leniency is set to VALID or stricter, we also want to skip numbers that are surrounded
            // by Latin alphabetic characters, to skip cases like abc8005001234 or 8005001234def.
            if ($this->leniency->compareTo(\MolliePrefix\libphonenumber\Leniency::VALID()) >= 0) {
                // If the candidate is not at the start of the text, and does not start with phone-number
                // punctuation, check the previous character.
                $leadClassMatcher = new \MolliePrefix\libphonenumber\Matcher(static::$leadClass, $candidate);
            ...
            if ($this->leniency->verify($number, $candidate, $this->phoneUtil)) {
                ...
                return new \MolliePrefix\libphonenumber\PhoneNumberMatch($offset, $candidate, $number);
            }
     ...
     * @return bool
     */
    public static function allNumberGroupsRemainGrouped(\MolliePrefix\libphonenumber\PhoneNumberUtil $util, \MolliePrefix\libphonenumber\PhoneNumber $number, $normalizedCandidate, $formattedNumberGroups)
    {
        $fromIndex = 0;
        if ($number->getCountryCodeSource() !== \MolliePrefix\libphonenumber\CountryCodeSource::FROM_DEFAULT_COUNTRY) {
        ...
        for ($i = 0; $i < $formattedNumberGroupsLength; $i++) {
            ...
            if ($i === 0 && $fromIndex < \mb_strlen($normalizedCandidate)) {
                ...
                if ($util->getNddPrefixForRegion($region, \true) !== null && \is_int(\mb_substr($normalizedCandidate, $fromIndex, 1))) {
                    // This means there is no formatting symbol after the NDC. In this case, we only
                    // accept the number if there is no formatting symbol at all in the number, except
                    // for extensions. This is only important for countries with national prefixes.
                    $nationalSignificantNumber = $util->getNationalSignificantNumber($number);
                    return \mb_substr(\mb_substr($normalizedCandidate, $fromIndex - \mb_strlen($formattedNumberGroups[$i])), \mb_strlen($nationalSignificantNumber)) === $nationalSignificantNumber;
                }
            }
        }
     ...
     * @return bool
     */
    public static function allNumberGroupsAreExactlyPresent(\MolliePrefix\libphonenumber\PhoneNumberUtil $util, \MolliePrefix\libphonenumber\PhoneNumber $number, $normalizedCandidate, $formattedNumberGroups)
    {
        $candidateGroups = \preg_split(\MolliePrefix\libphonenumber\PhoneNumberUtil::NON_DIGITS_PATTERN, $normalizedCandidate);
        ...
        if (\count($candidateGroups) == 1 || \mb_strpos($candidateGroups[$candidateNumberGroupIndex], $util->getNationalSignificantNumber($number)) !== \false) {
            return \true;
        }
        // Starting from the end, go through in reverse, excluding the first group, and check the
        // candidate and number groups are the same.
        for ($formattedNumberGroupIndex = \count($formattedNumberGroups) - 1; $formattedNumberGroupIndex > 0 && $candidateNumberGroupIndex >= 0; $formattedNumberGroupIndex--, $candidateNumberGroupIndex--) {
        ...
        return $candidateNumberGroupIndex >= 0 && \mb_substr($candidateGroups[$candidateNumberGroupIndex], -\mb_strlen($formattedNumberGroups[0])) == $formattedNumberGroups[0];
    }
    /**
     ...
     * @return string[]
     */
    protected static function getNationalNumberGroups(\MolliePrefix\libphonenumber\PhoneNumberUtil $util, \MolliePrefix\libphonenumber\PhoneNumber $number, \MolliePrefix\libphonenumber\NumberFormat $formattingPattern = null)
    {
        if ($formattingPattern === null) {
            // This will be in the format +CC-DG;ext=EXT where DG represents groups of digits.
            $rfc3966Format = $util->format($number, \MolliePrefix\libphonenumber\PhoneNumberFormat::RFC3966);
        ...
        $nationalSignificantNumber = $util->getNationalSignificantNumber($number);
        return \explode('-', $util->formatNsnUsingPattern($nationalSignificantNumber, $formattingPattern, \MolliePrefix\libphonenumber\PhoneNumberFormat::RFC3966));
    }
    /**
     * @param PhoneNumber $number
     * @param string $candidate
     ...
     * @return bool
     */
    public static function checkNumberGroupingIsValid(\MolliePrefix\libphonenumber\PhoneNumber $number, $candidate, \MolliePrefix\libphonenumber\PhoneNumberUtil $util, \Closure $checker)
    {
        $normalizedCandidate = \MolliePrefix\libphonenumber\PhoneNumberUtil::normalizeDigits($candidate, \true);
        ...
        if ($alternateFormats !== null) {
            foreach ($alternateFormats->numberFormats() as $alternateFormat) {
                if ($alternateFormat->leadingDigitsPatternSize() > 0) {
                    ...
                    $nationalSignificantNumberMatcher = new \MolliePrefix\libphonenumber\Matcher($pattern, $nationalSignificantNumber);
     ...
     * @return bool
     */
    public static function containsMoreThanOneSlashInNationalNumber(\MolliePrefix\libphonenumber\PhoneNumber $number, $candidate)
    {
        ...
        $candidateHasCountryCode = $number->getCountryCodeSource() === \MolliePrefix\libphonenumber\CountryCodeSource::FROM_NUMBER_WITH_PLUS_SIGN || $number->getCountryCodeSource() === \MolliePrefix\libphonenumber\CountryCodeSource::FROM_NUMBER_WITHOUT_PLUS_SIGN;
        if ($candidateHasCountryCode && \MolliePrefix\libphonenumber\PhoneNumberUtil::normalizeDigitsOnly(\mb_substr($candidate, 0, $firstSlashInBodyIndex)) == $number->getCountryCode()) {
     ...
     * @return bool
     */
    public static function containsOnlyValidXChars(\MolliePrefix\libphonenumber\PhoneNumber $number, $candidate, \MolliePrefix\libphonenumber\PhoneNumberUtil $util)
    {
        ...
        for ($index = 0; $index < $candidateLength - 1; $index++) {
            ...
            if ($charAtIndex == 'x' || $charAtIndex == 'X') {
                ...
                if ($charAtNextIndex == 'x' || $charAtNextIndex == 'X') {
                    ...
                    // significant number.
                    $index++;
                    if ($util->isNumberMatch($number, \mb_substr($candidate, $index)) != \MolliePrefix\libphonenumber\MatchType::NSN_MATCH) {
                        return \false;
                    }
                } elseif (!\MolliePrefix\libphonenumber\PhoneNumberUtil::normalizeDigitsOnly(\mb_substr($candidate, $index)) == $number->getExtension()) {
     ...
     * @return bool
     */
    public static function isNationalPrefixPresentIfRequired(\MolliePrefix\libphonenumber\PhoneNumber $number, \MolliePrefix\libphonenumber\PhoneNumberUtil $util)
    {
        ...
        if ($formatRule !== null && \mb_strlen($formatRule->getNationalPrefixFormattingRule()) > 0) {
            ...
            if (\MolliePrefix\libphonenumber\PhoneNumberUtil::formattingRuleHasFirstGroupOnly($formatRule->getNationalPrefixFormattingRule())) {
                // National Prefix not needed for this number.
                return \true;
            }
            // Normalize the remainder.
            $rawInputCopy = \MolliePrefix\libphonenumber\PhoneNumberUtil::normalizeDigitsOnly($number->getRawInput());
    ...
    {
        $countryCodeSet = \MolliePrefix\libphonenumber\AlternateFormatsCountryCodeSet::$alternateFormatsCountryCodeSet;
    ...
    protected static function loadAlternateFormatsMetadataFromFile($countryCallingCode)
    {
        ...
        $metadata = new \MolliePrefix\libphonenumber\PhoneMetadata();
